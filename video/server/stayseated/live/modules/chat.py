...
class ChatModule:
    async def join(self):
        ...
        if not room_config:
            await self.consumer.send_json(['error', self.content[1], {'message': 'Unknown room ID.'}])
            return
        if "chat.native" not in [m['type'] for m in room_config['modules']]:
            await self.consumer.send_json(['error', self.content[1], {'message': 'Room does not contain a chat.'}])
            return
