...
if not github_token:
    ...
    pyproject['project']['dependencies'] = [
        dep for dep in pyproject['project']['dependencies'] if not (dep.startswith('eventyay-stripe') or dep.startswith('eventyay-paypal'))
    ]
else:
    ...
    for i, dep in enumerate(pyproject['project']['dependencies']):
        if dep.startswith('eventyay-stripe'):
            ...
            pyproject['project']['dependencies'][i] = f'eventyay-paypal @ git+https://{github_token}@github.com/fossasia/eventyay-tickets-paypal.git@master'
