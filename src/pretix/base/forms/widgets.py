...
class DatePickerWidget(forms.DateInput):
    def __init__(self, attrs=None, date_format=None):
        ...
        def placeholder():
            ...
            return now().replace(year=2000, month=12, day=31, hour=18, minute=0, second=0, microsecond=0).strftime(df)
...
class SplitDateTimePickerWidget(forms.SplitDateTimeWidget):
    ...
    def __init__(self, attrs=None, date_format=None, time_format=None, min_date=None, max_date=None):
        ...
        time_attrs['autocomplete'] = 'off'
        if min_date:
            date_attrs['data-min'] = (min_date if isinstance(min_date, date) else min_date.astimezone(get_current_timezone()).date()).isoformat()
        if max_date:
            date_attrs['data-max'] = (max_date if isinstance(max_date, date) else max_date.astimezone(get_current_timezone()).date()).isoformat()
