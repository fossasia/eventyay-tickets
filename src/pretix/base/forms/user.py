...
class UserSettingsForm(forms.ModelForm):
    error_messages = {
        'duplicate_identifier': _('There already is an account associated with this e-mail address. Please choose a different one.'),
    ...
    old_pw = forms.CharField(max_length=255, required=False, label=_('Your current password'), widget=forms.PasswordInput())
    new_pw = forms.CharField(max_length=255, required=False, label=_('New password'), widget=forms.PasswordInput())
    new_pw_repeat = forms.CharField(max_length=255, required=False, label=_('Repeat new password'), widget=forms.PasswordInput())
    timezone = forms.ChoiceField(
        choices=((a, a) for a in common_timezones),
        label=_('Default timezone'),
        help_text=_('Only used for views that are not bound to an event. For all event views, the event timezone is used instead.'),
    )
