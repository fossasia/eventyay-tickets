...
class SettingsForm(i18nfield.forms.I18nFormMixin, HierarkeyForm):
    ...
    def get_new_filename(self, name: str) -> str:
        ...
        if isinstance(self.obj, Event):
            fname = '%s/%s/%s.%s.%s' % (self.obj.organizer.slug, self.obj.slug, name, nonce, name.split('.')[-1])
        else:
...
class SafeSessionWizardView(SessionWizardView):
    ...
    def get_context_data(self, form, **kwargs):
        ...
        context['wizard']['prefix_form'] = PrefixForm(prefix=super().get_prefix(self.request), initial={'prefix': self.get_prefix(self.request)})
        return context
...
class I18nMarkdownTextarea(i18nfield.forms.I18nTextarea):
    def format_output(self, rendered_widgets) -> str:
        markdown_note = _('You can use {name} in this field.').format(name='<a href="https://en.wikipedia.org/wiki/Markdown" target="_blank">Markdown</a>')
