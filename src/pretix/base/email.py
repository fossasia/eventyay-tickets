...
class SendGridEmail:
    ...
    def test(self, from_addr):
        message = Mail(from_email=from_addr, to_emails='testdummy@eventyay.com', subject='Eventyay test email', html_content='Eventyay test email')
    ...
    def send_messages(self, emails):
        for email in emails:
            ...
            message = Mail(from_email=email.from_email, to_emails=email.to, subject=email.subject, html_content=html_content)
            sg = SendGridAPIClient(self.api_key)
            bcc = []
...
class BaseHTMLMailRenderer:
    ...
    def render(self, plain_body: str, plain_signature: str, subject: str, order=None, position=None) -> str:
        """
...
class TemplateBasedMailRenderer(BaseHTMLMailRenderer):
    ...
    def render(self, plain_body: str, plain_signature: str, subject: str, order, position) -> str:
        ...
        if order:
            htmlctx['order'] = order
            positions = list(order.positions.select_related('item', 'variation', 'subevent', 'addon_to').annotate(has_addons=Count('addons')))
            htmlctx['cart'] = [
                (k, list(v))
                for k, v in groupby(
                    positions,
...
def base_placeholders(sender, **kwargs):
    ...
    ph = [
        ...
        SimpleFunctionalMailTextPlaceholder(
            'event', ['event_or_subevent'], lambda event_or_subevent: event_or_subevent.name, lambda event_or_subevent: event_or_subevent.name
        ),
        SimpleFunctionalMailTextPlaceholder('event_slug', ['event'], lambda event: event.slug, lambda event: event.slug),
        SimpleFunctionalMailTextPlaceholder('code', ['order'], lambda order: order.code, 'F8VVL'),
        SimpleFunctionalMailTextPlaceholder('total', ['order'], lambda order: LazyNumber(order.total), lambda event: LazyNumber(Decimal('42.23'))),
        SimpleFunctionalMailTextPlaceholder('currency', ['event'], lambda event: event.currency, lambda event: event.currency),
        SimpleFunctionalMailTextPlaceholder(
            'refund_amount',
        ...
        SimpleFunctionalMailTextPlaceholder(
            ...
            lambda order, event: build_absolute_uri(
                event, 'presale:event.order.open', kwargs={'order': order.code, 'secret': order.secret, 'hash': order.email_confirm_hash()}
            ),
            lambda event: build_absolute_uri(
                event, 'presale:event.order.open', kwargs={'order': 'F8VVL', 'secret': '6zzjnumtsx136ddy', 'hash': '98kusd8ofsj8dnkd'}
            ),
        ),
        SimpleFunctionalMailTextPlaceholder(
            'url_info_change',
        ...
        SimpleFunctionalMailTextPlaceholder(
            ...
            lambda event, position: build_absolute_uri(
                event, 'presale:event.order.position', kwargs={'order': position.order.code, 'secret': position.web_secret, 'position': position.positionid}
            ),
            lambda event: build_absolute_uri(event, 'presale:event.order.position', kwargs={'order': 'F8VVL', 'secret': '6zzjnumtsx136ddy', 'position': '123'}),
        ),
        SimpleFunctionalMailTextPlaceholder(
            'url',
            ['waiting_list_entry', 'event'],
            lambda waiting_list_entry, event: build_absolute_uri(event, 'presale:event.redeem') + '?voucher=' + waiting_list_entry.voucher.code,
            lambda event: build_absolute_uri(
                event,
        ...
        SimpleFunctionalMailTextPlaceholder(
            'invoice_company', ['invoice_address'], lambda invoice_address: invoice_address.company or '', _('Sample Corporation')
        ),
        SimpleFunctionalMailTextPlaceholder(
            'orders',
            ['event', 'orders'],
        ...
        SimpleFunctionalMailTextPlaceholder(
            'product', ['waiting_list_entry'], lambda waiting_list_entry: waiting_list_entry.item.name, _('Sample Admission Ticket')
        ),
        SimpleFunctionalMailTextPlaceholder('code', ['waiting_list_entry'], lambda waiting_list_entry: waiting_list_entry.voucher.code, '68CYU2H6ZTP3WLK5'),
        SimpleFunctionalMailTextPlaceholder(
    ...
    for f, l, w in name_scheme['fields']:
        ...
        ph.append(
            SimpleFunctionalMailTextPlaceholder(
                'attendee_name_%s' % f, ['position'], lambda position, f=f: position.attendee_name_parts.get(f, ''), name_scheme['sample'][f]
            )
        )
        ph.append(
            SimpleFunctionalMailTextPlaceholder(
    ...
    for k, v in sender.meta_data.items():
        ph.append(SimpleFunctionalMailTextPlaceholder('meta_%s' % k, ['event'], lambda event, k=k: event.meta_data[k], v))
