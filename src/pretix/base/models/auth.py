...
class User(AbstractBaseUser, PermissionsMixin, LoggingMixin):
    ...
    email = models.EmailField(unique=True, db_index=True, null=True, blank=True, verbose_name=_('E-mail'), max_length=190)
    ...
    locale = models.CharField(max_length=50, choices=settings.LANGUAGES, default=settings.LANGUAGE_CODE, verbose_name=_('Language'))
    ...
    notifications_send = models.BooleanField(
        default=True, verbose_name=_('Receive notifications according to my settings below'), help_text=_('If turned off, you will not get any notifications.')
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if is_new:
            self.notification_settings.create(action_type='pretix.event.order.refund.requested', event=None, method='mail', enabled=True)
    ...
    def send_security_notice(self, messages, email=None):
        ...
        try:
            ...
            mail(
                ...
                {'user': self, 'messages': msg, 'url': build_absolute_uri('control:user.settings')},
                event=None,
                user=self,
                locale=self.locale,
            )
    ...
    def send_password_reset(self):
        ...
        mail(
            ...
            {
                'user': self,
                'url': (build_absolute_uri('control:auth.forgot.recover') + '?id=%d&token=%s' % (self.id, default_token_generator.make_token(self))),
            },
            None,
            locale=self.locale,
            user=self,
    ...
    def _get_teams_for_event(self, organizer, event):
        if 'e{}'.format(event.pk) not in self._teamcache:
            self._teamcache['e{}'.format(event.pk)] = list(self.teams.filter(organizer=organizer).filter(Q(all_events=True) | Q(limit_events=event)))
...
class U2FDevice(Device):
    ...
    def webauthnpubkey(self):
        ...
        pub_key = binascii.unhexlify('A5010203262001215820{:064x}225820{:064x}'.format(pub_key.public_numbers().x, pub_key.public_numbers().y))
        return pub_key
