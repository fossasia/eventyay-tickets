...
class Voucher(LoggedModel):
    ...
    * If you select an item that has variations but do not select a variation, you cannot set block_quota
    ...
    code = models.CharField(verbose_name=_('Voucher code'), max_length=255, default=generate_code, db_index=True, validators=[MinLengthValidator(5)])
    max_usages = models.PositiveIntegerField(verbose_name=_('Maximum usages'), help_text=_('Number of times this voucher can be redeemed.'), default=1)
    ...
    price_mode = models.CharField(verbose_name=_('Price mode'), max_length=100, choices=PriceModeChoices.choices, default=PriceModeChoices.NONE)
    ...
    show_hidden_items = models.BooleanField(verbose_name=_('Shows hidden products that match this voucher'), default=True)
    ...
    def clean_item_properties(data, event, quota, item, variation, block_quota=False, seats_given=False):
        if quota:
            ...
            if variation and (not item or not item.has_variations):
                raise ValidationError(_('You cannot select a variation without having selected a product that provides variations.'))
            ...
            if item.has_variations and not variation and data.get('block_quota'):
                raise ValidationError(
                    _('You can only block quota if you specify a specific product variation. Otherwise it might be unclear which quotas to block.')
                )
            ...
            raise ValidationError(_('You need to select a specific product or quota if this voucher should reserve tickets.'))
        elif variation:
            raise ValidationError(_('You cannot select a variation without having selected a product that provides variations.'))
    ...
    def clean_max_usages(data, redeemed):
        if data.get('max_usages', 1) < redeemed:
            raise ValidationError(
                _('This voucher has already been redeemed %(redeemed)s times. You cannot reduce the maximum number of usages below this number.'),
    ...
    def clean_quota_check(data, cnt, old_instance, event, quota, item, variation):
        ...
        if avail[0] != Quota.AVAILABILITY_OK or (avail[1] is not None and avail[1] < cnt):
            raise ValidationError(
                _('You cannot create a voucher that blocks quota as the selected product or quota is currently sold out or completely reserved.')
            )
    ...
    def clean_seat_id(data, item, quota, event, pk):
        try:
            if event.has_subevents:
                ...
                seat = event.seats.select_related('product').get(seat_guid=data.get('seat'), subevent=data.get('subevent'))
            else:
                seat = event.seats.select_related('product').get(seat_guid=data.get('seat'))
        except Seat.DoesNotExist:
            raise ValidationError(_('The specified seat ID "{id}" does not exist for this event.').format(id=data.get('seat')))

        if not seat.is_available(ignore_voucher_id=pk, ignore_cart=True):
            raise ValidationError(_('The seat "{id}" is currently unavailable (blocked, already sold or a different voucher).').format(id=seat.seat_guid))
    ...
    def annotate_budget_used_orders(cls, qs):
        opq = (
            OrderPosition.objects.filter(
                voucher_id=OuterRef('pk'), price_before_voucher__isnull=False, order__status__in=[Order.STATUS_PAID, Order.STATUS_PENDING]
            )
            .order_by()
            .values('voucher_id')
    ...
    def budget_used(self):
        ops = OrderPosition.objects.filter(
            voucher=self, price_before_voucher__isnull=False, order__status__in=[Order.STATUS_PAID, Order.STATUS_PENDING]
...
class InvoiceVoucher(LoggedModel):
    code = models.CharField(
        verbose_name=_('Voucher code'), max_length=255, default=generate_code, db_index=True, validators=[MinLengthValidator(5)], unique=True
    )
    max_usages = models.PositiveIntegerField(verbose_name=_('Maximum usages'), help_text=_('Number of times this voucher can be redeemed.'), default=1)
    redeemed = models.PositiveIntegerField(verbose_name=_('Redeemed'), default=0)
    budget = models.DecimalField(
    ...
    limit_events = models.ManyToManyField('Event', verbose_name=_('Limit to events'), blank=True, related_name='invoice_vouchers')

    limit_organizer = models.ManyToManyField('Organizer', verbose_name=_('Limit to Organizer'), blank=True, related_name='invoice_vouchers')
