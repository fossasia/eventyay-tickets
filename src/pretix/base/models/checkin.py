...
class CheckinList(LoggedModel):
    ...
    subevent = models.ForeignKey('SubEvent', null=True, blank=True, verbose_name=pgettext_lazy('subevent', 'Date'), on_delete=models.CASCADE)
    ...
    gates = models.ManyToManyField(
        ...
        blank=True,
        help_text=_('Does not have any effect for the validation of tickets, only for the automatic configuration of check-in devices.'),
    )
    ...
    def position_count(self):
        return self.event.cache.get_or_set('checkin_list_{}_position_count'.format(self.pk), lambda: self.positions.count(), 60)
    ...
    def validate_rules(cls, rules, seen_nonbool=False, depth=0):
        ...
        # * in checkinrules.js
        # * in libpretixsync
        top_level_operators = {'<', '<=', '>', '>=', '==', '!=', 'inList', 'isBefore', 'isAfter', 'or', 'and'}
        ...
        allowed_vars = {'product', 'variation', 'now', 'entries_number', 'entries_today', 'entries_days'}
...
class Checkin(models.Model):
    ...
    device = models.ForeignKey('pretixbase.Device', related_name='checkins', on_delete=models.PROTECT, null=True, blank=True)
    gate = models.ForeignKey('pretixbase.Gate', related_name='checkins', on_delete=models.SET_NULL, null=True, blank=True)
