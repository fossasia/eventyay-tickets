...
class WaitingListEntry(LoggedModel):
    ...
    voucher = models.ForeignKey(
        'Voucher', verbose_name=_('Assigned voucher'), null=True, blank=True, related_name='waitinglistentries', on_delete=models.CASCADE
    )
    item = models.ForeignKey(
        Item, related_name='waitinglistentries', on_delete=models.CASCADE, verbose_name=_('Product'), help_text=_('The product the user waits for.')
    )
    ...
    def clean_duplicate(email, item, variation, subevent, pk):
        if (
            WaitingListEntry.objects.filter(item=item, variation=variation, email__iexact=email, voucher__isnull=True, subevent=subevent)
            .exclude(pk=pk)
            .exists()
        ):
            raise ValidationError(_('You are already on this waiting list! We will notify you as soon as we have a ticket available for you.'))
