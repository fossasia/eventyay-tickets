...
def itempicture_upload_to(instance, filename: str) -> str:
    return 'pub/%s/%s/item-%s-%s.%s' % (instance.event.organizer.slug, instance.event.slug, instance.id, str(uuid.uuid4()), filename.split('.')[-1])
...
class Item(LoggedModel):
    ...
    tax_rule = models.ForeignKey('TaxRule', verbose_name=_('Sales tax'), on_delete=models.PROTECT, null=True, blank=True)
    admission = models.BooleanField(
        verbose_name=_('Is an admission ticket'), help_text=_('Whether or not buying this product allows a person to enter your event'), default=False
    )
    ...
    allow_waitinglist = models.BooleanField(
        verbose_name=_('Show a waiting list for this ticket'), help_text=_('This will only work if waiting lists are enabled for this event.'), default=True
    )
    ...
    position = models.IntegerField(default=0)
    picture = models.ImageField(verbose_name=_('Product picture'), null=True, blank=True, max_length=255, upload_to=itempicture_upload_to)
    available_from = models.DateTimeField(
        verbose_name=_('Available from'), null=True, blank=True, help_text=_('This product will not be sold before the given date.')
    )
    available_until = models.DateTimeField(
        verbose_name=_('Available until'), null=True, blank=True, help_text=_('This product will not be sold after the given date.')
    )
    ...
    require_voucher = models.BooleanField(
        ...
        default=False,
        help_text=_('To buy this product, the user needs a voucher that applies to this product either directly or via a quota.'),
    )
    ...
    hide_without_voucher = models.BooleanField(
        ...
        default=False,
        help_text=_('This product will be hidden from the event page until the user enters a voucher that unlocks this product.'),
    )
    ...
    issue_giftcard = models.BooleanField(
        ...
        help_text=_('When a customer buys this product, they will get a gift card with a value corresponding to the product price.'),
        default=False,
    )
    ...
    def tax(self, price=None, base_price_is='auto', currency=None, invoice_address=None, override_tax_rate=None, include_bundled=False):
        ...
        if not self.tax_rule:
            t = TaxedPrice(gross=price, net=price, tax=Decimal('0.00'), rate=Decimal('0.00'), name='')
        else:
            t = self.tax_rule.tax(
                price,
                ...
                if b.designated_price and b.bundled_item.tax_rule_id != self.tax_rule_id:
                    if b.bundled_variation:
                        bprice = b.bundled_variation.tax(
                            b.designated_price * b.count, base_price_is='gross', invoice_address=invoice_address, currency=currency
                        )
                    else:
                        bprice = b.bundled_item.tax(b.designated_price * b.count, invoice_address=invoice_address, base_price_is='gross', currency=currency)
                    compare_price = self.tax_rule.tax(
                        b.designated_price * b.count, override_tax_rate=override_tax_rate, invoice_address=invoice_address, currency=currency
                    )
    ...
    def check_quotas(
        self, ignored_quotas=None, count_waitinglist=True, subevent=None, _cache=None, include_bundled=False, trust_parameters=False, fail_on_no_quotas=False
    ):
        """
    ...
    def clean_per_order(min_per_order, max_per_order):
        if min_per_order is not None and max_per_order is not None:
            if min_per_order > max_per_order:
                raise ValidationError(_('The maximum number per order can not be lower than the minimum number per order.'))
...
class ItemVariation(models.Model):
    ...
    def tax(self, price=None, base_price_is='auto', currency=None, include_bundled=False, override_tax_rate=None, invoice_address=None):
        ...
        if not self.item.tax_rule:
            ...
            t = self.item.tax_rule.tax(
                price, base_price_is=base_price_is, currency=currency, override_tax_rate=override_tax_rate, invoice_address=invoice_address
            )
        ...
        if include_bundled:
            for b in self.item.bundles.all():
                if b.designated_price and b.bundled_item.tax_rule_id != self.item.tax_rule_id:
                    if b.bundled_variation:
                        bprice = b.bundled_variation.tax(
                            b.designated_price * b.count, base_price_is='gross', currency=currency, invoice_address=invoice_address
                        )
                    else:
                        bprice = b.bundled_item.tax(b.designated_price * b.count, base_price_is='gross', currency=currency, invoice_address=invoice_address)
    ...
    def allow_delete(self):
        ...
        return not OrderPosition.objects.filter(variation=self).exists() and not CartPosition.objects.filter(variation=self).exists()
...
class ItemAddOn(models.Model):
    ...
    addon_category = models.ForeignKey(ItemCategory, related_name='addon_to', verbose_name=_('Category'), on_delete=models.CASCADE)
    ...
    price_included = models.BooleanField(
        ...
        help_text=_('If selected, adding add-ons to this ticket is free, even if the add-ons would normally cost money individually.'),
    )
...
class ItemBundle(models.Model):
    ...
    bundled_item = models.ForeignKey(Item, related_name='bundled_with', verbose_name=_('Bundled item'), on_delete=models.CASCADE)
    bundled_variation = models.ForeignKey(
        ItemVariation, related_name='bundled_with', verbose_name=_('Bundled variation'), null=True, blank=True, on_delete=models.CASCADE
    )
...
class Question(LoggedModel):
    ...
    * a date and a time (``TYPE_DATETIME``)
    ...
    identifier = models.CharField(
        ...
        help_text=_(
            'You can enter any value here to make it easier to match the data with other sources. If you do not input one, we will generate one automatically.'
        ),
    )
    ...
    items = models.ManyToManyField(
        Item, related_name='questions', verbose_name=_('Products'), blank=True, help_text=_('This question will be asked to buyers of the selected products')
    )
    position = models.PositiveIntegerField(default=0, verbose_name=_('Position'))
    ...
    hidden = models.BooleanField(verbose_name=_('Hidden question'), help_text=_('This question will only show up in the backend.'), default=False)
    print_on_invoice = models.BooleanField(verbose_name=_('Print answer on invoices'), default=False)
    dependency_question = models.ForeignKey('Question', null=True, blank=True, on_delete=models.SET_NULL, related_name='dependent_questions')
    ...
    valid_date_min = models.DateField(
        null=True, blank=True, verbose_name=_('Minimum value'), help_text=_('Currently not supported in our apps and during check-in')
    )
    valid_date_max = models.DateField(
        null=True, blank=True, verbose_name=_('Maximum value'), help_text=_('Currently not supported in our apps and during check-in')
    )
    ...
    def clean_answer(self, answer):
        ...
        if self.type == Question.TYPE_CHOICE:
            ...
            if isinstance(answer, str):
                l_ = list(self.options.filter(Q(pk__in=[a for a in answer.split(',') if a.isdigit()]) | Q(identifier__in=answer.split(','))))
                llen = len(answer.split(','))
            elif all(isinstance(o, QuestionOption) for o in answer):
                return o
            else:
                l_ = list(self.options.filter(Q(pk__in=[a for a in answer if isinstance(a, int) or a.isdigit()]) | Q(identifier__in=answer)))
                llen = len(answer)
            if len(l_) != llen:
                raise ValidationError(_('Invalid option selected.'))
            return l_
...
class Quota(LoggedModel):
    ...
    size = models.PositiveIntegerField(verbose_name=_('Total capacity'), null=True, blank=True, help_text=_('Leave empty for an unlimited number of tickets.'))
    ...
    def availability(self, now_dt: datetime = None, count_waitinglist=True, _cache=None, allow_cache=False) -> Tuple[int, int]:
        """
