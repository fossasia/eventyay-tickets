...
class BasePaymentProvider:
    ...
    def calculate_fee(self, price: Decimal) -> Decimal:
        ...
        if fee_reverse_calc:
            return ((price + fee_abs) * (1 / (1 - fee_percent / 100)) - price).quantize(Decimal('1') / 10**places, ROUND_HALF_UP)
        else:
            ...
            [
                ...
                (
                    ...
                    forms.MultipleChoiceField(
                        ...
                        choices=((c.identifier, c.verbose_name) for c in get_all_sales_channels().values() if c.payment_restrictions_supported),
                        initial=['web'],
                ...
                (
                    ...
                    forms.BooleanField(
                        ...
                        required=False,
                        help_text=_('The payment method will not be shown by default but only to people who enter the shop through a special link.'),
                    ),
                ),
                (
                    '_hidden_url',
    ...
    def payment_form(self, request: HttpRequest) -> Form:
        ...
        form = self.payment_form_class(
            ...
            initial={k.replace('payment_%s_' % self.identifier, ''): v for k, v in request.session.items() if k.startswith('payment_%s_' % self.identifier)},
        )
            ...
            if self.event.has_subevents and cart_id:
                dates = [
                    ...
                    for se in self.event.subevents.filter(
                        id__in=CartPosition.objects.filter(cart_id=cart_id, event=self.event).values_list('subevent', flat=True)
                    )
                ]
                availability_date = min(dates) if dates else None
            elif self.event.has_subevents and order:
                dates = [availability_date.datetime(se).date() for se in self.event.subevents.filter(id__in=order.positions.values_list('subevent', flat=True))]
                availability_date = min(dates) if dates else None
...
class ManualPayment(BasePaymentProvider):
    ...
    def test_mode_message(self):
        return _('In test mode, you can just manually mark this order as paid in the backend after it has been created.')
    ...
    def payment_pending_render(self, request, payment) -> str:
        return rich_text(str(self.settings.get('pending_description', as_type=LazyI18nString)).format_map(self.format_map(payment.order, payment)))
...
class GiftCardPayment(BasePaymentProvider):
    ...
    def checkout_prepare(self, request: HttpRequest, cart: Dict[str, Any]) -> Union[bool, str, None]:
        ...
        try:
            ...
            # applied.
            fees = get_fees(self.event, request, total, cart['invoice_address'], cs.get('payment'), cart['raw'])
            ...
            if remainder > Decimal('0.00'):
                ...
                messages.success(
                    request,
                    _('Your gift card has been applied, but {} still need to be paid. Please select a payment method.').format(
            ...
            if self.event.vouchers.filter(code__iexact=request.POST.get('giftcard')).exists():
                messages.warning(
                    request,
                    _('You entered a voucher instead of a gift card. Vouchers can only be entered on the first page of the shop below the product selection.'),
                )
            else:
