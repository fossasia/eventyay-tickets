...
def timeline_for_event(event, subevent=None):
    ...
    if subevent:
        ev_edit_url = reverse('control:event.subevent', kwargs={'event': event.slug, 'organizer': event.organizer.slug, 'subevent': subevent.pk})
    else:
        ev_edit_url = reverse('control:event.settings', kwargs={'event': event.slug, 'organizer': event.organizer.slug})

    tl.append(
        TimelineEvent(event=event, subevent=subevent, datetime=ev.date_from, description=pgettext_lazy('timeline', 'Your event starts'), edit_url=ev_edit_url)
    )

    if ev.date_to:
        tl.append(
            TimelineEvent(event=event, subevent=subevent, datetime=ev.date_to, description=pgettext_lazy('timeline', 'Your event ends'), edit_url=ev_edit_url)
        )
    ...
    if ev.presale_start:
        tl.append(
            TimelineEvent(
                event=event, subevent=subevent, datetime=ev.presale_start, description=pgettext_lazy('timeline', 'Start of ticket sales'), edit_url=ev_edit_url
            )
        )
    ...
    if ev.presale_end:
        tl.append(
            TimelineEvent(
                event=event, subevent=subevent, datetime=ev.presale_end, description=pgettext_lazy('timeline', 'End of ticket sales'), edit_url=ev_edit_url
            )
        )
    ...
    if rd:
        d = make_aware(datetime.combine(rd.date(ev), time(hour=23, minute=59, second=59)), event.timezone)
        tl.append(
            TimelineEvent(
                event=event,
                subevent=subevent,
                ...
                edit_url=reverse('control:event.settings.payment', kwargs={'event': event.slug, 'organizer': event.organizer.slug}),
            )
        )
    ...
    if rd and event.settings.ticket_download:
        tl.append(
            TimelineEvent(
                ...
                edit_url=reverse('control:event.settings.tickets', kwargs={'event': event.slug, 'organizer': event.organizer.slug}),
            )
        )
    ...
    if rd and event.settings.cancel_allow_user:
        tl.append(
            TimelineEvent(
                ...
                edit_url=reverse('control:event.settings.cancel', kwargs={'event': event.slug, 'organizer': event.organizer.slug}),
            )
        )
        ...
        if days is not None and event.settings.ticket_download:
            ...
            tl.append(
                TimelineEvent(
                    ...
                    edit_url=reverse('control:event.settings.mail', kwargs={'event': event.slug, 'organizer': event.organizer.slug}),
                )
            )
    ...
    for pprov in pprovs.values():
        ...
        if availability_date:
            d = make_aware(datetime.combine(availability_date.date(ev), time(hour=23, minute=59, second=59)), event.timezone)
            tl.append(
                TimelineEvent(
                    event=event,
                    subevent=subevent,
                    datetime=d,
                    description=pgettext_lazy('timeline', 'Payment provider "{name}" can no longer be selected').format(name=str(pprov.verbose_name)),
                    edit_url=reverse(
