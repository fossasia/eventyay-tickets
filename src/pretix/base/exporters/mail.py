...
class MailExporter(BaseExporter):
    ...
    def render(self, form_data: dict):
        ...
        pos = OrderPosition.objects.filter(order__event__in=self.events, order__status__in=form_data['status']).values('attendee_email')
        data = '\r\n'.join(set(a['email'] for a in addrs if a['email']) | set(a['attendee_email'] for a in pos if a['attendee_email']))
