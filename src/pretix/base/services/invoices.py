...
{country}""",
            ...
            invoice.invoice_to = '\n'.join(
                a.strip()
                for a in addr_template.format(i=ia, country=ia.country.name if ia.country else ia.country_old, state=ia.state_for_address).split('\n')
                if a.strip()
            )
                ...
                if settings.FETCH_ECB_RATES:
                    ...
                    if convert:
                        invoice.foreign_currency_rate = (
                            Decimal(rates_dict[invoice.foreign_currency_display]) / Decimal(rates_dict[invoice.event.currency])
        ...
        for i, p in enumerate(positions):
            ...
            for answ in p.answers.all():
                if not answ.question.print_on_invoice:
                    continue
                desc += '<br />{}{} {}'.format(answ.question.question, '' if str(answ.question.question).endswith('?') else ':', str(answ))
...
def invoice_qualified(order: Order):
    if order.total == Decimal('0.00') or order.require_approval or order.sales_channel not in order.event.settings.get('invoice_generate_sales_channels'):
        return False
    return True
...
def build_preview_invoice_pdf(event):
    ...
    with rolledback_transaction(), language(locale, event.settings.region):
        order = event.orders.create(
            status=Order.STATUS_PENDING, datetime=timezone.now(), expires=timezone.now(), code='PREVIEW', total=100 * event.tax_rules.count()
        )
        invoice = Invoice(order=order, event=event, invoice_no='PREVIEW', date=timezone.now().date(), locale=locale, organizer=event.organizer)
        ...
        invoice.invoice_to = '{}\n{}\n{} {}'.format(invoice.invoice_to_name, invoice.invoice_to_street, invoice.invoice_to_zipcode, invoice.invoice_to_city)
        ...
        if event.tax_rules.exists():
            for i, tr in enumerate(event.tax_rules.all()):
                ...
                InvoiceLine.objects.create(
                    invoice=invoice, description=_('Sample product {}').format(i + 1), gross_value=tax.gross, tax_value=tax.tax, tax_rate=tax.rate
                )
        else:
            InvoiceLine.objects.create(invoice=invoice, description=_('Sample product A'), gross_value=100, tax_value=0, tax_rate=0)
