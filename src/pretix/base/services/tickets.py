        ...
        for receiver, response in responses:
            ...
            if prov.identifier == provider:
                ...
                for ct in CachedTicket.objects.filter(order_position=order_position, provider=provider):
                    ct.delete()
                ct = CachedTicket.objects.create(order_position=order_position, provider=provider, extension=ext, type=ttype, file=None)
        ...
        for receiver, response in responses:
            ...
            if prov.identifier == provider:
                ...
                for ct in CachedCombinedTicket.objects.filter(order=order, provider=provider):
                    ct.delete()
                ct = CachedCombinedTicket.objects.create(order=order, provider=provider, extension=ext, type=ttype, file=None)
                ct.file.save(filename, ContentFile(data))
                return ct.pk
...
def preview(event: int, provider: str):
    ...
    with rolledback_transaction(), language(event.settings.locale, event.settings.region):
        item = event.items.create(name=_('Sample product'), default_price=42.23, description=_('Sample product description'))
        item2 = event.items.create(name=_('Sample workshop'), default_price=23.40)

        from pretix.base.models import Order

        order = event.orders.create(
            status=Order.STATUS_PENDING, datetime=now(), email='sample@eventyay.com', locale=event.settings.locale, expires=now(), code='PREVIEW1234', total=119
        )
    ...
    for p in providers:
        ...
        if p.multi_download_enabled and not base_position:
            ...
            for pos in positions:
                try:
                    ct = CachedTicket.objects.filter(order_position=pos, provider=p.identifier, file__isnull=False).last()
                    if not ct or not ct.file:
                    ...
                    if pos.subevent:
                        ...
                        fname = '{}-{}-{}-{}-{}{}'.format(
                            order.event.slug.upper(), order.code, pos.positionid, pos.subevent.date_from.strftime('%Y_%m_%d'), ct.provider, ct.extension
                        )
                    else:
                        fname = '{}-{}-{}-{}{}'.format(order.event.slug.upper(), order.code, pos.positionid, ct.provider, ct.extension)
