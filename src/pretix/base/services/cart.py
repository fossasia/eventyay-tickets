...
error_messages = {
    ...
    'in_part': _('Some of the products you selected are no longer available in the quantity you selected. Please see below for details.'),
    ...
    'min_items_per_product_removed': _('We removed %(product)s from your cart as you can not buy less than %(min)s items of it.'),
    ...
    'some_subevent_not_started': _('The presale period for this event has not yet started. The affected positions have been removed from your cart.'),
    ...
    'voucher_double': _('You already used this voucher code. Remove the associated line from your cart if you want to use it for a different product.'),
    ...
    'voucher_item_not_available': _('Your voucher is valid for a product that is currently not for sale.'),
    ...
    'addon_min_count': _('You need to select at least %(min)s add-ons from the category %(cat)s for the product %(base)s.'),
...
class CartManager:
    AddOperation = namedtuple(
        'AddOperation',
        ('count', 'item', 'variation', 'price', 'voucher', 'quotas', 'addon_to', 'subevent', 'includes_tax', 'bundled', 'seat', 'price_before_voucher'),
    )
    RemoveOperation = namedtuple('RemoveOperation', ('position',))
    VoucherOperation = namedtuple('VoucherOperation', ('position', 'voucher', 'price'))
    ExtendOperation = namedtuple(
        'ExtendOperation', ('position', 'count', 'item', 'variation', 'price', 'voucher', 'quotas', 'subevent', 'seat', 'price_before_voucher')
    )
    order = {RemoveOperation: 10, VoucherOperation: 15, ExtendOperation: 20, AddOperation: 30}

    def __init__(self, event: Event, cart_id: str, invoice_address: InvoiceAddress = None, widget_data=None, sales_channel='web'):
        self.event = event
    ...
    @property
    def positions(self):
        return CartPosition.objects.filter(Q(cart_id=self.cart_id) & Q(event=self.event)).select_related('item', 'subevent')
    ...
    def _check_presale_dates(self):
        ...
        if not self.event.has_subevents:
            ...
            if tlv:
                term_last = make_aware(datetime.combine(tlv.datetime(self.event).date(), time(hour=23, minute=59, second=59)), self.event.timezone)
    ...
    def _delete_out_of_timeframe(self):
        ...
        for cp in self.positions:
            ...
            if cp.subevent:
                tlv = self.event.settings.get('payment_term_last', as_type=RelativeDateWrapper)
                if tlv:
                    term_last = make_aware(datetime.combine(tlv.datetime(cp.subevent).date(), time(hour=23, minute=59, second=59)), self.event.timezone)
                    if term_last < self.now_dt:
                        err = error_messages['some_subevent_ended']
                        cp.addons.all().delete()
                        cp.delete()
    ...
    def _update_subevents_cache(self, se_ids: List[int]):
        self._subevents_cache.update({i.pk: i for i in self.event.subevents.filter(id__in=[i for i in se_ids if i and i not in self._subevents_cache])})

    def _update_items_cache(self, item_ids: List[int], variation_ids: List[int]):
        self._items_cache.update(
            {
                i.pk: i
                for i in self.event.items.select_related('category')
                .prefetch_related('addons', 'bundles', 'addons__addon_category', 'quotas')
                .annotate(
                    has_variations=Count('variations'),
                )
                .filter(id__in=[i for i in item_ids if i and i not in self._items_cache])
                .order_by()
            }
        )
        self._variations_cache.update(
            {
                v.pk: v
                for v in ItemVariation.objects.filter(item__event=self.event)
    ...
    def _check_max_cart_size(self):
        if not get_all_sales_channels()[self._sales_channel].unlimited_items_per_order:
            ...
            cartsize -= len([1 for op in self._operations if isinstance(op, self.RemoveOperation) if not op.position.addon_to_id])
    ...
    def _check_item_constraints(self, op, current_ops=[]):
        if isinstance(op, (self.AddOperation, self.ExtendOperation)):
            if not ((isinstance(op, self.AddOperation) and op.addon_to == 'FAKE') or (isinstance(op, self.ExtendOperation) and op.position.is_bundled)):
            ...
            if op.subevent:
                tlv = self.event.settings.get('payment_term_last', as_type=RelativeDateWrapper)
                if tlv:
                    term_last = make_aware(datetime.combine(tlv.datetime(op.subevent).date(), time(hour=23, minute=59, second=59)), self.event.timezone)
                    if term_last < self.now_dt:
                        raise CartError(error_messages['payment_ended'])
    ...
    def extend_expired_positions(self):
        requires_seat = Exists(
            SeatCategoryMapping.objects.filter(
                Q(product=OuterRef('item')) & (Q(subevent=OuterRef('subevent')) if self.event.has_subevents else Q(subevent__isnull=True))
            )
        )
        if not self.event.settings.seating_choice:
        ...
        expired = (
            ...
            .select_related('item', 'variation', 'voucher', 'addon_to', 'addon_to__item')
            .annotate(requires_seat=requires_seat)
            .prefetch_related('item__quotas', 'variation__quotas', 'addons')
            .order_by('-is_bundled')
        )
        err = None
        changed_prices = {}
        for cp in expired:
            ...
            if cp.is_bundled:
                ...
                if not cp.includes_tax:
                    price = self._get_price(cp.item, cp.variation, cp.voucher, price, cp.subevent, force_custom_price=True, cp_is_net=False)
                    price = TaxedPrice(net=price.net, gross=price.net, rate=0, tax=0, name='')
                else:
                    price = self._get_price(cp.item, cp.variation, cp.voucher, price, cp.subevent, force_custom_price=True)
                pbv = TAXED_ZERO
            else:
                bundled_sum = Decimal('0.00')
                ...
                if not cp.includes_tax:
                    price = self._get_price(cp.item, cp.variation, cp.voucher, cp.price, cp.subevent, cp_is_net=True, bundled_sum=bundled_sum)
                    price = TaxedPrice(net=price.net, gross=price.net, rate=0, tax=0, name='')
                    pbv = self._get_price(cp.item, cp.variation, None, cp.price, cp.subevent, cp_is_net=True, bundled_sum=bundled_sum)
                    pbv = TaxedPrice(net=pbv.net, gross=pbv.net, rate=0, tax=0, name='')
                else:
                    price = self._get_price(cp.item, cp.variation, cp.voucher, cp.price, cp.subevent, bundled_sum=bundled_sum)
                    pbv = self._get_price(cp.item, cp.variation, None, cp.price, cp.subevent, bundled_sum=bundled_sum)
    ...
    def add_new_items(self, items: List[dict]):
        ...
        for i in items:
            ...
            quotas = list(item.quotas.filter(subevent=subevent) if variation is None else variation.quotas.filter(subevent=subevent))
            if not quotas:
                raise CartError(error_messages['unavailable'])
            ...
            for bundle in db_bundles:
                ...
                bundle_quotas = list(bitem.quotas.filter(subevent=subevent) if bvar is None else bvar.quotas.filter(subevent=subevent))
                ...
                if bundle.designated_price:
                    bprice = self._get_price(bitem, bvar, None, bundle.designated_price, subevent, force_custom_price=True, cp_is_net=False)
                else:
                    bprice = TAXED_ZERO
    ...
    def set_addons(self, addons):
        ...
        toplevel_cp = (
            ...
            .prefetch_related('addons', 'item__addons', 'item__addons__addon_category')
        ...
        for a in addons:
            ...
            # Fetch all quotas. If there are no quotas, this item is not allowed to be sold.
            quotas = list(item.quotas.filter(subevent=cp.subevent) if variation is None else variation.quotas.filter(subevent=cp.subevent))
            if not quotas:
                raise CartError(error_messages['unavailable'])
        ...
        for cp in toplevel_cp:
            ...
            for iao in item.addons.all():
                ...
                validate_cart_addons.send(
                    sender=self.event,
                    addons={(self._items_cache[s[0]], self._variations_cache[s[1]] if s[1] else None): c for s, c in selected.items() if c > 0},
                    base_position=cp,
                    iao=iao,
                )
    ...
    def _get_voucher_availability(self):
        ...
        for voucher, count in self._voucher_use_diff.items():
            ...
            redeemed_in_carts = CartPosition.objects.filter(Q(voucher=voucher) & Q(event=self.event) & Q(expires__gte=self.now_dt)).exclude(
                pk__in=[op.position.id for op in self._operations if isinstance(op, self.ExtendOperation)]
            )
    ...
    def _check_min_max_per_product(self):
        ...
        for item, count in items.items():
            ...
            if item.max_per_order and count > item.max_per_order:
                raise CartError(_(error_messages['max_items_per_product']) % {'max': item.max_per_order, 'product': item.name})

            if item.min_per_order and count < item.min_per_order:
                self._operations = [o for o in self._operations if not (isinstance(o, self.AddOperation) and o.item.pk == item.pk)]
                ...
                for p in self.positions:
                    if p.item_id == item.pk and p.pk not in removals:
                        ...
                        err = _(error_messages['min_items_per_product_removed']) % {'min': item.min_per_order, 'product': item.name}
                if not err:
                    raise CartError(_(error_messages['min_items_per_product']) % {'min': item.min_per_order, 'product': item.name})
        return err
            ...
            if isinstance(op, self.RemoveOperation):
                ...
                if isinstance(op, self.AddOperation):
                    if op.seat and not op.seat.is_available(
                        ignore_voucher_id=op.voucher.id if op.voucher else None, sales_channel=self._sales_channel, distance_ignore_cart_id=self.cart_id
                    ):
                        available_count = 0
                    ...
                    for k in range(available_count):
                        ...
                        if self.event.settings.attendee_names_asked:
                            ...
                            if any('attendee-name-{}'.format(k.replace('_', '-')) in self._widget_data for k, l, w in scheme['fields']):
                                cp.attendee_name_parts = {
                                    k: self._widget_data.get('attendee-name-{}'.format(k.replace('_', '-')), '') for k, l, w in scheme['fields']
                                }
                    ...
                    if op.seat and not op.seat.is_available(
                        ignore_cart=op.position, sales_channel=self._sales_channel, ignore_voucher_id=op.position.voucher_id
                    ):
                        err = err or error_messages['seat_unavailable']
                        op.position.addons.all().delete()
                        op.position.delete()
...
def update_tax_rates(event: Event, cart_id: str, invoice_address: InvoiceAddress):
    positions = CartPosition.objects.filter(cart_id=cart_id, event=event).select_related('item', 'item__tax_rule')
...
def get_fees(event, request, total, invoice_address, provider, positions):
    ...
    fees = []
    for recv, resp in fee_calculation_for_cart.send(sender=event, request=request, invoice_address=invoice_address, total=total, positions=positions):
        if resp:
            fees += resp
...
def add_items_to_cart(
    self, event: int, items: List[dict], cart_id: str = None, locale='en', invoice_address: int = None, widget_data=None, sales_channel='web'
) -> None:
    """
    ...
    with language(locale):
        ...
        try:
            try:
                cm = CartManager(event=event, cart_id=cart_id, invoice_address=ia, widget_data=widget_data, sales_channel=sales_channel)
...
@app.task(base=ProfiledEventTask, bind=True, max_retries=5, default_retry_delay=1, throws=(CartError,))
def set_cart_addons(self, event: Event, addons: List[dict], cart_id: str = None, locale='en', invoice_address: int = None, sales_channel='web') -> None:
    """
    Removes a list of items from a user's cart.
    :param event: The event ID in question
