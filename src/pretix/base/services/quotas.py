...
class QuotaAvailability:
    ...
    * count_cart (dict mapping quotas to ints)
    ...
    def compute(self, now_dt=None, allow_cache=False, allow_cache_stale=False):
        ...
        for q in quotas_original:
            for recv, resp in quota_availability.send(sender=q.event, quota=q, result=self.results[q], count_waitinglist=self.count_waitinglist):
                self.results[q] = resp
    ...
    def _write_cache(self, quotas, now_dt):
        ...
        for eventid, quotas in update.items():
            rc.hmset(f'quotas:{eventid}:availabilitycache', {str(q.id): ','.join([str(i) for i in self.results[q]] + [str(int(time.time()))]) for q in quotas})
    ...
    def _compute(self, quotas, now_dt):
        ...
        q_items = Quota.items.through.objects.filter(quota_id__in=[q.pk for q in quotas]).values('quota_id', 'item_id')
        for m in q_items:
            self._item_to_quotas[m['item_id']].append(self._quota_objects[m['quota_id']])

        q_vars = Quota.variations.through.objects.filter(quota_id__in=[q.pk for q in quotas]).values('quota_id', 'itemvariation_id')
        for m in q_vars:
    ...
    def _compute_orders(self, quotas, q_items, q_vars, size_left):
        ...
        op_lookup = (
            ...
            .filter(seq)
            .filter(
                Q(Q(variation_id__isnull=True) & Q(item_id__in={i['item_id'] for i in q_items if self._quota_objects[i['quota_id']] in quotas}))
                | Q(variation_id__in={i['itemvariation_id'] for i in q_vars if self._quota_objects[i['quota_id']] in quotas})
            )
            .order_by()
        )
    ...
    def _compute_vouchers(self, quotas, q_items, q_vars, size_left, now_dt):
        ...
        v_lookup = (
            ...
            .order_by()
            .values('subevent_id', 'item_id', 'quota_id', 'variation_id')
    ...
    def _compute_carts(self, quotas, q_items, q_vars, size_left, now_dt):
        ...
        cart_lookup = (
            ...
            .order_by()
            .values('item_id', 'subevent_id', 'variation_id')
            .annotate(c=Count('*'))
        )
