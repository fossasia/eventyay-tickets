...
def validate_plan_change(event, subevent, plan):
    ...
    if leftovers:
        raise SeatProtected(_('You can not change the plan since seat "%s" is not present in the new plan and is already sold.'), leftovers[0])


def generate_seats(event, subevent, plan, mapping, blocked_guids=None):
    current_seats = {}
    for s in event.seats.select_related('product').annotate(has_op=Count('orderposition'), has_v=Count('vouchers')).filter(subevent=subevent).order_by():
    ...
    for s in current_seats.values():
        if s.has_op:
            raise SeatProtected(_('You can not change the plan since seat "%s" is not present in the new plan and is already sold.', s.name))
        if s.has_v:
            raise SeatProtected(_('You can not change the plan since seat "%s" is not present in the new plan and is already used in a voucher.', s.name))
