...
class BaseReportlabInvoiceRenderer(BaseInvoiceRenderer):
    ...
    def _register_fonts(self):
        ...
        pdfmetrics.registerFontFamily('OpenSans', normal='OpenSans', bold='OpenSansBd', italic='OpenSansIt', boldItalic='OpenSansBI')
    ...
    def _get_first_page_frames(self, doc):
        ...
        """
        return [
            Frame(doc.leftMargin, doc.bottomMargin, doc.width, doc.height - 75 * mm, leftPadding=0, rightPadding=0, topPadding=0, bottomPadding=0, id='normal')
        ]
    ...
    def _get_other_page_frames(self, doc):
        ...
        """
        return [Frame(doc.leftMargin, doc.bottomMargin, doc.width, doc.height, leftPadding=0, rightPadding=0, topPadding=0, bottomPadding=0, id='normal')]
    ...
    def _build_doc(self, fhandle):
        ...
        doc.addPageTemplates(
            [
                PageTemplate(id='FirstPage', frames=self._get_first_page_frames(doc), onPage=self._on_first_page, pagesize=self.pagesize),
                PageTemplate(id='OtherPages', frames=self._get_other_page_frames(doc), onPage=self._on_other_page, pagesize=self.pagesize),
            ]
        )
...
class ClassicInvoiceRenderer(BaseReportlabInvoiceRenderer):
    ...
    def _draw_invoice_from(self, canvas):
        p = Paragraph(bleach.clean(self.invoice.full_invoice_from, tags=[]).strip().replace('\n', '<br />\n'), style=self.stylesheet['InvoiceFrom'])
            ...
            if self.invoice.event.settings.show_date_to and self.invoice.event.date_to:
                p_str = (
                    ...
                    + pgettext('invoice', '{from_date}\nuntil {to_date}').format(
                        from_date=self.invoice.event.get_date_from_display(show_times=False), to_date=self.invoice.event.get_date_to_display(show_times=False)
                    )
                )
            else:
    ...
    def _get_other_page_frames(self, doc):
        ...
        return [
            Frame(
                doc.leftMargin, doc.bottomMargin, doc.width, doc.height, leftPadding=0, rightPadding=0, topPadding=0, bottomPadding=footer_length, id='normal'
            )
        ]
    ...
    def _get_intro(self):
        ...
        if self.invoice.invoice_to_beneficiary:
            story.append(
                Paragraph(
                    pgettext('invoice', 'Beneficiary') + ':<br />' + bleach.clean(self.invoice.invoice_to_beneficiary, tags=[]).replace('\n', '<br />\n'),
                    self.stylesheet['Normal'],
                )
            )
    ...
    def _get_story(self, doc):
        ...
        story = [
            ...
            Paragraph(
                (pgettext('invoice', 'Tax Invoice') if str(self.invoice.invoice_from_country) == 'AU' else pgettext('invoice', 'Invoice'))
        ...
        if has_taxes:
            tdata.append([pgettext('invoice', 'Invoice total'), '', '', '', money_filter(total, self.invoice.event.currency)])
            colwidths = [a * doc.width for a in (0.50, 0.05, 0.15, 0.15, 0.15)]
        else:
            tdata.append([pgettext('invoice', 'Invoice total'), '', money_filter(total, self.invoice.event.currency)])
            colwidths = [a * doc.width for a in (0.65, 0.05, 0.30)]

        if self.invoice.event.settings.invoice_show_payments and not self.invoice.is_cancellation and self.invoice.order.status == Order.STATUS_PENDING:
        ...
        thead = [pgettext('invoice', 'Tax rate'), pgettext('invoice', 'Net value'), pgettext('invoice', 'Gross value'), pgettext('invoice', 'Tax'), '']
        tdata = [thead]
        ...
        if len(tdata) > 1 and has_taxes:
            ...
            story.append(KeepTogether([Paragraph(pgettext('invoice', 'Included taxes'), self.stylesheet['FineprintHeading']), table]))
            ...
            if self.invoice.foreign_currency_display and self.invoice.foreign_currency_rate:
                ...
                for idx, gross in grossvalue_map.items():
                    ...
                    tdata.append([localize(rate) + ' % ' + name, fmt(net), fmt(gross), fmt(tax), ''])
                ...
                story.append(
                    KeepTogether(
                        [
                            ...
                            Paragraph(
                                pgettext(
                                    'invoice', 'Using the conversion rate of 1:{rate} as published by the European Central Bank on {date}, this corresponds to:'
                                ).format(
            ...
            story.append(
                Paragraph(
                    pgettext(
                        'invoice',
                        'Using the conversion rate of 1:{rate} as published by the European Central Bank on {date}, the invoice total corresponds to {total}.',
                    ).format(
                        rate=localize(self.invoice.foreign_currency_rate),
                        date=date_format(self.invoice.foreign_currency_rate_date, 'SHORT_DATE_FORMAT'),
...
class Modern1Renderer(ClassicInvoiceRenderer):
    ...
    def _draw_invoice_from(self, canvas):
        if not self.invoice.invoice_from:
            return
        c = [bleach.clean(l, tags=[]).strip().replace('\n', '<br />\n') for l in self.invoice.address_invoice_from.strip().split('\n')]
