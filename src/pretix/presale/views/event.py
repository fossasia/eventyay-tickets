...
def get_grouped_items(
    ...
    items = (
        ...
        .prefetch_related(
            Prefetch('quotas', to_attr='_subevent_quotas', queryset=event.quotas.using(settings.DATABASE_REPLICA).filter(subevent=subevent)),
            Prefetch(
                'bundles',
    ...
    for item in items:
        ...
        if not item.has_variations:
            ...
            if voucher and (voucher.allow_ignore_quota or voucher.block_quota):
                item.cached_availability = (Quota.AVAILABILITY_OK, voucher.max_usages - voucher.redeemed)
            else:
                item.cached_availability = list(item.check_quotas(subevent=subevent, _cache=quota_cache, include_bundled=True))

            if event.settings.hide_sold_out and item.cached_availability[0] < Quota.AVAILABILITY_RESERVED:
                item._remove = True
                continue

            item.order_max = min(item.cached_availability[1] if item.cached_availability[1] is not None else sys.maxsize, max_per_order)
            ...
            for var in item.available_variations:
                ...
                if voucher and (voucher.allow_ignore_quota or voucher.block_quota):
                    var.cached_availability = (Quota.AVAILABILITY_OK, voucher.max_usages - voucher.redeemed)
                else:
                    var.cached_availability = list(var.check_quotas(subevent=subevent, _cache=quota_cache, include_bundled=True))

                var.order_max = min(var.cached_availability[1] if var.cached_availability[1] is not None else sys.maxsize, max_per_order)
            ...
            item.original_price = (
                item.tax(
                    item.original_price, currency=event.currency, include_bundled=True, base_price_is='net' if event.settings.display_net_prices else 'gross'
                )  # backwards-compat
                if item.original_price
                else None
            )
            ...
            item.available_variations = [
                v for v in item.available_variations if v._subevent_quotas and (not voucher or not voucher.quota_id or v in restrict_vars)
            ]

            if event.settings.hide_sold_out:
                item.available_variations = [v for v in item.available_variations if v.cached_availability[0] >= Quota.AVAILABILITY_RESERVED]

            if voucher and voucher.variation_id:
                item.available_variations = [v for v in item.available_variations if v.pk == voucher.variation_id]

            if len(item.available_variations) > 0:
                item.min_price = min([v.display_price.net if event.settings.display_net_prices else v.display_price.gross for v in item.available_variations])
                item.max_price = max([v.display_price.net if event.settings.display_net_prices else v.display_price.gross for v in item.available_variations])
    ...
    items = [item for item in items if (len(item.available_variations) > 0 or not item.has_variations) and not item._remove]
...
class EventIndex(EventViewMixin, EventListMixin, CartMixin, TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.GET.get('src', '') == 'widget' and 'take_cart_id' in request.GET:
            ...
            get_or_create_cart_id(request)
            return redirect(
                eventreverse(request.event, 'presale:event.index', kwargs=kwargs) + '?require_cookie=true&cart_id={}'.format(request.GET.get('take_cart_id'))
            )
            ...
            r = render(
                ...
                {
                    'url': eventreverse(request.event, 'presale:event.index', kwargs={'cart_namespace': kwargs.get('cart_namespace') or ''})
                    + ('?src=widget&take_cart_id={}'.format(request.GET.get('cart_id')) if 'cart_id' in request.GET else '')
                },
            )
            r._csp_ignore = True
            return r
    ...
    def get_context_data(self, **kwargs):
        ...
        if not self.request.event.has_subevents or self.subevent:
            ...
            context['allfree'] = all(item.display_price.gross == Decimal('0.00') for item in items if not item.has_variations) and all(
                all(var.display_price.gross == Decimal('0.00') for var in item.available_variations) for item in items if item.has_variations
            )
        ...
        context['show_cart'] = context['cart']['positions'] and (self.request.event.has_subevents or self.request.event.presale_is_running)
        if self.request.event.settings.redirect_to_checkout_directly:
            context['cart_redirect'] = eventreverse(
                self.request.event, 'presale:event.checkout.start', kwargs={'cart_namespace': kwargs.get('cart_namespace') or ''}
            )
        ...
        if getattr(getattr(getattr(pretix_venueless, 'apps', None), 'PluginApp', None), 'name', None) in self.request.event.get_plugins():
    ...
    def _subevent_list_context(self):
        ...
        if context['list_type'] == 'calendar':
            ...
            context['show_names'] = ebd.get('_subevents_different_names', False) or sum(len(i) for i in ebd.values() if isinstance(i, list)) < 2
            ...
            before = datetime(week.monday().year, week.monday().month, week.monday().day, 0, 0, 0, tzinfo=tz) - timedelta(days=1)
            after = datetime(week.sunday().year, week.sunday().month, week.sunday().day, 0, 0, 0, tzinfo=tz) + timedelta(days=1)
            ...
            if self.request.event.settings.event_list_available_only and not voucher:
                context['subevent_list'] = [
                    se for se in context['subevent_list'] if not se.presale_has_ended and se.best_availability_state >= Quota.AVAILABILITY_RESERVED
                ]
        return context
...
class JoinOnlineVideoView(EventViewMixin, View):
    def get(self, request, *args, **kwargs):
        ...
        return JsonResponse({'redirect_url': self.generate_token_url(request, order_position, order)}, status=200)
    ...
    def validate_access(self, request, *args, **kwargs):
        if not hasattr(self.request, 'user'):
            ...
            order_list = (
                Order.objects.filter(Q(event=self.request.event) & (Q(email__iexact=self.request.user.email))).select_related('event').order_by('-datetime')
            )
            # Check qs is empty
            if not order_list:
                # no order placed yet
    ...
    def generate_token_url(self, request, order_position, order):
        ...
        payload = {
            ...
            'traits': list(
                ...
                | {'eventyay-video-variation-{}'.format(p.variation_id) for p in order_position.addons.all() if p.variation_id}
                | {'eventyay-video-category-{}'.format(p.item.category_id) for p in order_position.addons.all() if p.item.category_id}
            ),
        }
