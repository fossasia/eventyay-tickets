...
class CartMixin:
    ...
    def get_cart(self, answers=False, queryset=None, order=None, downloads=False):
        if queryset is not None:
            ...
            cartpos = (
                queryset.order_by('item__category__position', 'item__category_id', 'item__position', 'item__name', 'variation__value')
                .select_related('item', 'variation', 'addon_to', 'subevent', 'subevent__event', 'subevent__event__organizer', 'seat')
            ...
            for r, response in sorted(responses, key=lambda r: str(r[0])):
                if response:
                    for key, value in response.items():
                        pos_additional_fields[cp.pk].append({'answer': data.get('question_form_data', {}).get(key), 'question': value.label})
        ...
        def keyfunc(pos):
            ...
            has_attendee_data = pos.item.admission and (
                self.request.event.settings.attendee_names_asked or self.request.event.settings.attendee_emails_asked or pos_additional_fields.get(pos.pk)
            )
            ...
            return (
                ...
                + item_key
                + variation_key
                + (pos.price, (pos.voucher_id or 0), (pos.subevent_id or 0), (pos.seat_id or 0))
            )
        ...
        if order:
            ...
            fees = get_fees(self.request.event, self.request, total, self.invoice_address, self.cart_session.get('payment'), cartpos)
        else:
            fees = []
    ...
    if not hasattr(request, '_cart_cache'):
        ...
        if not cart_id:
            ...
            request._cart_cache = (
                ...
                .select_related('item', 'variation', 'subevent', 'subevent__event', 'subevent__event__organizer', 'item__tax_rule', 'addon_to')
                .select_related('addon_to')
                .prefetch_related(
                    'addons',
                    'addons__item',
                    'addons__variation',
                    Prefetch('answers', QuestionAnswer.objects.prefetch_related('options'), to_attr='answerlist'),
                    Prefetch(
                        'item__questions',
...
def get_cart_total(request):
    ...
    if not hasattr(request, '_cart_total_cache'):
        if hasattr(request, '_cart_cache'):
            ...
            request._cart_total_cache = CartPosition.objects.filter(cart_id=get_or_create_cart_id(request), event=request.event).aggregate(sum=Sum('price'))[
                'sum'
            ] or Decimal('0.00')
