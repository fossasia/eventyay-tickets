...
class OrderDetailMixin(NoSearchIndexViewMixin):
    ...
    def get_order_url(self):
        return eventreverse(self.request.event, 'presale:event.order', kwargs={'order': self.order.code, 'secret': self.order.secret})
    ...
    def position(self):
        p = (
            OrderPosition.objects.filter(
                order__event=self.request.event, addon_to__isnull=True, order__code=self.kwargs['order'], positionid=self.kwargs['position']
            )
...
class TicketPageMixin:
    def get_context_data(self, **kwargs):
        ...
        ctx['download_email_required'] = can_download and (
            self.request.event.settings.ticket_download_require_validated_email and self.order.sales_channel == 'web' and not self.order.email_known_to_work
        )
...
class OrderDetails(EventViewMixin, OrderDetailMixin, CartMixin, TicketPageMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx['can_download_multi'] = any([b['multi'] for b in self.download_buttons]) and ([p.generate_ticket for p in ctx['cart']['positions']].count(True) > 1)
        ...
        if ctx['can_generate_invoice']:
            if (
                not self.order.payments.exclude(state__in=[OrderPayment.PAYMENT_STATE_CANCELED, OrderPayment.PAYMENT_STATE_FAILED]).exists()
                and self.order.status == Order.STATUS_PENDING
            ):
                ctx['generate_invoice_requires'] = 'payment'
        ctx['url'] = build_absolute_uri(self.request.event, 'presale:event.order', kwargs={'order': self.order.code, 'secret': self.order.secret})
        ...
        if self.order.status == Order.STATUS_PENDING:
            ...
            if lp and lp.state not in (OrderPayment.PAYMENT_STATE_CONFIRMED, OrderPayment.PAYMENT_STATE_REFUNDED, OrderPayment.PAYMENT_STATE_CANCELED):
                ctx['last_payment'] = lp
...
class OrderPositionDetails(EventViewMixin, OrderPositionDetailMixin, CartMixin, TicketPageMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx['cart'] = self.get_cart(
            answers=True,
            downloads=ctx['can_download'],
            queryset=self.order.positions.select_related('tax_rule').filter(Q(pk=self.position.pk) | Q(addon_to__id=self.position.pk)),
            order=self.order,
        )
        ctx['tickets_with_download'] = [p for p in ctx['cart']['positions'] if p.generate_ticket]
        return ctx
...
class OrderPaymentStart(EventViewMixin, OrderDetailMixin, TemplateView):
    ...
    def get_confirm_url(self):
        return eventreverse(
            self.request.event, 'presale:event.order.pay.confirm', kwargs={'order': self.order.code, 'secret': self.order.secret, 'payment': self.payment.pk}
        )
...
class OrderPaymentConfirm(EventViewMixin, OrderDetailMixin, TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if self.payment.state != OrderPayment.PAYMENT_STATE_CREATED or not self.order._can_be_paid():
            messages.error(request, _('The payment for this order cannot be continued.'))
            return redirect(self.get_order_url())
        if not self.payment.payment_provider.payment_is_valid_session(request) or not self.payment.payment_provider.is_enabled:
    ...
    def get_payment_url(self):
        return eventreverse(
            self.request.event, 'presale:event.order.pay', kwargs={'order': self.order.code, 'secret': self.order.secret, 'payment': self.payment.pk}
        )
...
class OrderPaymentComplete(EventViewMixin, OrderDetailMixin, View):
    ...
    def get_payment_url(self):
        return eventreverse(
            self.request.event, 'presale:event.order.pay', kwargs={'order': self.order.code, 'payment': self.payment.pk, 'secret': self.order.secret}
        )
...
class OrderPayChangeMethod(EventViewMixin, OrderDetailMixin, TemplateView):
    ...
    def get_payment_url(self, payment):
        return eventreverse(
            self.request.event, 'presale:event.order.pay', kwargs={'order': self.order.code, 'secret': self.order.secret, 'payment': payment.pk}
        )
    ...
    def mark_paid_free(self):
        p = self.order.payments.create(state=OrderPayment.PAYMENT_STATE_CREATED, provider='manual', amount=Decimal('0.00'), fee=None)
        try:
            p.confirm()
    ...
    def get_confirm_url(self, payment):
        return eventreverse(
            self.request.event, 'presale:event.order.pay.confirm', kwargs={'order': self.order.code, 'secret': self.order.secret, 'payment': payment.pk}
        )
...
def can_generate_invoice(event, order, ignore_payments=False):
    ...
    if not ignore_payments:
        v = v and not (
            not order.payments.exclude(state__in=[OrderPayment.PAYMENT_STATE_CANCELED, OrderPayment.PAYMENT_STATE_FAILED]).exists()
...
class OrderModify(EventViewMixin, OrderDetailMixin, OrderQuestionsViewMixin, TemplateView):
    ...
    def get_question_override_sets(self, order_position):
        override_sets = [resp for recv, resp in question_form_fields_overrides.send(self.request.event, position=order_position, request=self.request)]
        ...
        if 'country' in self.invoice_form.cleaned_data:
            ...
            for tr in trs:
                if tr.get_matching_rule(self.invoice_form.instance).get('action', 'vat') == 'block':
                    messages.error(self.request, _('One of the selected products is not available in the selected country.'))
                    return self.get(request, *args, **kwargs)
...
class OrderCancelDo(EventViewMixin, OrderDetailMixin, AsyncAction, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        giftcard = self.request.event.settings.cancel_allow_user_paid_refund_as_giftcard == 'force' or (
            self.request.event.settings.cancel_allow_user_paid_refund_as_giftcard == 'option' and self.request.POST.get('giftcard') == 'true'
        )
        if self.request.event.settings.cancel_allow_user_paid_require_approval:
            ...
            return self.do(self.order.pk, cancellation_fee=fee, try_auto_refund=True, refund_as_giftcard=giftcard, comment=comment)
...
class OrderDownloadMixin:
    ...
    def post(self, request, *args, **kwargs):
        ...
        if self.request.event.settings.ticket_download_require_validated_email and self.order.sales_channel == 'web' and not self.order.email_known_to_work:
...
class OrderDownload(OrderDownloadMixin, EventViewMixin, OrderDetailMixin, AsyncAction, View):
    ...
    def get_self_url(self):
        return eventreverse(
            self.request.event, 'presale:event.order.download' if 'position' in self.kwargs else 'presale:event.order.download.combined', kwargs=self.kwargs
        )
...
class OrderPositionDownload(OrderDownloadMixin, EventViewMixin, OrderPositionDetailMixin, AsyncAction, View):
    ...
    def get_self_url(self):
        return eventreverse(self.request.event, 'presale:event.order.position.download', kwargs=self.kwargs)

    @cached_property
    def order_position(self):
        try:
            return self.order.positions.get(Q(pk=self.kwargs.get('pid')) & Q(Q(pk=self.position.pk) | Q(addon_to__id=self.position.pk)))
        except OrderPosition.DoesNotExist:
            return None
...
class OrderChange(EventViewMixin, OrderDetailMixin, TemplateView):
    ...
    def positions(self):
        positions = list(
            self.order.positions.select_related('item', 'item__tax_rule').prefetch_related('item__quotas', 'item__variations', 'item__variations__quotas')
        )
        try:
        ...
        for p in self.positions:
            ...
            try:
                ...
                if change_item is not None:
                    ...
                    new_price = get_price(change_item[0], change_item[1], voucher=p.voucher, subevent=p.subevent, invoice_address=ia)
        ...
        if not form_valid:
            ...
            try:
                ...
                if self.order.status != Order.STATUS_PAID and was_paid:
                    messages.success(
                        self.request,
                        _('The order has been changed. You can now proceed by paying the open amount of {amount}.').format(
                            amount=money_filter(self.order.pending_sum, self.request.event.currency)
                        ),
                    )
                    return redirect(
                        eventreverse(self.request.event, 'presale:event.order.pay.change', kwargs={'order': self.order.code, 'secret': self.order.secret})
                    )
                else:
