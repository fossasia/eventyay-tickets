...
class TicketPageMixin:
    ...
    def get_context_data(self, **kwargs):
        ...
        can_download = all(
            [
                r
                for rr, r in allow_ticket_download.send(
                    self.request.event, order=self.order
                )
            ]
        )
...
class OrderPaymentStart(EventViewMixin, OrderDetailMixin, TemplateView):
    ...
    def form(self):
        if (
            ...
            return self.payment.payment_provider.payment_form_render(
                self.request, self.payment.amount
            )
        else:
...
class OrderPayChangeMethod(EventViewMixin, OrderDetailMixin, TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if self.open_payment:
            ...
            if (
                self.open_payment.state == OrderPayment.PAYMENT_STATE_PENDING
                and not pp.abort_pending_allowed
            ):
                messages.error(
    ...
    def post(self, request, *args, **kwargs):
        ...
        if (
            ...
            if (
                ...
                and fee <= custom_fee <= self.order.payment_refund_sum
            ):
                if self.request.event.settings.cancel_allow_user_paid_adjust_fees_step:
                    if (
                        custom_fee - fee
                    ) % self.request.event.settings.cancel_allow_user_paid_adjust_fees_step != Decimal(
                        "0.00"
                    ):
                        messages.error(
        ...
        for p in self.positions:
            ...
            try:
                ...
                if change_item is not None:
                    ...
                    if (
                        change_item[0].tax_rule != p.tax_rule
                        or new_price.rate != p.tax_rate
                    ):
