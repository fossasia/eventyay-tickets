...
class WaitingView(EventViewMixin, FormView):
    ...
    def get(self, request, *args, **kwargs):
        if request.GET.get('iframe', '') == '1' and 'require_cookie' not in request.GET:
            ...
            r = render(
                ...
                {
                    'url': eventreverse(request.event, 'presale:event.waitinglist', kwargs={'cart_namespace': kwargs.get('cart_namespace')})
                    + '?'
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if request.event.has_subevents:
            if 'subevent' in request.GET:
                self.subevent = get_object_or_404(SubEvent, event=request.event, pk=request.GET['subevent'], active=True)
            else:
    ...
    def form_valid(self, form):
        ...
        if availability[0] == 100:
            messages.error(self.request, _('You cannot add yourself to the waiting list as this product is currently available.'))
            return redirect(self.get_index_url())

        form.save()
        messages.success(self.request, _("We've added you to the waiting list. You will receive an email as soon as tickets get available again."))
