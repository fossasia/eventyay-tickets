...
def widget_css(request, **kwargs):
    ...
    if o.settings.presale_widget_css_file:
        try:
            resp = FileResponse(default_storage.open(o.settings.presale_widget_css_file), content_type='text/css')
            return resp
...
class WidgetAPIProductList(EventListMixin, View):
    def _get_items(self):
        ...
        items, display_add_to_cart = get_grouped_items(
            self.request.event, subevent=self.subevent, voucher=self.voucher, channel=self.request.sales_channel.identifier, base_qs=qs
        )
                    ...
                    'items': [
                        {
                            ...
                            'avail': [item.cached_availability[0], item.cached_availability[1] if item.do_show_quota_left else None]
                            if not item.has_variations
                            else None,
                            'original_price': (
                                (item.original_price.net if self.request.event.settings.display_net_prices else item.original_price.gross)
                            ...
                            'variations': [
                                {
                                    ...
                                    'original_price': (
                                        (var.original_price.net if self.request.event.settings.display_net_prices else var.original_price.gross)
                                    ...
                                    'avail': [var.cached_availability[0], var.cached_availability[1] if item.do_show_quota_left else None],
                                }
    ...
    def get(self, request, *args, **kwargs):
        ...
        if request.sales_channel.identifier not in request.event.sales_channels:
            return self.response({'error': gettext('Tickets for this event cannot be purchased on this sales channel.')})
    ...
    def _get_event_list(self, request, **kwargs):
        ...
        if list_type == 'calendar':
            ...
            if hasattr(self.request, 'event'):
                add_subevents_for_days(
                    filter_qs_by_attr(
                        self.request.event.subevents_annotated('web').filter(event__sales_channels__contains=self.request.sales_channel.identifier),
                        self.request,
                    ),
                    before,
                    after,
                ...
                add_events_for_days(
                    self.request,
                    filter_qs_by_attr(
                        Event.annotated(self.request.organizer.events, 'web').filter(sales_channels__contains=self.request.sales_channel.identifier),
                        self.request,
                    ),
                    before,
                    after,
            ...
            if hasattr(self.request, 'event'):
                ...
                timezones = set()
                add_events_for_days(
                    self.request, filter_qs_by_attr(Event.annotated(self.request.organizer.events, 'web'), self.request), before, after, ebd, timezones
                )
                add_subevents_for_days(
                    filter_qs_by_attr(
                        SubEvent.annotated(
            ...
            if hasattr(self.request, 'event'):
                ...
                if self.request.event.settings.event_list_available_only:
                    evs = [se for se in evs if not se.presale_has_ended and se.best_availability_state >= Quota.AVAILABILITY_RESERVED]
        ...
        if 'voucher' in request.GET:
            try:
                ...
                if cart_id:
                    ...
                    redeemed_in_carts = CartPosition.objects.filter(Q(voucher=self.voucher) & Q(event=request.event) & Q(expires__gte=now()))
