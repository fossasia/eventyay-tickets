...
class CartActionMixin:
    def get_next_url(self):
        ...
        if '?' in u:
            ...
            u += '?require_cookie=true'
        disclose_cart_id = ('iframe' in self.request.GET or settings.SESSION_COOKIE_NAME not in self.request.COOKIES) and self.kwargs.get('cart_namespace')
        if disclose_cart_id:
    ...
    def _item_from_post_value(self, key, value, voucher=None):
        ...
        if key.startswith('item_'):
            try:
                return {'item': int(parts[1]), 'variation': None, 'count': amount, 'price': price, 'voucher': voucher, 'subevent': subevent}
            except ValueError:
                raise CartError(_('Please enter numbers only.'))
        elif key.startswith('variation_'):
            try:
                return {'item': int(parts[1]), 'variation': int(parts[2]), 'count': amount, 'price': price, 'voucher': voucher, 'subevent': subevent}
            except ValueError:
                raise CartError(_('Please enter numbers only.'))
...
class CartClear(EventViewMixin, CartActionMixin, AsyncAction, View):
    ...
    def post(self, request, *args, **kwargs):
        return self.do(self.request.event.id, get_or_create_cart_id(self.request), translation.get_language(), request.sales_channel.identifier)
...
class CartAdd(EventViewMixin, CartActionMixin, AsyncAction, View):
    ...
    def _ajax_response_data(self):
        cart_id = get_or_create_cart_id(self.request)
        return {'cart_id': cart_id, 'has_cart': CartPosition.objects.filter(cart_id=cart_id, event=self.request.event).exists()}
    ...
    def post(self, request, *args, **kwargs):
        ...
        if items:
            return self.do(
                self.request.event.id, items, cart_id, translation.get_language(), self.invoice_address.pk, widget_data, self.request.sales_channel.identifier
            )
        else:
            if 'ajax' in self.request.GET or 'ajax' in self.request.POST:
                return JsonResponse({'redirect': self.get_error_url(), 'success': False, 'message': _(error_messages['empty'])})
            else:
                return redirect(self.get_error_url())
...
class RedeemView(NoSearchIndexViewMixin, EventViewMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Fetch all items
        items, display_add_to_cart = get_grouped_items(self.request.event, self.subevent, voucher=self.voucher, channel=self.request.sales_channel.identifier)
        ...
        if self.request.event.settings.redirect_to_checkout_directly:
            ...
            context['cart_redirect'] = eventreverse(self.request.event, 'presale:event.index', kwargs={'cart_namespace': kwargs.get('cart_namespace') or ''})
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if v:
            ...
            try:
                ...
                redeemed_in_carts = CartPosition.objects.filter(
                    Q(voucher=self.voucher) & Q(event=request.event) & (Q(expires__gte=now()) | Q(cart_id=get_or_create_cart_id(request)))
                )
        ...
        if request.event.has_subevents:
            if request.GET.get('subevent'):
                self.subevent = get_object_or_404(SubEvent, event=request.event, pk=request.GET.get('subevent'), active=True)
            ...
            if not err and not self.subevent:
                return redirect(
                    eventreverse(self.request.event, 'presale:event.index', kwargs={'cart_namespace': kwargs.get('cart_namespace') or ''})
                    + '?voucher='
...
class AnswerDownload(EventViewMixin, View):
    def get(self, request, *args, **kwargs):
        ...
        answer = get_object_or_404(QuestionAnswer, cartposition__cart_id=get_or_create_cart_id(self.request), id=answid)
        if not answer.file:
            return Http404()
        ...
        resp['Content-Disposition'] = 'attachment; filename="{}-cart-{}"'.format(
            self.request.event.slug.upper(), os.path.basename(answer.file.name).split('.', 1)[1]
        )
        return resp
