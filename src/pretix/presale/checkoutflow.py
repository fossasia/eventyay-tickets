...
class CustomerStep(QuestionsViewMixin, CartMixin, TemplateFlowStep):
    ...
    def signup_allowed(self):
        return self.request.event.settings.native_customer_accounts
    ...
    def _handle_sso_login(self):
        ...
        except (BadSignature, KeyError, Customer.DoesNotExist):
            return False
        ...
        if request.POST.get("customer_mode") == 'login':
            if self.cart_session.get('customer'):
                ...
                if not self._handle_sso_login():
                    messages.error(request, _('Authentication failed, please try again in sometime.'))
                    return self.render()
                return redirect_to_url(self.get_next_url(request))
            elif self.login_form.is_valid() and self.event.settings.native_customer_accounts:
...
class AddOnsStep(CartMixin, AsyncAction, TemplateFlowStep):
    ...
    def _clean_category(self, form, category):
        ...
        elif any(sum(v[0] for k, v in selected.items() if k[0] == i) > 1 for i in category['items']) and not category[
            'multi_allowed']:
            raise ValidationError(
