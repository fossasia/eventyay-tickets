    ...
    try:
        if hasattr(request, 'event_domain'):
            ...
            if 'event' in url.kwargs and 'organizer' in url.kwargs:
                request.event = Event.objects.select_related('organizer').using(db).get(slug=url.kwargs['event'], organizer__slug=url.kwargs['organizer'])
        ...
        if hasattr(request, 'event'):
            ...
            if require_live and not request.event.live:
                can_access = url.url_name == 'event.auth' or (
                    request.user.is_authenticated and request.user.has_event_permission(request.organizer, request.event, request=request)
                )
                ...
                if not can_access:
                    return permission_denied(request, PermissionDenied(_('The selected ticket shop is currently not available.')))
        ...
        try:
            if hasattr(request, 'organizer_domain'):
                ...
                if 'event' in url.kwargs and 'organizer' in url.kwargs:
                    event = Event.objects.select_related('organizer').get(slug__iexact=url.kwargs['event'], organizer__slug__iexact=url.kwargs['organizer'])
                    pathparts = request.get_full_path().split('/')
...
def event_view(function=None, require_live=True):
    warnings.warn(
        'The event_view decorator is deprecated since it will be automatically applied by the URL routing layer when you use event_urls.', DeprecationWarning
    )
