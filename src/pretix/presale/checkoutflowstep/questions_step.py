...
class QuestionsStep(QuestionsViewMixin, CartMixin, TemplateFlowStep):
    ...
    def contact_form(self):
        ...
        initial = {
            'email': ((self.request.user.email if self.request.user.is_authenticated else '') or self.cart_session.get('email', '') or wd.get('email', '')),
    ...
    def get_question_override_sets(self, cart_position):
        return [resp for recv, resp in question_form_fields_overrides.send(self.request.event, position=cart_position, request=self.request)]

    @cached_property
    def eu_reverse_charge_relevant(self):
        return any([p.item.tax_rule and (p.item.tax_rule.eu_reverse_charge or p.item.tax_rule.custom_rules) for p in self.positions])
    ...
    def invoice_form(self):
        ...
        if not self.invoice_address.pk:
            wd_initial = {
                'name_parts': {k[21:].replace('-', '_'): v for k, v in wd.items() if k.startswith('invoice-address-name-')},
    ...
    def post(self, request):
        ...
        if self.address_asked or self.request.event.settings.invoice_name_required:
            ...
            try:
                diff = update_tax_rates(event=request.event, cart_id=get_or_create_cart_id(request), invoice_address=addr)
    ...
    def is_completed(self, request, warn=False):
        ...
        for cp in self._positions_for_questions:
            ...
            def question_is_required(q):
                return q.required and (not q.dependency_question_id or question_is_visible(q.dependency_question_id, q.dependency_values))
            ...
            for q in cp.item.questions_to_ask:
                if question_is_required(q) and q.id not in answ:
                    ...
                    return False
            if cp.item.admission and self.request.event.settings.get('attendee_names_required', as_type=bool) and not cp.attendee_name_parts:
                if warn:
                    messages.warning(request, _('Please fill in answers to all required questions.'))
                return False
            if cp.item.admission and self.request.event.settings.get('attendee_emails_required', as_type=bool) and cp.attendee_email is None:
                if warn:
                    messages.warning(request, _('Please fill in answers to all required questions.'))
                return False
            if cp.item.admission and self.request.event.settings.get('attendee_company_required', as_type=bool) and cp.company is None:
                if warn:
                    messages.warning(request, _('Please fill in answers to all required questions.'))
                return False
            if (
