...
class CartPositionCreateSerializer(I18nAwareModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if not validated_data.get('expires'):
            validated_data['expires'] = now() + timedelta(minutes=self.context['event'].settings.get('reservation_time', as_type=int))
        ...
        with self.context['event'].lock():
            ...
            if len(new_quotas) == 0:
                raise ValidationError(gettext_lazy('The product "{}" is not assigned to a quota.').format(str(validated_data.get('item'))))
            for quota in new_quotas:
                avail = quota.availability()
                if avail[0] != Quota.AVAILABILITY_OK or (avail[1] is not None and avail[1] < 1):
                    raise ValidationError(gettext_lazy('There is not enough quota available on quota "{}" to perform the operation.').format(quota.name))
