...
class PluginsField(Field):
    def to_representation(self, obj):
        ...
        return sorted([p.module for p in get_all_plugins() if not p.name.startswith('.') and getattr(p, 'visible', True) and p.module in obj.get_plugins()])
...
class ValidKeysField(Field):
    ...
    def _get(self, value):
        return {'pretix_sig1': [value.settings.ticket_secrets_pretix_sig1_pubkey] if value.settings.ticket_secrets_pretix_sig1_pubkey else []}
...
class EventSerializer(I18nAwareModelSerializer):
    ...
    def validate_live(self, value):
        if value:
            if self.instance is None:
                raise ValidationError(_("Events cannot be created as 'live'. Quotas and payment must be added to the event before sales can go live."))
            else:
    ...
    def validate_plugins(self, value):
        ...
        plugins_available = {p.module for p in get_all_plugins(self.instance) if not p.name.startswith('.') and getattr(p, 'visible', True)}
    ...
    def ignored_meta_properties(self):
        perm_holder = self.context['request'].auth if isinstance(self.context['request'].auth, (Device, TeamAPIToken)) else self.context['request'].user
    ...
    def update(self, instance, validated_data):
        ...
        if 'seating_plan' in validated_data or seat_category_mapping is not None:
            generate_seats(
                ...
                None,
                event.seating_plan,
                {m.layout_category: m.product for m in event.seat_category_mappings.select_related('product').filter(subevent=None)},
            )
...
class SubEventSerializer(I18nAwareModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if subevent.seating_plan:
            if seat_category_mapping is not None:
                for key, value in seat_category_mapping.items():
                    self.context['request'].event.seat_category_mappings.create(product=value, layout_category=key, subevent=subevent)
            generate_seats(
    ...
    def update(self, instance, validated_data):
        ...
        # Seats
        if seat_category_mapping is not None or ('seating_plan' in validated_data and validated_data['seating_plan'] is None):
            current_mappings = {m.layout_category: m for m in self.context['request'].event.seat_category_mappings.filter(subevent=subevent)}
...
class EventSettingsSerializer(SettingsSerializer):
    ...
    def get_new_filename(self, name: str) -> str:
        ...
        fname = '%s/%s/%s.%s.%s' % (self.event.organizer.slug, self.event.slug, name.split('/')[-1], nonce, name.split('.')[-1])
