...
class ItemBundleSerializer(serializers.ModelSerializer):
    class Meta:
        model = ItemBundle
        fields = ('id', 'bundled_item', 'bundled_variation', 'count', 'designated_price')
...
class ItemSerializer(I18nAwareModelSerializer):
    ...
    picture = UploadedFileField(required=False, allow_null=True, allowed_types=('image/png', 'image/jpeg', 'image/gif'), max_size=10 * 1024 * 1024)
    ...
    def validate(self, data):
        ...
        if self.instance and ('addons' in data or 'variations' in data or 'bundles' in data):
            raise ValidationError(_('Updating add-ons, bundles, or variations via PATCH/PUT is not supported. Please use the dedicated nested endpoint.'))
...
class QuestionSerializer(I18nAwareModelSerializer):
    ...
    def validate(self, data):
        ...
        if self.instance and 'options' in data:
            raise ValidationError(_('Updating options via PATCH/PUT is not supported. Please use the dedicated nested endpoint.'))
        ...
        if not self.instance:
            ...
            for opt_data in value:
                if opt_data.get('identifier'):
                    QuestionOption.clean_identifier(self.context['event'], opt_data.get('identifier'), self.instance, known)
