...
class OAuthApplication(AbstractApplication):
    ...
    redirect_uris = models.TextField(
        blank=False, validators=[URIValidator], verbose_name=_('Redirection URIs'), help_text=_('Allowed URIs list, space separated')
    )
    ...
    client_id = models.CharField(verbose_name=_('Client ID'), max_length=100, unique=True, default=generate_client_id, db_index=True)
    client_secret = ClientSecretField(verbose_name=_('Client secret'), max_length=255, blank=False, default=generate_client_secret, db_index=True)
...
class OAuthRefreshToken(AbstractRefreshToken):
    application = models.ForeignKey(OAuthApplication, on_delete=models.CASCADE)
    access_token = models.OneToOneField(OAuthAccessToken, on_delete=models.SET_NULL, blank=True, null=True, related_name='refresh_token')
