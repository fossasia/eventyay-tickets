...
class OAuthAllowForm(AllowForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['organizers'].queryset = Organizer.objects.filter(pk__in=user.teams.values_list('organizer', flat=True))
        if scope == 'profile':
    ...
    def form_valid(self, form):
        ...
        try:
            uri, headers, body, status = self.create_authorization_response(
                request=self.request, scopes=scopes, credentials=credentials, allow=allow, organizers=form.cleaned_data.get('organizers')
            )
        ...
        msgs = [_('The application "{application_name}" has been authorized to access your account.').format(application_name=application.name)]
