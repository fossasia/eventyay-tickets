...
class CheckinListViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer):
        ...
        serializer.instance.log_action('pretix.event.checkinlist.added', user=self.request.user, auth=self.request.auth, data=self.request.data)
    ...
    def perform_update(self, serializer):
        serializer.save(event=self.request.event)
        serializer.instance.log_action('pretix.event.checkinlist.changed', user=self.request.user, auth=self.request.auth, data=self.request.data)
    ...
    def status(self, *args, **kwargs):
        with language(self.request.event.settings.locale):
            ...
            cqs = clist.positions.annotate(checkedin=Exists(Checkin.objects.filter(list_id=clist.pk, position=OuterRef('pk'), type=Checkin.TYPE_ENTRY))).filter(
                checkedin=True,
            )
            pqs = clist.positions
            ...
            op_by_variation = {p['variation']: p['cnt'] for p in pqs.order_by().values('variation').annotate(cnt=Count('id'))}
            c_by_item = {p['item']: p['cnt'] for p in cqs.order_by().values('item').annotate(cnt=Count('id'))}
            c_by_variation = {p['variation']: p['cnt'] for p in cqs.order_by().values('variation').annotate(cnt=Count('id'))}
...
def _checkin_list_position_queryset(checkinlists, ignore_status=False, ignore_products=False, pdf_data=False, expand=None):
    ...
    for checkinlist in checkinlists:
        ...
        if not ignore_status:
            if checkinlist.include_pending:
                ...
                list_q &= Q(Q(order__status=Order.STATUS_PAID) | Q(order__status=Order.STATUS_PENDING, order__valid_if_pending=True))
    ...
    if pdf_data:
        qs = qs.prefetch_related(
            Prefetch(lookup='checkins', queryset=Checkin.objects.filter(list_id__in=[cl.pk for cl in checkinlists])),
            'answers',
            'answers__options',
            'answers__question',
        ...
        ).select_related('item', 'variation', 'item__category', 'addon_to', 'order', 'order__invoice_address', 'seat')
    else:
        qs = qs.prefetch_related(
            Prefetch(lookup='checkins', queryset=Checkin.objects.filter(list_id__in=[cl.pk for cl in checkinlists])),
            'answers',
    ...
    if event:
        return {
            **base_context,
            'event': event,
            'pdf_data': pdf_data
            and (user if user and user.is_authenticated else auth).has_event_permission(request.organizer, event, 'can_view_orders', request),
        }
    return base_context
...
def _filter_matching_candidates(op_candidates, list_by_event, raw_barcode, legacy_url_support):
    return [
        ...
        if (
            ...
            and (list_by_event[op.order.event_id].all_products or op.item_id in {i.pk for i in list_by_event[op.order.event_id].limit_products.all()})
        )
    ]
...
def _redeem_process(
    *,
    ...
    common_checkin_args = _get_common_checkin_args(checkinlists, checkin_type, dateandtime, device, gate, nonce, force, simulate)
    ...
    if not op_candidates:
        ...
        if len(revoked) == 0:
            return _handle_no_candidates(checkinlists, raw_barcode, common_checkin_args, dateandtime, checkin_type, user, auth, simulate)

    if len(op_candidates) > 1:
        op_candidates_matching_product = _filter_matching_candidates(op_candidates, list_by_event, raw_barcode, legacy_url_support)

        if len(op_candidates_matching_product) == 0:
            op_candidates = [op_candidates[0]]
        elif len(op_candidates_matching_product) > 1:
            return _handle_ambiguous_candidates(op_candidates[0], common_checkin_args, list_by_event, context, user, auth, simulate)
        else:
            ...
            return Response(
                {
                    'status': 'incomplete',
                    'require_attention': op.require_checkin_attention,
                    'position': CheckinListOrderPositionSerializer(op, context=_setup_context(request, expand, op.order.event, pdf_data, user, auth)).data,
...
class CheckinListPositionViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self, ignore_status=False, ignore_products=False):
        ...
        if self.request.query_params.get('ignore_status', 'false') != 'true' and not ignore_status:
            qs = qs.filter(order__status__in=[Order.STATUS_PAID, Order.STATUS_PENDING] if self.checkinlist.include_pending else [Order.STATUS_PAID])
        if self.request.query_params.get('pdf_data', 'false') == 'true':
            qs = qs.prefetch_related(
                Prefetch(lookup='checkins', queryset=Checkin.objects.filter(list_id=self.checkinlist.pk)),
                'checkins',
                'answers',
                'answers__options',
                'answers__question',
        ...
        try:
            ...
            if len(revoked_matches) == 0 or not force:
                self.request.event.log_action(
                    'pretix.event.checkin.unknown',
                    data={'datetime': dt, 'type': type, 'list': self.checkinlist.pk, 'barcode': self.kwargs['pk']},
                    user=self.request.user,
                    auth=self.request.auth,
                )
                raise Http404()
...
class CheckinSearchView(ListAPIView):
    ...
    def lists(self):
        ...
        requested_list_ids = [int(list_id) for list_id in self.request.query_params.getlist('list') if list_id.isdigit()]
    ...
    def has_full_access_permission(self):
        ...
        return CheckinList.objects.filter(event__in=events, id__in=[checkin_list.pk for checkin_list in self.lists]).count() == len(self.lists)
