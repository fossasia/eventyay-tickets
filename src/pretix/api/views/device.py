...
class DeviceSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Device
        fields = ['organizer', 'device_id', 'unique_serial', 'api_token', 'name', 'security_profile', 'gate']
...
class EventSelectionView(APIView):
    ...
    def base_subevent_qs(self):
        ...
        if self.request.auth.gate:
            has_cl = CheckinList.objects.filter(
                Q(subevent__isnull=True) | Q(subevent=OuterRef('pk')), event_id=OuterRef('event_id'), gates__in=[self.request.auth.gate]
            )
            qs = qs.annotate(has_cl=Exists(has_cl)).filter(has_cl=True)
        return qs
    ...
    def get(self, request, format=None):
        ...
        last_started_ev = self.base_event_qs.filter(first_date__lte=now()).last() or self.base_subevent_qs.filter(first_date__lte=now()).last()
    ...
    def _suggest_event(self, current_event, ev):
        ...
        if current_event and 'current_checkinlist' in self.request.query_params:
            current_checkinlist = current_event.checkin_lists.filter(pk=self.request.query_params['current_checkinlist']).first()
        ...
        if r == {
            ...
            'subevent': (int(self.request.query_params.get('current_subevent')) if self.request.query_params.get('current_subevent') else None),
            'checkinlist': (int(self.request.query_params.get('current_checkinlist')) if self.request.query_params.get('current_checkinlist') else None),
        }:
            return Response(status=status.HTTP_304_NOT_MODIFIED)
        return Response(r)
