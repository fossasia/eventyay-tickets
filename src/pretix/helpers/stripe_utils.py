...
def get_stripe_key(key_type: str) -> str:
    ...
    try:
        ...
        raise ValidationError(
            'Missing attribute for Stripe {} key: {}. Please contact the administrator to set the Stripe key.'.format(key_type, str(e)),
        )
...
def create_setup_intent(customer_id: str) -> str:
    ...
    billing_settings_updated = OrganizerBillingModel.objects.filter(stripe_customer_id=customer_id).update(stripe_setup_intent_id=stripe_setup_intent.id)
...
def update_payment_info(setup_intent_id: str, customer_id: str):
    ...
    customer_info_updated = stripe.Customer.modify(customer_id, invoice_settings={'default_payment_method': payment_method})
