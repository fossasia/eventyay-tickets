...
def send_team_webhook(self, user_id, team):
    ...
    try:
        # Send the POST request with the payload and the headers
        response = requests.post(urljoin(settings.TALK_HOSTNAME, 'webhook/team/'), json=payload, headers=headers)
        response.raise_for_status()  # Raise exception for bad status codes
    except requests.RequestException as e:
        # Log any errors that occur
        logger.error('Error sending webhook to talk component: %s', e)
...
@shared_task(bind=True, max_retries=5, default_retry_delay=60, base=SendEventTask)  # Retries up to 5 times with a 60-second delay
def send_event_webhook(self, user_id: int, event: dict, action: str) -> Optional[dict]:
    ...
    try:
        # Send the POST request with the payload and the headers
        response = requests.post(urljoin(settings.TALK_HOSTNAME, 'webhook/event/'), json=payload, headers=headers)
        response.raise_for_status()  # Raise exception for bad status codes
...
@shared_task(bind=True, max_retries=5, default_retry_delay=60, base=CreateWorldTask)  # Retries up to 5 times with a 60-second delay
def create_world(self, is_video_creation: bool, event_data: dict) -> Optional[dict]:
    ...
    try:
        return _create_world(payload=payload, headers={'Authorization': 'Bearer ' + event_data.get('token', '')})
    except requests.RequestException as e:
        try:
            self.retry(exc=e)
        except self.MaxRetriesExceededError:
...
def collect_billing_invoice(event: Event, last_month_date: datetime, ticket_rate: Decimal, invoice_voucher: Optional[InvoiceVoucher]) -> CollectBillingResponse:
    """
    ...
    ticket_fee, final_ticket_fee, voucher_discount = calculate_ticket_fee(total_amount, ticket_rate, event, invoice_voucher)
...
def update_billing_invoice_information(invoice_id: str):
    ...
    try:
        ...
        invoice_information_updated = BillingInvoice.objects.filter(
            id=invoice_id,
        ).update(status=BillingInvoice.STATUS_PAID, paid_datetime=datetime.now(), payment_method='stripe', reminder_enabled=False)
    ...
    try:
        ...
        for invoice in pending_invoices:
            if invoice.final_ticket_fee > 0:
                ...
                process_auto_billing_charge_stripe(
                    billing_settings.organizer.slug, invoice.final_ticket_fee, currency=invoice.currency, metadata=metadata, invoice_id=invoice.id
                )
            else:
...
def calculate_ticket_fee(amount: Decimal, rate: Decimal, event: Event, invoice_voucher: Optional[InvoiceVoucher] = None) -> Tuple[Decimal, Decimal, Decimal]:
    """
    ...
    def _apply_voucher(ticket_fee: Decimal, voucher_discount: Decimal, invoice_voucher: InvoiceVoucher) -> Tuple[Decimal, Decimal]:
...
def retry_failed_payment(self):
    ...
    for invoice in pending_invoices:
        ...
        for reminder_date in reminder_dates:
            ...
            if (not invoice.last_reminder_datetime or invoice.last_reminder_datetime < reminder_date) and reminder_date <= today:
