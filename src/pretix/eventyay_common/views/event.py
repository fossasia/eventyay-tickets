...
class EventCreateView(SafeSessionWizardView):
        ...
        if step == 'foundation' and 'organizer' in request_get:
            try:
                ...
                if not request_user.has_active_staff_session(self.request.session.session_key):
                    queryset = queryset.filter(id__in=request_user.teams.filter(can_create_events=True).values_list('organizer', flat=True))
    ...
    def done(self, form_list, form_dict, **kwargs):
        ...
        create_world.delay(is_video_creation=foundation_data.get('is_video_creation'), event_data=event_data)

        return redirect(reverse('eventyay_common:event.index', kwargs={'event': event.slug, 'organizer': event.organizer.slug}))
...
class EventUpdate(
    ...
    def get_context_data(self, *args, **kwargs) -> dict:
        ...
        if self.object.settings.create_for == EventCreatedFor.BOTH or self.object.settings.talk_schedule_public is not None:
