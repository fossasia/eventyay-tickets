...
class EventIndexView(TemplateView):
    ...
    def _get_user_permissions(self) -> Dict[str, bool]:
        ...
        return {
            ...
            'can_change_items': request.user.has_event_permission(request.organizer, request.event, 'can_change_items', request=request),
        }
    ...
    def _check_event_statuses(self, can_view_orders: bool) -> Dict[str, Any]:
        ...
        return {
            ...
            'has_pending_approvals': can_view_orders and request.event.orders.filter(status=Order.STATUS_PENDING, require_approval=True).exists(),
            'has_cancellation_requests': can_view_orders and CancellationRequest.objects.filter(order__event=request.event).exists(),
        }
        ...
        context.update(
            {
                ...
                'logs': qs[:5],
                'subevent': subevent,
                'actions': (request.event.requiredaction_set.filter(done=False)[:5] if permissions['can_change_orders'] else []),
...
class EventWidgetGenerator:
    ...
    def get_event_query(qs: QuerySet[Event], nmax: int, lazy: bool = False) -> QuerySet[Event]:
        ...
        return qs.prefetch_related('_settings_objects', 'organizer___settings_objects').select_related('organizer')[:nmax]
...
def widgets_for_event_qs(request: HttpRequest, qs: QuerySet[Event], nmax: int, lazy: bool = False) -> List[Dict[str, Any]]:
    """
