...
class BadgePreviewView(APIView):
    ...
    def get(self, request, organizer, event, position):
        op = get_object_or_404(OrderPosition, order__event__slug=event, order__event__organizer__slug=organizer, pk=position)

        # Check if badges plugin is enabled
        if 'pretix.plugins.badges' not in op.order.event.plugins:
            return Response({'error': 'Badges plugin is not enabled for this event'}, status=status.HTTP_400_BAD_REQUEST)
...
class BadgeDownloadView(APIView):
    ...
    def get(self, request, organizer, event, position):
        try:
            ...
            cached_file = CachedFile.objects.filter(filename__startswith=f'badge_{position}_', expires__isnull=True).last()

            if cached_file and cached_file.file:
                base64_pdf = base64.b64encode(cached_file.file.read()).decode('utf-8')
                return Response({'filename': cached_file.filename, 'type': 'application/pdf', 'base64_pdf': base64_pdf})
            ...
            try:
                ...
                return Response({'filename': filename, 'mimetype': mimetype, 'pdf_base64': base64_pdf})

            except Exception:
                # If immediate generation fails, fall back to async generation
                generate_orderposition.apply_async(args=(op.pk, 'badge'))
                return Response(
                    {'status': 'generating', 'message': 'Badge generation has been started. Please retry in a few seconds.'}, status=status.HTTP_202_ACCEPTED
                )
