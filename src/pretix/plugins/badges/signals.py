...
def control_nav_import(sender, request=None, **kwargs):
    ...
    p = request.user.has_event_permission(request.organizer, request.event, 'can_change_settings', request) or request.user.has_event_permission(
...
def control_item_forms(sender, request, item, **kwargs):
    ...
    return BadgeItemForm(instance=inst, event=sender, data=(request.POST if request.method == 'POST' else None), prefix='badgeitem')
...
def _cached_rendermap(event):
    ...
    renderermap = {bi.item_id: bi.layout_id for bi in BadgeItem.objects.select_related('layout').filter(item__event=event)}
    try:
...
def badges_logentry_object_link(sender, logentry, **kwargs):
    if not logentry.action_type.startswith('pretix.plugins.badges.layout') or not isinstance(logentry.content_object, BadgeLayout):
        return

    a_text = _('Badge layout {val}')
    a_map = {
        'href': reverse('plugins:badges:edit', kwargs={'event': sender.slug, 'organizer': sender.organizer.slug, 'layout': logentry.content_object.id}),
