...
class CheckInListMixin(BaseExporter):
    ...
    def _fields(self):
        ...
        d = OrderedDict(
            [
                ...
                ('secrets', forms.BooleanField(label=_('Include QR-code secret'), required=False)),
                ('attention_only', forms.BooleanField(label=_('Only tickets requiring special attention'), required=False)),
                (
                    'sort',
                    forms.ChoiceField(
                        label=_('Sort by'),
                        ...
                        + (
                            [('name:{}'.format(k), _('Attendee name: {part}').format(part=label)) for k, label, w in name_scheme['fields']]
    ...
    def _get_queryset(self, cl, form_data):
        cqs = Checkin.objects.filter(position_id=OuterRef('pk'), list_id=cl.pk).order_by().values('position_id').annotate(m=Max('datetime')).values('m')
        ...
        qs = (
            ...
            .annotate(
                ...
                auto_checked_in=Exists(Checkin.objects.filter(position_id=OuterRef('pk'), list_id=cl.pk, auto_checked_in=True)),
            )
            .prefetch_related('answers', 'answers__question', 'addon_to__answers', 'addon_to__answers__question')
        ...
        if form_data.get('date_from'):
            dt = make_aware(datetime.combine(dateutil.parser.parse(form_data['date_from']).date(), time(hour=0, minute=0, second=0)), self.event.timezone)
            qs = qs.filter(subevent__date_from__gte=dt)

        if form_data.get('date_to'):
            dt = make_aware(
                datetime.combine(dateutil.parser.parse(form_data['date_to']).date() + timedelta(days=1), time(hour=0, minute=0, second=0)), self.event.timezone
            )
...
class PDFCheckinList(ReportlabExportMixin, CheckInListMixin, BaseExporter):
    ...
    def get_story(self, doc, form_data):
        ...
        colwidths = [3 * mm, 8 * mm, 8 * mm] + [
            a * (doc.width - 8 * mm) for a in [0.1, 0.25, (0.25 if questions else 0.60)] + ([0.35 / len(questions)] * len(questions) if questions else [])
        ]
        tstyledata = [
        ...
        for op in qs:
            ...
            if self.event.has_subevents and not cl.subevent:
                item += '<br/>{} ({})'.format(op.subevent.name, date_format(op.subevent.date_from.astimezone(self.event.timezone), 'SHORT_DATETIME_FORMAT'))
            if op.seat:
                item += '<br/>' + str(op.seat)
            row = [
...
class CSVCheckinList(CheckInListMixin, ListExporter):
    ...
    def iterate_list(self, form_data):
        ...
        headers += [_('Product'), _('Price'), _('Checked in'), _('Checked out'), _('Automatically checked in')]
        if not cl.include_pending:
            qs = qs.filter(order__status=Order.STATUS_PAID)
        else:
            qs = qs.filter(order__status__in=(Order.STATUS_PAID, Order.STATUS_PENDING))
        ...
        for op in qs:
            ...
            if len(name_scheme['fields']) > 1:
                for k, label, w in name_scheme['fields']:
                    row.append((op.attendee_name_parts or (op.addon_to.attendee_name_parts if op.addon_to else {}) or ia.name_parts).get(k, ''))
            row += [
                str(op.item) + (' – ' + str(op.variation.value) if op.variation else ''),
                op.price,
            ...
            if self.event.has_subevents:
                ...
                if op.subevent.date_to:
                    row.append(date_format(op.subevent.date_to.astimezone(self.event.timezone), 'SHORT_DATETIME_FORMAT'))
                else:
                    row.append('')
            acache = {}
            if op.addon_to:
...
class CheckinLogList(ListExporter):
    ...
    def iterate_list(self, form_data):
        ...
        qs = qs.select_related('position__item', 'position__order', 'position__order__invoice_address', 'position', 'list', 'device').order_by('datetime')
