    ...
    def get_context_data(self, **kwargs):
        ...
        if not ctx['obd_data']:
            ...
            for d in dateutil.rrule.rrule(
                ...
                d = d.date()
                data.append({'date': d.strftime('%Y-%m-%d'), 'ordered': ordered_by_day.get(d, 0), 'paid': paid_by_day.get(d, 0)})
        ...
        if not ctx['obp_data']:
            ...
            num_ordered = {p['item']: p['cnt'] for p in (opqs.filter(order__event=self.request.event).values('item').annotate(cnt=Count('id')).order_by())}
            num_paid = {
                p['item']: p['cnt']
                for p in (opqs.filter(order__event=self.request.event, order__status=Order.STATUS_PAID).values('item').annotate(cnt=Count('id')).order_by())
            }
        ...
        if not ctx['rev_data']:
            ...
            if subevent:
                for o in (
                    ...
                    .filter(order__event=self.request.event, subevent=subevent, order__status=Order.STATUS_PAID, payment_date__isnull=False)
                ...
                for o in (
                    ...
                    .filter(event=self.request.event, status=Order.STATUS_PAID, payment_date__isnull=False)
        ...
        if not self.request.event.has_subevents or (ckey != 'all' and subevent):
            ...
            if ev.seating_plan_id is not None:
                ...
                ctx['seats']['purchased_seats'] = ev.seats.count() - ctx['seats']['blocked_seats'] - ctx['seats']['free_seats']
                ...
                for item in seats_qs:
                    ...
                    if item_cache[item['product']] not in ctx['seats']['products']:
                        ...
                        if product and product.has_variations:
                            price = product.variations.filter(active=True).aggregate(Min('default_price'))['default_price__min']
