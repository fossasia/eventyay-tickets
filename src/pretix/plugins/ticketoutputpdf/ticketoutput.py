    ...
    def layout_map(self):
        if not hasattr(self.event, '_ticketoutputpdf_cache_layoutmap'):
            self.event._ticketoutputpdf_cache_layoutmap = {
                (bi.item_id, bi.sales_channel): bi.layout for bi in TicketLayoutItem.objects.select_related('layout').filter(item__event=self.event)
            }
    ...
    def default_layout(self):
        if not hasattr(self.event, '_ticketoutputpdf_cache_default_layout'):
            try:
                ...
                self.event._ticketoutputpdf_cache_default_layout = TicketLayout(layout=json.dumps(self._default_layout()))
        ...
        with language(order.locale, self.event.settings.region):
            for op in order.positions_with_tickets:
                layout = override_layout.send_chained(
                    ...
                    'layout',
                    orderposition=op,
                    layout=self.layout_map.get((op.item_id, order.sales_channel), self.layout_map.get((op.item_id, 'web'), self.default_layout)),
                )
