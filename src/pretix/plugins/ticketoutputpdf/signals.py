...
def control_item_forms(sender, request, item, **kwargs):
    ...
    for k, v in sorted(list(get_all_sales_channels().items()), key=lambda a: (int(a[0] != 'web'), a[0])):
        ...
        forms.append(
            TicketLayoutItemForm(
                instance=inst, event=sender, sales_channel=v, data=(request.POST if request.method == 'POST' else None), prefix='ticketlayoutitem_{}'.format(k)
            )
        )
    return forms
...
def pdf_event_copy_data_receiver(sender, other, item_map, question_map, **kwargs):
    ...
    for bi in TicketLayoutItem.objects.filter(item__event=other):
        TicketLayoutItem.objects.create(item=item_map.get(bi.item_id), layout=layout_map.get(bi.layout_id), sales_channel=bi.sales_channel)
    return layout_map
...
def pdf_logentry_object_link(sender, logentry, **kwargs):
    if not logentry.action_type.startswith('pretix.plugins.ticketoutputpdf.layout') or not isinstance(logentry.content_object, TicketLayout):
        return

    a_text = _('Ticket layout {val}')
    a_map = {
        'href': reverse(
            'plugins:ticketoutputpdf:edit', kwargs={'event': sender.slug, 'organizer': sender.organizer.slug, 'layout': logentry.content_object.id}
        ),
