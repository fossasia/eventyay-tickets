...
class LayoutCreate(EventPermissionRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        form.instance.log_action('pretix.plugins.ticketoutputpdf.layout.added', user=self.request.user, data=dict(form.cleaned_data))
        return redirect(
            reverse(
                'plugins:ticketoutputpdf:edit',
                kwargs={'organizer': self.request.event.organizer.slug, 'event': self.request.event.slug, 'layout': form.instance.pk},
            )
        )
...
class LayoutGetDefault(EventPermissionRequiredMixin, View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return redirect(
            reverse(
                'plugins:ticketoutputpdf:edit', kwargs={'organizer': self.request.event.organizer.slug, 'event': self.request.event.slug, 'layout': layout.pk}
            )
        )
...
class LayoutEditorView(BaseEditorView):
    ...
    def save_layout(self):
        ...
        self.layout.log_action(action='pretix.plugins.ticketoutputpdf.layout.changed', user=self.request.user, data={'layout': self.request.POST.get('data')})
        invalidate_cache.apply_async(kwargs={'event': self.request.event.pk, 'provider': 'pdf'})
