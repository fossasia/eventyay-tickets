...
class ActionView(View):
    ...
    def _accept_ignore_amount(self, trans):
        if trans.amount < Decimal('0.00'):
            ref = trans.order.refunds.filter(
                amount=trans.amount * -1, provider='manual', state__in=(OrderRefund.REFUND_STATE_CREATED, OrderRefund.REFUND_STATE_CREATED)
            ).first()
            p = trans.order.payments.filter(
                amount=trans.amount * -1, provider='banktransfer', state__in=(OrderPayment.PAYMENT_STATE_CONFIRMED, OrderPayment.PAYMENT_STATE_REFUNDED)
            ).first()
            if ref:
                ...
                p.create_external_refund(
                    ...
                    info=json.dumps(
                        {'reference': trans.reference, 'date': trans.date, 'payer': trans.payer, 'iban': trans.iban, 'bic': trans.bic, 'trans_id': trans.pk}
                    ),
                )
                trans.state = BankTransaction.STATE_VALID
                trans.save()
                ...
                return JsonResponse({'status': 'error', 'message': _("Negative amount but refund can't be logged, please create manual refund first.")})
        ...
        p.info_data = {'reference': trans.reference, 'date': trans.date, 'payer': trans.payer, 'iban': trans.iban, 'bic': trans.bic, 'trans_id': trans.pk}
        try:
    ...
    def post(self, request, *args, **kwargs):
        for k, v in request.POST.items():
            ...
            elif v.startswith('assign:') and trans.state in (BankTransaction.STATE_NOMATCH, BankTransaction.STATE_DUPLICATE):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return JsonResponse(
            {
                'results': [
                    {'code': o.event.slug.upper() + '-' + o.code, 'status': o.get_status_display(), 'total': money_filter(o.total, o.event.currency)}
                    for o in qs
                ]
            }
        )
...
class JobDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx['transactions_invalid'] = qs.filter(state__in=[BankTransaction.STATE_INVALID, BankTransaction.STATE_ERROR]).count()
        ctx['transactions_ignored'] = qs.filter(state__in=[BankTransaction.STATE_DUPLICATE, BankTransaction.STATE_NOMATCH]).count()
        ctx['job'] = self.job
...
class ImportView(ListView):
    ...
    def get_queryset(self):
        ...
        qs = qs.select_related('order').filter(
            state__in=[BankTransaction.STATE_INVALID, BankTransaction.STATE_ERROR, BankTransaction.STATE_DUPLICATE, BankTransaction.STATE_NOMATCH]
        )
    ...
    def post(self, *args, **kwargs):
        if self.request.POST.get('discard', '') == 'all':
            ...
            messages.error(self.request, _('We were unable to detect the file type of this import. Please contact support for help.'))
            return self.redirect_back()
    ...
    def process_csv_file(self):
        ...
        try:
            ...
            messages.error(self.request, _("I'm sorry, but we were unable to import this CSV file. Please contact support for help."))
            return self.redirect_back()

        if len(data) == 0:
            messages.error(self.request, _("I'm sorry, but we detected this file as empty. Please contact support for help."))
    ...
    def get_context_data(self, **kwargs):
        ...
        if 'event' in self.kwargs:
            ...
            ctx['lastimport'] = (
                BankImportJob.objects.filter(state=BankImportJob.STATE_COMPLETED, organizer=self.request.organizer, event=self.request.event)
                .order_by('created')
                .last()
            )
            ctx['runningimport'] = (
                BankImportJob.objects.filter(
                    state__in=[BankImportJob.STATE_PENDING, BankImportJob.STATE_RUNNING], organizer=self.request.organizer, event=self.request.event
                )
                .order_by('created')
                .last()
            )
            ...
            ctx['lastimport'] = (
                BankImportJob.objects.filter(state=BankImportJob.STATE_COMPLETED, organizer=self.request.organizer, event__isnull=True)
                .order_by('created')
                .last()
            )
            ctx['runningimport'] = (
                BankImportJob.objects.filter(
                    state__in=[BankImportJob.STATE_PENDING, BankImportJob.STATE_RUNNING], organizer=self.request.organizer, event__isnull=True
                )
                .order_by('created')
                .last()
            )
...
class OrganizerBanktransferView:
    def dispatch(self, request, *args, **kwargs):
        if len(request.organizer.events.order_by('currency').values_list('currency', flat=True).distinct()) > 1:
            messages.error(request, _('Please perform per-event bank imports as this organizer has events with multiple currencies.'))
...
class OrganizerImportView(OrganizerBanktransferView, OrganizerPermissionRequiredMixin, OrganizerDetailViewMixin, ImportView):
    permission = 'can_change_orders'
...
class OrganizerJobDetailView(OrganizerBanktransferView, OrganizerPermissionRequiredMixin, OrganizerDetailViewMixin, JobDetailView):
    permission = 'can_change_orders'
...
class OrganizerActionView(OrganizerBanktransferView, OrganizerPermissionRequiredMixin, OrganizerDetailViewMixin, ActionView):
    permission = 'can_change_orders'

    def order_qs(self):
        all = self.request.user.teams.filter(organizer=self.request.organizer, can_change_orders=True, can_view_orders=True, all_events=True).exists()
        if self.request.user.has_active_staff_session(self.request.session.session_key) or all:
            ...
            return Order.objects.filter(
                event_id__in=self.request.user.teams.filter(organizer=self.request.organizer, can_change_orders=True, can_view_orders=True).values_list(
    ...
    def post(self, request, *args, **kwargs):
        ...
        for refund in self.get_unexported().select_related('order', 'order__event'):
            if not refund.info_data:
                ...
                messages.warning(
                    request,
                    _('We could not find bank account information for the refund {refund_id}. It was marked as failed.').format(refund_id=refund.full_id),
                )
        ...
        if valid_refunds:
            ...
            for refund in valid_refunds:
                ...
                transaction_rows.append(
                    {'amount': refund.amount, 'id': refund.full_id, 'comment': refund.comment, **{key: data.get(key) for key in ('payer', 'iban', 'bic')}}
                )
...
class OrganizerRefundExportListView(OrganizerPermissionRequiredMixin, RefundExportListView):
    ...
    def dispatch(self, request, *args, **kwargs):
        if len(request.organizer.events.order_by('currency').values_list('currency', flat=True).distinct()) > 1:
            messages.error(request, _('Please perform per-event refund exports as this organizer has events with multiple currencies.'))
            return redirect('control:organizer', organizer=request.organizer.slug)
        return super().dispatch(request, *args, **kwargs)
    ...
    def get_queryset(self):
        return RefundExport.objects.filter(Q(organizer=self.request.organizer) | Q(event__organizer=self.request.organizer)).order_by('-datetime')
