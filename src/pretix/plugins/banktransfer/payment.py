...
class BankTransfer(BasePaymentProvider):
            ...
            for f in (
                ...
                if not cleaned_data.get("payment_banktransfer_%s" % f):
                    raise ValidationError(
                        {
                            "payment_banktransfer_%s"
                            % f: _("Please fill out your bank account details.")
                        }
                    )
        else:
    ...
    def execute_refund(self, refund: OrderRefund):
        ...
        refund.info_data = {
            ...
            "bic": (
                self.norm(refund.payment.info_data["bic"])
    ...
    def new_refund_control_form_render(self, request: HttpRequest, order: Order) -> str:
        f = self.NewRefundForm(
            ...
            data=(
                ...
                and request.POST.get("refund-banktransfer-iban")
                else None
            ),
        )
