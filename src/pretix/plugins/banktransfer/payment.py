...
class BankTransfer(BasePaymentProvider):
    ...
    def form_fields():
        return OrderedDict(
            [
                ...
                ('public_name', I18nFormField(label=_('Payment method name'), widget=I18nTextInput, required=False)),
                (
                    'omit_hyphen',
                    forms.BooleanField(
                        label=_('Do not include hyphens in the payment reference.'), help_text=_('This is required in some countries.'), required=False
                    ),
                ),
                ('include_invoice_number', forms.BooleanField(label=_('Include invoice number in the payment reference.'), required=False)),
                (
                    'prefix',
                    forms.CharField(
                ...
                (
                    ...
                    I18nFormField(
                        ...
                        help_text=_('This text will be shown on the order confirmation page for pending orders in addition to the standard text.'),
                        widget=I18nTextarea,
                        required=False,
                    ),
                ),
        ...
        if cleaned_data.get('payment_banktransfer_bank_details_type') == 'sepa':
            for f in ('bank_details_sepa_name', 'bank_details_sepa_bank', 'bank_details_sepa_bic', 'bank_details_sepa_iban'):
                if not cleaned_data.get('payment_banktransfer_%s' % f):
                    raise ValidationError({'payment_banktransfer_%s' % f: _('Please fill out your bank account details.')})
        else:
            if not cleaned_data.get('payment_banktransfer_bank_details'):
                raise ValidationError({'payment_banktransfer_bank_details': _('Please enter your bank account details.')})
        return cleaned_data
    ...
    def order_pending_mail_render(self, order, payment) -> str:
        ...
        if self.settings.get('bank_details_type') == 'sepa':
            bankdetails += [
                _('Account holder'),
                ': ',
                self.settings.get('bank_details_sepa_name'),
                '\n',
                _('IBAN'),
                ': ',
                ...
                _('BIC'),
                ': ',
                self.settings.get('bank_details_sepa_bic'),
                '\n',
                _('Bank'),
                ': ',
                self.settings.get('bank_details_sepa_bank'),
            ]
    ...
    def _render_control_info(self, request, order, info_data, **extra_context):
        ...
        ctx = {'request': request, 'event': self.event, 'code': self._code(order), 'payment_info': info_data, 'order': order, **extra_context}
        return template.render(ctx)
    ...
    def new_refund_control_form_process(self, request: HttpRequest, amount: Decimal, order: Order) -> OrderRefund:
        ...
        return OrderRefund(order=order, payment=None, state=OrderRefund.REFUND_STATE_CREATED, amount=amount, provider=self.identifier, info=json.dumps(d))
