...
class PermissionMiddleware:
    ...
    def __call__(self, request):
        ...
        if not request.path.startswith(get_script_prefix() + 'control') and not request.path.startswith(get_script_prefix() + 'common'):
        ...
        if 'event' in url.kwargs and 'organizer' in url.kwargs:
            ...
            if not request.event or not request.user.has_event_permission(request.event.organizer, request.event, request=request):
    ...
    def __call__(self, request):
        if request.path.startswith(get_script_prefix() + 'control') and request.user.is_authenticated:
            if getattr(request.user, 'is_hijacked', False):
                ...
                if ss:
                    ss.logs.create(url=request.path, method=request.method, impersonating=request.user)
            else:
