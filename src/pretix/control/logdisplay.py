...
def _display_order_changed(event: Event, logentry: LogEntry):
    ...
    if logentry.action_type == 'pretix.event.order.changed.item':
        ...
        return (
            text
            + ' '
            + _('Position #{posid}: {old_item} ({old_price}) changed to {new_item} ({new_price}).').format(
        ...
        return (
            text
            + ' '
            + _('Position #{posid}: Event date "{old_event}" ({old_price}) changed to "{new_event}" ({new_price}).').format(
                posid=data.get('positionid', '?'),
                old_event=old_se,
                new_event=new_se,
                old_price=money_filter(Decimal(data['old_price']), event.currency),
        ...
        if data['addon_to']:
            ...
            return (
                text
                + ' '
                + _('Position #{posid} created: {item} ({price}) as an add-on to position #{addon_to}.').format(
                    posid=data.get('positionid', '?'),
                    item=item,
        ...
        if data['old_variation']:
            old_item += ' - ' + str(ItemVariation.objects.get(pk=data['old_variation']))
        url = reverse('control:event.order', kwargs={'event': event.slug, 'organizer': event.organizer.slug, 'code': data['new_order']})
        return mark_safe(
            escape(text)
            + ' '
            + _('Position #{posid} ({old_item}, {old_price}) split into new order: {order}').format(
...
def _display_checkin(event, logentry):
    ...
    if logentry.action_type == 'pretix.event.checkin.unknown':
        if show_dt:
            return _('Unknown scan of code "{barcode}…" at {datetime} for list "{list}", type "{type}".').format(
                posid=data.get('positionid'), type=data.get('type'), barcode=data.get('barcode')[:16], datetime=dt_formatted, list=checkin_list
            )
        else:
            return _('Unknown scan of code "{barcode}…" for list "{list}", type "{type}".').format(
                posid=data.get('positionid'), type=data.get('type'), barcode=data.get('barcode')[:16], list=checkin_list
            )

    if logentry.action_type == 'pretix.event.checkin.revoked':
        if show_dt:
            return _('Scan scan of revoked code "{barcode}…" at {datetime} for list "{list}", type "{type}", was uploaded.').format(
                posid=data.get('positionid'), type=data.get('type'), barcode=data.get('barcode')[:16], datetime=dt_formatted, list=checkin_list
            )
        else:
    ...
    if logentry.action_type == 'pretix.event.checkin.denied':
        if show_dt:
            return _('Denied scan of position #{posid} at {datetime} for list "{list}", type "{type}", error code "{errorcode}".').format(
                posid=data.get('positionid'), type=data.get('type'), errorcode=data.get('errorcode'), datetime=dt_formatted, list=checkin_list
            )
        else:
            return _('Denied scan of position #{posid} for list "{list}", type "{type}", error code "{errorcode}".').format(
                posid=data.get('positionid'), type=data.get('type'), errorcode=data.get('errorcode'), list=checkin_list
            )
    ...
    if data.get('type') == Checkin.TYPE_EXIT:
        if show_dt:
            ...
            return _('Position #{posid} has been checked out for list "{list}".').format(posid=data.get('positionid'), list=checkin_list)
    if data.get('first'):
        if show_dt:
            ...
            return _('Position #{posid} has been checked in for list "{list}".').format(posid=data.get('positionid'), list=checkin_list)
    else:
        if data.get('forced'):
            return _(
                'A scan for position #{posid} at {datetime} for list "{list}" has been uploaded even though it has been scanned already.'.format(
                    posid=data.get('positionid'), datetime=dt_formatted, list=checkin_list
                )
            )
        return _(
            'Position #{posid} has been scanned and rejected because it has already been scanned before on list "{list}".'.format(
...
def pretixcontrol_logentry_display(sender: Event, logentry: LogEntry, **kwargs):
    plains = {
        ...
        'pretix.event.order.contact.changed': _('The email address has been changed from "{old_email}" to "{new_email}".'),
        'pretix.event.order.contact.confirmed': _(
            'The email address has been confirmed to be working (the user clicked on a link in the email for the first time).'
        ),
        'pretix.event.order.phone.changed': _('The phone number has been changed from "{old_phone}" to "{new_phone}".'),
        ...
        'pretix.event.order.checkin_attention': _("The order's flag to require attention at check-in has been toggled."),
        ...
        'pretix.event.order.email.download_reminder_sent': _('An email has been sent with a reminder that the ticket is available for download.'),
        'pretix.event.order.email.expire_warning_sent': _('An email has been sent with a warning that the order is about to expire.'),
        'pretix.event.order.email.order_canceled': _('An email has been sent to notify the user that the order has been canceled.'),
        'pretix.event.order.email.event_canceled': _('An email has been sent to notify the user that the event has been canceled.'),
        ...
        'pretix.event.order.email.order_approved': _('An email has been sent to notify the user that the order has been approved.'),
        ...
        'pretix.user.settings.2fa.device.added': _('A new two-factor authentication device "{name}" has been added to your account.'),
        'pretix.user.settings.2fa.device.deleted': _('The two-factor authentication device "{name}" has been removed from your account.'),
        ...
        'pretix.user.oauth.authorized': _('The application "{application_name}" has been authorized to access your account.'),
    ...
    if logentry.action_type == 'pretix.event.order.consent':
        return _('The user confirmed the following message: "{}"').format(bleach.clean(logentry.parsed_data.get('msg'), tags=[], strip=True))
    ...
    if logentry.action_type == 'pretix.team.member.joined':
        return _('{user} has joined the team using the invite sent to {email}.').format(user=data.get('email'), email=data.get('invite_email'))
