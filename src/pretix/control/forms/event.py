...
class EventWizardCopyForm(forms.Form):
    ...
    def copy_from_queryset(user, session):
        ...
        return Event.objects.filter(
            ...
            | Q(id__in=user.teams.filter(can_change_event_settings=True, can_change_items=True).values_list('limit_events__id', flat=True))
        )
...
class EventSettingsForm(SettingsForm):
    ...
    name_scheme_titles = forms.ChoiceField(
        ...
        help_text=_('If the naming scheme you defined above allows users to input a title, you can use this to restrict the set of selectable titles.'),
        required=False,
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['name_scheme_titles'].choices = [('', _('Free text input'))] + [
            (k, '{scheme}: {samples}'.format(scheme=v[0], samples=', '.join(v[1]))) for k, v in PERSON_NAME_TITLE_GROUPS.items()
        ]
...
class InvoiceSettingsForm(SettingsForm):
    ...
    invoice_generate_sales_channels = forms.MultipleChoiceField(
        ...
        choices=[],
        widget=forms.CheckboxSelectMultiple,
        help_text=_('If you have enabled invoice generation in the previous setting, you can limit it here to specific sales channels.'),
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
        self.fields['invoice_renderer'].choices = [(r.identifier, r.verbose_name) for r in event.get_invoice_renderers().values()]
        ...
        self.fields['invoice_generate_sales_channels'].choices = ((c.identifier, c.verbose_name) for c in get_all_sales_channels().values())
...
class MailSettingsForm(SettingsForm):
    ...
    mail_days_order_expire_warning = forms.IntegerField(
        ...
        required=True,
        min_value=0,
        help_text=_('This email will be sent out this many days before the order expires. If the value is 0, the mail will never be sent.'),
    )
    ...
    mail_days_download_reminder = forms.IntegerField(
        ...
        required=False,
        min_value=0,
        help_text=_('This email will be sent out this many days before the order event starts. If the field is empty, the mail will never be sent.'),
    )
    ...
    mail_text_order_approved = I18nFormField(
        ...
        required=False,
        widget=I18nTextarea,
        help_text=_('This will only be sent out for non-free orders. Free orders will receive the free order template from below instead.'),
    )
    mail_text_order_approved_free = I18nFormField(
        label=_('Approved free order'),
        required=False,
        widget=I18nTextarea,
        help_text=_('This will only be sent out for free orders. Non-free orders will receive the non-free order template from above instead.'),
    )
    ...
    email_vendor = forms.ChoiceField(label=_(''), required=True, widget=forms.RadioSelect, choices=smtp_select)
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
        self.fields['mail_html_renderer'].choices = [(r.identifier, r.verbose_name) for r in event.get_html_mail_renderers().values()]
...
class TicketSettingsForm(SettingsForm):
    ...
    def __init__(self, *args, **kwargs):
        event = kwargs.get('obj')
        super().__init__(*args, **kwargs)
        self.fields['ticket_secret_generator'].choices = [(r.identifier, r.verbose_name) for r in event.ticket_secret_generators.values()]
...
class WidgetCodeForm(forms.Form):
    ...
    compatibility_mode = forms.BooleanField(
        ...
        required=False,
        help_text=_("Our regular widget doesn't work in all website builders. If you run into trouble, try using this compatibility mode."),
    )
...
class QuickSetupForm(I18nForm):
    ...
    attendee_names_required = forms.BooleanField(
        ...
        help_text=_('By default, we will ask for names but not require them. You can turn this off completely in the settings.'),
        required=False,
    )
