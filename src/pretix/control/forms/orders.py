...
class ExtendForm(I18nModelForm):
    quota_ignore = forms.BooleanField(
        ...
        help_text=_(
            'If you check this box, this operation will be performed even if it leads to an overbooked quota and you having sold more tickets than you planned!'
        ),
        required=False,
    )
    ...
    def clean(self):
        ...
        if data.get('expires'):
            if isinstance(data['expires'], date):
                data['expires'] = make_aware(datetime.combine(data['expires'], time(hour=23, minute=59, second=59)), self.instance.event.timezone)
            else:
...
class ForceQuotaConfirmationForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
        quota_success = self.instance.status == Order.STATUS_PENDING or self.instance._is_still_available(now(), count_waitinglist=False) is True
    ...
    def __init__(self, *args, **kwargs):
        ...
        if prs > 0:
            ...
            self.fields['cancellation_fee'].widget.attrs['placeholder'] = floatformat(
                Decimal('0.00'), settings.CURRENCY_PLACES.get(self.instance.event.currency, 2)
            )
...
class OtherOperationsForm(forms.Form):
    ...
    notify = forms.BooleanField(
        label=_('Notify user'), required=False, initial=True, help_text=_('Send an email to the customer notifying that their order has been changed.')
    )
...
class OrderPositionAddForm(forms.Form):
    ...
    seat = forms.CharField(
        required=False, widget=forms.TextInput(attrs={'placeholder': _('General admission'), 'data-seat-guid-field': 'true'}), label=_('Seat')
    )
    ...
    subevent = forms.ModelChoiceField(SubEvent.objects.none(), label=pgettext_lazy('subevent', 'Date'), required=True, empty_label=None)
        ...
        for i in self.items:
            ...
            if variations:
                for v in variations:
                    ...
                    choices.append(('%d-%d' % (i.pk, v.pk), '%s – %s (%s)' % (pname, v.value, p.print(order.event.currency))))
            else:
                p = get_price(i, invoice_address=ia)
                choices.append((str(i.pk), '%s (%s)' % (pname, p.print(order.event.currency))))
        self.fields['itemvar'].choices = choices
        if order.event.cache.get_or_set('has_addon_products', default=lambda: ItemAddOn.objects.filter(base_item__event=order.event).exists(), timeout=300):
            self.fields['addon_to'].queryset = order.positions.filter(addon_to__isnull=True).select_related('item', 'variation')
        else:
...
class OrderPositionChangeForm(forms.Form):
    ...
    seat = forms.CharField(required=False, widget=forms.TextInput(attrs={'placeholder': _('(Unchanged)'), 'data-seat-guid-field': 'true'}))
    price = forms.DecimalField(required=False, max_digits=10, decimal_places=2, localize=True, label=_('New price (gross)'))
...
class OrderFeeChangeForm(forms.Form):
    value = forms.DecimalField(required=False, max_digits=10, decimal_places=2, localize=True, label=_('New price (gross)'))
    tax_rule = forms.ModelChoiceField(TaxRule.objects.none(), required=False, empty_label=_('(Unchanged)'))
...
class OrderRefundForm(forms.Form):
    action = forms.ChoiceField(
        ...
        choices=(
            ...
            ('mark_pending', _('Mark the order as pending and allow the user to pay the open amount with another payment method.')),
...
class EventCancelForm(forms.Form):
    subevent = forms.ModelChoiceField(
        SubEvent.objects.none(), label=pgettext_lazy('subevent', 'Date'), required=False, empty_label=pgettext_lazy('subevent', 'All dates')
    )
    ...
    keep_fee_fixed = forms.DecimalField(label=_('Keep a fixed cancellation fee'), max_digits=10, decimal_places=2, required=False)
    ...
    keep_fee_percentage = forms.DecimalField(label=_('Keep a percentual cancellation fee'), max_digits=10, decimal_places=2, required=False)
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._set_field_placeholders('send_subject', ['event_or_subevent', 'refund_amount', 'position_or_address', 'order', 'event'])
        self._set_field_placeholders('send_message', ['event_or_subevent', 'refund_amount', 'position_or_address', 'order', 'event'])
