...
from pretix.base.customersso.oidc import validate_config
...
class SSOProviderForm(I18nModelForm):
    ...
    config_oidc_scope = forms.CharField(
        ...
        help_text=pgettext_lazy('sso_oidc', 'Separated with spaces.'),
        required=False,
    )
    config_oidc_uid_field = forms.CharField(
        label=pgettext_lazy('sso_oidc', 'User ID field'),
        help_text=pgettext_lazy('sso_oidc', 'Must be unique and will not be changed for a user.'),
        required=True,
        initial='sub',
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.initialize_name_scheme_fields()
        self.filter_method_choices()
        self.initialize_config_fields()

    def initialize_name_scheme_fields(self):
    ...
    def clean(self):
        ...
        if method:
            config = self.collect_config_settings(method)
            self.validate_and_set_config(method, config)

        return data

    def collect_config_settings(self, method):
        """
        Collects configuration settings for the specified method.

        Args:
            method (str): The method for which to collect configuration settings.

        Returns:
            dict: The configuration settings.
        """
        return {
            fname.split('_', 2)[2]: self.cleaned_data.get(fname)
            for fname in self.fields
            if fname.startswith(f'config_{method}_')
        }

    def validate_and_set_config(self, method, config):
        """
        Validates the configuration settings and sets the instance's configuration.

        Args:
            method (str): The method for which to validate and set configuration.
            config (dict): The configuration settings to validate and set.
        """
        if method == "oidc":
