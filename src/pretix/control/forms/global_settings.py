    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields = OrderedDict(
            ...
            + [
                ...
                ('send_grid_api_key', forms.CharField(required=False, label=_('Sendgrid Token'), widget=forms.TextInput(attrs={'placeholder': 'SG.xxxxxxxx'}))),
                ('smtp_host', forms.CharField(label=_('Hostname'), required=False, widget=forms.TextInput(attrs={'placeholder': 'mail.example.org'}))),
                ('smtp_port', forms.IntegerField(label=_('Port'), required=False, widget=forms.TextInput(attrs={'placeholder': 'e.g. 587, 465, 25, ...'}))),
                ('smtp_username', forms.CharField(label=_('Username'), widget=forms.TextInput(attrs={'placeholder': 'myuser@example.org'}), required=False)),
                (
                    'smtp_password',
                    forms.CharField(
                        label=_('Password'),
                ...
                ('smtp_use_tls', forms.BooleanField(label=_('Use STARTTLS'), help_text=_('Commonly enabled on port 587.'), required=False)),
...
class SSOConfigForm(SettingsForm):
    redirect_url = forms.URLField(required=True, label=_('Redirect URL'), help_text=_('e.g. {sample}').format(sample='https://app-test.eventyay.com/talk/'))
