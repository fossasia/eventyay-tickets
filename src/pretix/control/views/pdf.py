...
class BaseEditorView(EventPermissionRequiredMixin, TemplateView):
    ...
    def _get_preview_position(self):
        item = self.request.event.items.create(name=_('Sample product'), default_price=42.23, description=_('Sample product description'))
    ...
    def save_background(self, f: CachedFile):
        ...
        fname = 'pub/%s-%s/%s/%s.%s.%s' % ('event', 'settings', self.request.event.pk, self.get_layout_settings_key(), nonce, 'pdf')
            ...
            return JsonResponse(
                {
                    'status': 'ok',
                    'id': c.id,
                    'url': reverse('control:pdf.background', kwargs={'event': request.event.slug, 'organizer': request.organizer.slug, 'filename': str(c.id)}),
                }
            )
        ...
        if 'preview' in request.POST:
            with rolledback_transaction(), language(request.event.settings.locale, request.event.settings.region):
                ...
                fname, mimet, data = self.generate(
                    p,
                    override_layout=(json.loads(self.request.POST.get('data')) if self.request.POST.get('data') else None),
