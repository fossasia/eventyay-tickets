...
class GiftCardListView(OrganizerDetailViewMixin, OrganizerPermissionRequiredMixin, ListView):
    ...
    def get_queryset(self):
        s = GiftCardTransaction.objects.filter(card=OuterRef('pk')).order_by().values('card').annotate(s=Sum('value')).values('s')
        qs = self.request.organizer.issued_gift_cards.annotate(cached_value=Coalesce(Subquery(s), Decimal('0.00'))).order_by('-issuance')
    ...
    def post(self, request, *args, **kwargs):
        if 'add' in request.POST:
            ...
            if o:
                ...
                self.request.organizer.log_action('pretix.giftcards.acceptance.added', data={'issuer': o.slug}, user=request.user)
        ...
        if 'del' in request.POST:
            ...
            if o:
                ...
                self.request.organizer.log_action('pretix.giftcards.acceptance.removed', data={'issuer': o.slug}, user=request.user)
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx['other_organizers'] = self.request.user.get_organizers_with_permission('can_manage_gift_cards', self.request).exclude(pk=self.request.organizer.pk)
        return ctx
        ...
        if 'revert' in request.POST:
            ...
            if self.object.value - t.value < Decimal('0.00'):
                ...
                try:
                    ...
                    with transaction.atomic():
                        ...
                        r.save()
                        t.order.log_action('pretix.event.order.payment.failed', {'local_id': r.local_id, 'provider': r.provider, 'error': str(e)})
            ...
            try:
                ...
                if self.object.value + value < Decimal('0.00'):
                    ...
                    return redirect(reverse('control:organizer.giftcard', kwargs={'organizer': request.organizer.slug, 'giftcard': self.object.pk}))
...
class GiftCardCreateView(OrganizerDetailViewMixin, OrganizerPermissionRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        if form.cleaned_data['value']:
            form.instance.log_action('pretix.giftcards.transaction.manual', user=self.request.user, data={'value': form.cleaned_data['value']})
        return redirect(reverse('control:organizer.giftcard', kwargs={'organizer': self.request.organizer.slug, 'giftcard': self.object.pk}))
