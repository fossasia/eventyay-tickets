    ...
    def form_valid(self, form):
        ...
        if self.sform.has_changed():
            self.request.organizer.log_action(
                ...
                data={
                    k: (self.sform.cleaned_data.get(k).name if isinstance(self.sform.cleaned_data.get(k), File) else self.sform.cleaned_data.get(k))
            ...
            if any(p in self.sform.changed_data for p in SETTINGS_AFFECTING_CSS):
                change_css = True
        if form.has_changed():
            self.request.organizer.log_action('pretix.organizer.changed', user=self.request.user, data={k: form.cleaned_data.get(k) for k in form.changed_data})
...
class OrganizerDelete(AdministratorPermissionRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        try:
            ...
            messages.error(self.request, _('The organizer could not be deleted as some constraints (e.g. data created by plug-ins) do not allow it.'))
            return self.get(self.request, *self.args, **self.kwargs)
...
class OrganizerDetailViewMixin:
    def get_context_data(self, **kwargs):
        ...
        for recv, retv in nav_organizer.send(sender=self.request.organizer, request=self.request, organizer=self.request.organizer):
