    ...
    def get_queryset(self):
        qs = (
            ...
            .prefetch_related(
                ...
                Prefetch('meta_values', EventMetaValue.objects.select_related('property'), to_attr='meta_values_cached'),
            )
            .order_by('-date_from')
        )
        ...
        qs = qs.prefetch_related(
            Prefetch('quotas', queryset=Quota.objects.filter(subevent__isnull=True).annotate(s=Coalesce(F('size'), 0)).order_by('-s'), to_attr='first_quotas')
        )
...
def condition_copy(wizard):
    return not wizard.clone_from and EventWizardCopyForm.copy_from_queryset(wizard.request.user, wizard.request.session).exists()
...
class EventWizard(SafeSessionWizardView):
            ...
            if step == 'foundation':
                try:
                    ...
                    if not self.request.user.has_active_staff_session(self.request.session.session_key):
                        qs = qs.filter(id__in=self.request.user.teams.filter(can_create_events=True).values_list('organizer', flat=True))
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if 'clone' in self.request.GET:
            try:
                ...
                allow = request.user.has_event_permission(
                    clone_from.organizer, clone_from, 'can_change_event_settings', request
    ...
    def get_form_kwargs(self, step=None):
        ...
        if step != 'foundation':
            fdata = self.get_cleaned_data_for_step('foundation')
            if fdata is None:
                fdata = {'organizer': Organizer(slug='_nonexisting'), 'has_subevents': False, 'locales': ['en']}
        ...
        with transaction.atomic(), language(basics_data['locale']):
            ...
            if basics_data['tax_rate']:
                if not event.settings.tax_rate_default or event.settings.tax_rate_default.rate != basics_data['tax_rate']:
                    event.settings.tax_rate_default = event.tax_rules.create(name=LazyI18nString.from_gettext(gettext('VAT')), rate=basics_data['tax_rate'])
