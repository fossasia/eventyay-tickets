...
class ImportView(EventPermissionRequiredMixin, TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        return redirect(
            reverse('control:event.orders.import.process', kwargs={'event': request.event.slug, 'organizer': request.organizer.slug, 'file': cf.id})
        )
...
class ProcessView(EventPermissionRequiredMixin, AsyncAction, FormView):
    ...
    def get_form_kwargs(self):
        ...
        k.update({'event': self.request.event, 'initial': self.request.event.settings.order_import_settings, 'headers': self.parsed.fieldnames})
        return k

    def form_valid(self, form):
        self.request.event.settings.order_import_settings = form.cleaned_data
        return self.do(self.request.event.pk, self.file.id, form.cleaned_data, self.request.LANGUAGE_CODE, self.request.user.pk)
    ...
    def get_error_url(self):
        return reverse(
            'control:event.orders.import.process', kwargs={'event': self.request.event.slug, 'organizer': self.request.organizer.slug, 'file': self.file.id}
        )
