...
class AutoAssign(EventPermissionRequiredMixin, AsyncAction, View):
    ...
    def get_error_url(self):
        return reverse('control:event.orders.waitinglist', kwargs={'event': self.request.event.slug, 'organizer': self.request.event.organizer.slug})

    def post(self, request, *args, **kwargs):
        return self.do(self.request.event.id, self.request.user.id, self.request.POST.get('subevent'))
...
class WaitingListView(EventPermissionRequiredMixin, PaginationMixin, ListView):
        ...
        if 'assign' in request.POST:
            try:
                ...
                try:
                    ...
                    messages.success(request, _('An email containing a voucher code has been sent to the specified address.'))
                return self._redirect_back()
    ...
    def _redirect_back(self):
        ...
        return redirect(reverse('control:event.orders.waitinglist', kwargs={'event': self.request.event.slug, 'organizer': self.request.event.organizer.slug}))
    ...
    def get_queryset(self):
        ...
        if s == 's':
            ...
            qs = qs.filter(
                voucher__isnull=False, voucher__redeemed__lt=F('voucher__max_usages'), voucher__valid_until__isnull=False, voucher__valid_until__lte=now()
            )
        else:
    ...
    def get_context_data(self, **kwargs):
        ...
        for wle in ctx[self.context_object_name]:
            if (wle.item, wle.variation, wle.subevent) in itemvar_cache:
                ...
                disabled = not ev.presale_is_running or (wle.subevent and not wle.subevent.active) or not wle.item.is_available()
                if disabled:
        ...
        ctx['running'] = self.request.event.live and (self.request.event.has_subevents or self.request.event.presale_is_running)
    ...
    def _download_csv(self):
        ...
        for w in self.get_queryset():
            ...
            row = [w.name, w.email, w.phone, prod, w.created.isoformat(), status, w.voucher.code if w.voucher else '', w.locale, str(w.priority)]
            if self.request.event.has_subevents:
