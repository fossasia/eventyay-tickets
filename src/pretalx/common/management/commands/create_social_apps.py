...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--eventyay-ticket-client-id",
            type=str,
            help="Eventyay-Ticket Provider Client ID",
        )
        parser.add_argument(
            "--eventyay-ticket-secret", type=str, help="Eventyay-Ticket Provider Secret"
        )

    def handle(self, *args, **options):
        site = Site.objects.get(pk=settings.SITE_ID)
        eventyay_ticket_client_id = options.get("eventyay-ticket-client-id") or input(
            "Enter Eventyay-Ticket Provider Client ID: "
        )
        eventyay_ticket_secret = options.get("eventyay-ticket-secret") or input(
            "Enter Eventyay-Ticket Provider Secret: "
        )
