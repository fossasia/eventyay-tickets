...
class VoucherFormTest(SoupTestMixin, TransactionTestCase):
    ...
    def test_create_blocking_item_voucher_quota_full(self):
        self._create_voucher({'itemvar': '%d' % self.shirt.pk, 'block_quota': 'on'}, expected_failure=True)
    ...
    def test_create_blocking_variation_voucher_quota_free(self):
        self._create_voucher({'itemvar': '%d-%d' % (self.shirt.pk, self.shirt_red.pk), 'block_quota': 'on'})
        with scopes_disabled():
            v = Voucher.objects.latest('pk')
        assert v.block_quota

    def test_create_short_code(self):
        self._create_voucher({'itemvar': '%d-%d' % (self.shirt.pk, self.shirt_red.pk), 'code': 'ABC'}, expected_failure=True)

    def test_create_blocking_variation_voucher_quota_full(self):
        self.quota_shirts.size = 0
        self.quota_shirts.save()
        self._create_voucher({'itemvar': '%d-%d' % (self.shirt.pk, self.shirt_red.pk), 'block_quota': 'on'}, expected_failure=True)
    ...
    def test_create_blocking_quota_voucher_quota_full(self):
        ...
        self._create_voucher({'itemvar': 'q-%d' % self.quota_tickets.pk, 'block_quota': 'on'}, expected_failure=True)
    ...
    def test_change_voucher_validity_to_valid_quota_full(self):
        ...
        self.quota_tickets.save()
        with scopes_disabled():
            v = self.event.vouchers.create(item=self.ticket, valid_until=now() - datetime.timedelta(days=3), block_quota=True)
    ...
    def test_create_blocking_bulk_quota_full(self):
        self.quota_tickets.size = 0
        self.quota_tickets.save()
        self._create_bulk_vouchers({'codes': 'ABCDE\nDEFGH', 'itemvar': '%d' % self.ticket.pk, 'block_quota': 'on'}, expected_failure=True)

    def test_create_blocking_bulk_quota_free(self):
        self.quota_tickets.size = 5
        self.quota_tickets.save()
        self._create_bulk_vouchers({'codes': 'ABCDE\nDEFGH', 'itemvar': '%d' % self.ticket.pk, 'block_quota': 'on'})
    ...
    def test_delete_voucher(self):
        with scopes_disabled():
            v = self.event.vouchers.create(quota=self.quota_tickets)
        doc = self.get_doc('/control/event/%s/%s/vouchers/%s/delete' % (self.orga.slug, self.event.slug, v.pk), follow=True)
        assert not doc.select('.alert-danger')

        doc = self.post_doc('/control/event/%s/%s/vouchers/%s/delete' % (self.orga.slug, self.event.slug, v.pk), {}, follow=True)
        assert doc.select('.alert-success')
        with scopes_disabled():
    ...
    def test_subevent_required_for_blocking(self):
        self.event.has_subevents = True
        self.event.save()
        self._create_voucher({'itemvar': '%d' % self.ticket.pk, 'block_quota': 'on'}, expected_failure=True)
    ...
    def test_subevent_blocking_quota_full(self):
        ...
        self._create_voucher({'itemvar': '%d' % self.ticket.pk, 'block_quota': 'on', 'subevent': se1.pk}, expected_failure=True)

    def test_order_warning_deduplication(self):
        with scopes_disabled():
            shirt_voucher = Voucher.objects.create(event=self.event, item=self.shirt, price_mode='set', value=0.0, max_usages=100)
            ...
            OrderPosition.objects.create(order=shirt_order, item=self.shirt, variation=self.shirt_red, price=decimal.Decimal('0'), voucher=shirt_voucher)

            OrderPosition.objects.create(order=shirt_order, item=self.shirt, variation=self.shirt_blue, price=decimal.Decimal('0'), voucher=shirt_voucher)
