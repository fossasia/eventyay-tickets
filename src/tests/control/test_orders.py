...
def env():
    ...
    event = Event.objects.create(
        organizer=o, name='Dummy', slug='dummy', date_from=now(), plugins='pretix.plugins.banktransfer,pretix.plugins.stripe,tests.testdummy'
    )
    ...
    o.payments.create(amount=o.total, provider='banktransfer', state=OrderPayment.PAYMENT_STATE_PENDING)
    ticket = Item.objects.create(event=event, name='Early-bird ticket', category=None, default_price=23, admission=True)
    event.settings.set('attendee_names_asked', True)
    event.settings.set('locales', ['en', 'de'])
    OrderPosition.objects.create(order=o, item=ticket, variation=None, price=Decimal('14'), attendee_name_parts={'full_name': 'Peter', '_scheme': 'full'})
    OrderPosition.objects.create(
        order=o, item=ticket, variation=None, price=Decimal('14'), canceled=True, attendee_name_parts={'full_name': 'Lukas Gelöscht', '_scheme': 'full'}
    )
...
def test_order_list(client, env):
    with scopes_disabled():
        otherticket = Item.objects.create(event=env[0], name='Early-bird ticket', category=None, default_price=23, admission=True)
...
def test_order_transition_to_paid_in_time_success(client, env):
    ...
    client.login(email='dummy@dummy.dummy', password='dummy')
    client.post(
        '/control/event/dummy/dummy/orders/FOO/transition', {'amount': str(env[2].pending_sum), 'payment_date': now().date().isoformat(), 'status': 'p'}
    )
    with scopes_disabled():
        o = Order.objects.get(id=env[2].id)
...
def test_order_transition(client, env, process):
    ...
    client.post(
        '/control/event/dummy/dummy/orders/FOO/transition', {'amount': str(o.pending_sum), 'payment_date': now().date().isoformat(), 'status': process[1]}
    )
    with scopes_disabled():
        o = Order.objects.get(id=env[2].id)
    if process[2]:
...
def test_order_cancel_paid_keep_fee(client, env):
    ...
    client.login(email='dummy@dummy.dummy', password='dummy')
    client.get('/control/event/dummy/dummy/orders/FOO/transition?status=c')
    client.post('/control/event/dummy/dummy/orders/FOO/transition', {'status': 'c', 'cancellation_fee': '6.00'})
    with scopes_disabled():
        o = Order.objects.get(id=env[2].id)
...
def test_order_extend_not_expired(client, env):
    ...
    client.login(email='dummy@dummy.dummy', password='dummy')
    response = client.post('/control/event/dummy/dummy/orders/FOO/extend', {'expires': newdate}, follow=True)
    assert 'alert-success' in response.content.decode()
    with scopes_disabled():
        o = Order.objects.get(id=env[2].id)
...
def test_order_extend_expired_quota_empty_ignore(client, env):
    ...
    newdate = (now() + timedelta(days=20)).strftime('%Y-%m-%d')
    client.login(email='dummy@dummy.dummy', password='dummy')
    response = client.post('/control/event/dummy/dummy/orders/FOO/extend', {'expires': newdate, 'quota_ignore': 'on'}, follow=True)
    assert b'alert-success' in response.content
    with scopes_disabled():
        o = Order.objects.get(id=env[2].id)
    assert o.status == Order.STATUS_PENDING
...
def test_order_extend_expired_seat_taken(client, env):
    with scopes_disabled():
        ...
        OrderPosition.objects.create(
            order=o, item=env[3], variation=None, price=Decimal('14'), attendee_name_parts={'full_name': 'Peter', '_scheme': 'full'}, seat=seat_a1
        )
...
def test_order_extend_expired_quota_partial(client, env):
    with scopes_disabled():
        o = Order.objects.get(id=env[2].id)
        OrderPosition.objects.create(order=o, item=env[3], variation=None, price=Decimal('14'), attendee_name_parts={'full_name': 'Peter', '_scheme': 'full'})
        o.expires = now() - timedelta(days=5)
        o.status = Order.STATUS_EXPIRED
        olddate = o.expires
        o.save()
...
def test_order_extend_expired_voucher_budget_ok(client, env):
    with scopes_disabled():
        ...
        o.status = Order.STATUS_EXPIRED
        o.save()
        v = env[0].vouchers.create(code='foo', price_mode='subtract', value=Decimal('1.50'), budget=Decimal('1.50'))
        p = o.positions.first()
        p.voucher = v
...
def test_order_extend_expired_voucher_budget_fail(client, env):
    with scopes_disabled():
        ...
        olddate = o.expires
        o.save()
        v = env[0].vouchers.create(code='foo', price_mode='subtract', value=Decimal('1.50'), budget=Decimal('0.00'))
        p = o.positions.first()
        p.voucher = v
        p.price_before_voucher = p.price
        p.price -= Decimal('1.50')
...
def test_order_mark_paid_blocked(client, env):
    ...
    client.login(email='dummy@dummy.dummy', password='dummy')
    response = client.post(
        '/control/event/dummy/dummy/orders/FOO/transition', {'amount': str(o.pending_sum), 'payment_date': now().date().isoformat(), 'status': 'p'}, follow=True
    )
    assert 'alert-danger' in response.content.decode()
    with scopes_disabled():
        o = Order.objects.get(id=env[2].id)
...
def test_order_mark_paid_overpaid_expired(client, env):
    ...
    response = client.post(
        '/control/event/dummy/dummy/orders/FOO/transition',
        {'status': 'p', 'payment_date': now().date().isoformat(), 'amount': '0.00', 'force': 'on'},
        follow=True,
    )
    assert 'alert-success' in response.content.decode()
    with scopes_disabled():
...
def test_order_mark_paid_forced(client, env):
    ...
    response = client.post(
        '/control/event/dummy/dummy/orders/FOO/transition',
        {'status': 'p', 'payment_date': now().date().isoformat(), 'amount': str(o.pending_sum), 'force': 'on'},
        follow=True,
    )
    print(response.content.decode())
    assert 'alert-success' in response.content.decode()
...
def order_url(env):
    event = env[0]
    order = env[2]
    url = '/control/event/{orga}/{event}/orders/{code}'.format(event=event.slug, orga=event.organizer.slug, code=order.code)
    return url
...
def test_order_sendmail_simple_case(client, order_url, env):
    ...
    sendmail_url = order_url + '/sendmail'
    mail.outbox = []
    response = client.post(sendmail_url, {'sendto': order.email, 'subject': 'Test subject', 'message': 'This is a test file for sending mails.'}, follow=True)
...
def test_order_sendmail_preview(client, order_url, env):
    ...
    mail.outbox = []
    response = client.post(
        sendmail_url, {'sendto': order.email, 'subject': 'Test subject', 'message': 'This is a test file for sending mails.', 'action': 'preview'}, follow=True
    )
...
class OrderChangeTests(SoupTest):
    ...
    def setUp(self):
        ...
        self.ticket = Item.objects.create(event=self.event, name='Early-bird ticket', tax_rule=self.tr7, default_price=Decimal('23.00'), admission=True)
        self.shirt = Item.objects.create(event=self.event, name='T-Shirt', tax_rule=self.tr19, default_price=Decimal('12.00'))
        self.op1 = OrderPosition.objects.create(
            order=self.order, item=self.ticket, variation=None, price=Decimal('23.00'), attendee_name_parts={'full_name': 'Peter', '_scheme': 'full'}
        )
        self.op2 = OrderPosition.objects.create(
            order=self.order, item=self.ticket, variation=None, price=Decimal('23.00'), attendee_name_parts={'full_name': 'Dieter', '_scheme': 'full'}
        )
    ...
    def test_do_not_show_canceled(self):
        r = self.client.get('/control/event/{}/{}/orders/{}/change'.format(self.event.organizer.slug, self.event.slug, self.order.code))
    ...
    def test_recalculate_reverse_charge(self):
        ...
        self.tr19.save()
        with scopes_disabled():
            InvoiceAddress.objects.create(order=self.order, is_business=True, vat_id='ATU1234567', vat_id_validated=True, country=Country('AT'))
...
def test_process_refund_overpaid_externally(client, env):
    with scopes_disabled():
        ...
        env[2].payments.create(state='confirmed', provider='stripe', amount=Decimal('14.00'), payment_date=now())
...
def test_process_refund_mark_refunded(client, env):
    ...
    client.login(email='dummy@dummy.dummy', password='dummy')
    response = client.post('/control/event/dummy/dummy/orders/FOO/refunds/{}/process'.format(r.pk), {'action': 'r'}, follow=True)
    assert 'alert-success' in response.content.decode()
    r.refresh_from_db()
    assert r.state == OrderRefund.REFUND_STATE_DONE
    env[2].refresh_from_db()
...
def test_refund_paid_order_fully_mark_as_refunded(client, env):
    ...
    client.post(
        '/control/event/dummy/dummy/orders/FOO/refund', {'start-partial_amount': '14.00', 'start-mode': 'full', 'start-action': 'mark_refunded'}, follow=True
    )
    client.post(
...
def test_refund_paid_order_partially_mark_as_pending(client, env):
    ...
    assert doc.select('input[name$=partial_amount]')[0]['value'] == '14.00'
    client.post(
        '/control/event/dummy/dummy/orders/FOO/refund', {'start-partial_amount': '7.00', 'start-mode': 'partial', 'start-action': 'mark_pending'}, follow=True
    )
    client.post(
        '/control/event/dummy/dummy/orders/FOO/refund',
        {
...
def test_refund_propose_lower_payment(client, env):
    with scopes_disabled():
        ...
        p.confirm()
        env[2].payments.create(amount=Decimal('6.00'), provider='stripe', state=OrderPayment.PAYMENT_STATE_CONFIRMED)
    client.login(email='dummy@dummy.dummy', password='dummy')
...
def test_refund_propose_equal_payment(client, env):
    with scopes_disabled():
        ...
        p.confirm()
        env[2].payments.create(amount=Decimal('7.00'), provider='stripe', state=OrderPayment.PAYMENT_STATE_CONFIRMED)
    client.login(email='dummy@dummy.dummy', password='dummy')
    client.get('/control/event/dummy/dummy/orders/FOO/refund')
    response = client.post(
        '/control/event/dummy/dummy/orders/FOO/refund', {'start-partial_amount': '7.00', 'start-mode': 'partial', 'start-action': 'mark_pending'}, follow=True
...
def test_refund_propose_higher_payment(client, env):
    with scopes_disabled():
        ...
        p.confirm()
        env[2].payments.create(amount=Decimal('8.00'), provider='stripe', state=OrderPayment.PAYMENT_STATE_CONFIRMED)
    client.login(email='dummy@dummy.dummy', password='dummy')
    client.get('/control/event/dummy/dummy/orders/FOO/refund')
    response = client.post(
        '/control/event/dummy/dummy/orders/FOO/refund', {'start-partial_amount': '7.00', 'start-mode': 'partial', 'start-action': 'mark_pending'}, follow=True
...
def test_delete_cancellation_request(client, env):
    ...
    client.login(email='dummy@dummy.dummy', password='dummy')
    response = client.post('/control/event/dummy/dummy/orders/FOO/cancellationrequests/{}/delete'.format(r.pk), {}, follow=True)
    assert 'alert-success' in response.content.decode()
...
def test_approve_cancellation_request(client, env):
    ...
    response = client.post('/control/event/dummy/dummy/orders/FOO/transition?req={}'.format(r.pk), {'status': 'c', 'cancellation_fee': '4.00'}, follow=True)
    doc = BeautifulSoup(response.content.decode(), 'lxml')
