...
class SubEventsTest(SoupTest):
    ...
    def test_delete_bulk(self):
        ...
        doc = self.post_doc(
            ...
            {'subevent': [str(self.subevent1.pk), str(self.subevent2.pk)], 'action': 'delete_confirm'},
            follow=True,
        )
        assert doc.select('.alert-success')
        with scopes_disabled():
    ...
    def test_delete_bulk_by_query(self):
        doc = self.post_doc('/control/event/ccc/30c3/subevents/bulk_action', {'__ALL': 'on', 'filter-query': 'SE2', 'action': 'delete_confirm'}, follow=True)
        assert doc.select('.alert-success')
        with scopes_disabled():
            assert not self.event1.subevents.filter(pk=self.subevent2.pk).exists()
            assert self.event1.subevents.filter(pk=self.subevent1.pk).exists()
    ...
    def test_disable_bulk(self):
        self.subevent2.active = True
        self.subevent2.save()
        doc = self.post_doc('/control/event/ccc/30c3/subevents/bulk_action', {'subevent': str(self.subevent2.pk), 'action': 'disable'}, follow=True)
        assert doc.select('.alert-success')
        with scopes_disabled():
    ...
    def test_enable_bulk(self):
        ...
        self.subevent2.save()
        doc = self.post_doc('/control/event/ccc/30c3/subevents/bulk_action', {'subevent': str(self.subevent2.pk), 'action': 'enable'}, follow=True)
        assert doc.select('.alert-success')
        with scopes_disabled():
