...
class TaxRateFormTest(SoupTest):
    ...
    def test_update(self):
        ...
        doc = self.post_doc('/control/event/%s/%s/settings/tax/%s/' % (self.orga1.slug, self.event1.slug, tr.id), form_data)
        assert doc.select('.alert-success')
        tr.refresh_from_db()
    ...
    def test_delete(self):
        ...
        form_data = extract_form_fields(doc.select('.container-fluid form')[0])
        doc = self.post_doc('/control/event/%s/%s/settings/tax/%s/delete' % (self.orga1.slug, self.event1.slug, tr.id), form_data)
        assert doc.select('.alert-success')
