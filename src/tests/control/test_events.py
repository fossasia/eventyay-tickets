...
class EventsTest(SoupTest):
    ...
    def setUp(self):
        ...
        self.team1 = Team.objects.create(organizer=self.orga1, can_create_events=True, can_change_event_settings=True, can_change_items=True)
        self.team1.members.add(self.user)
        self.team1.limit_events.add(self.event1)

        self.team2 = Team.objects.create(
            organizer=self.orga1, can_change_event_settings=True, can_change_items=True, can_change_orders=True, can_change_vouchers=True
        )
    ...
    def test_quick_setup_total_quota(self):
        ...
        doc = self.post_doc(
            '/control/event/%s/%s/quickstart/' % (self.orga1.slug, self.event1.slug), extract_form_fields(doc.select('.container-fluid form')[0])
        )
    ...
    def test_settings(self):
        ...
        doc = self.post_doc('/control/event/%s/%s/settings/' % (self.orga1.slug, self.event1.slug), extract_form_fields(doc.select('.container-fluid form')[0]))
        assert len(doc.select('.alert-success')) > 0
    ...
    def test_testmode_enable(self):
        ...
        self.event1.save()
        self.post_doc('/control/event/%s/%s/live/' % (self.orga1.slug, self.event1.slug), {'testmode': 'true'})
        self.event1.refresh_from_db()
    ...
    def test_testmode_disable(self):
        with scopes_disabled():
            ...
            self.event1.save()
        self.post_doc('/control/event/%s/%s/live/' % (self.orga1.slug, self.event1.slug), {'testmode': 'false'})
        self.event1.refresh_from_db()
    ...
    def test_testmode_disable_delete(self):
        with scopes_disabled():
            ...
            self.event1.testmode = True
            self.event1.save()
        self.post_doc('/control/event/%s/%s/live/' % (self.orga1.slug, self.event1.slug), {'testmode': 'false', 'delete': 'yes'})
        self.event1.refresh_from_db()
        assert not self.event1.testmode
        with scopes_disabled():
    ...
    def test_live_disable(self):
        ...
        self.event1.save()
        self.post_doc('/control/event/%s/%s/live/' % (self.orga1.slug, self.event1.slug), {'live': 'false'})
        self.event1.refresh_from_db()
    ...
    def test_live_ok(self):
        ...
        self.post_doc('/control/event/%s/%s/live/' % (self.orga1.slug, self.event1.slug), {'live': 'true'})
        self.event1.refresh_from_db()
    ...
    def test_invoice_settings(self):
        ...
        doc = self.post_doc('/control/event/%s/%s/settings/invoice' % (self.orga1.slug, self.event1.slug), data, follow=True)
    ...
    def test_display_settings(self):
        with mocker_context() as mocker:
            ...
            doc = self.post_doc('/control/event/%s/%s/settings/' % (self.orga1.slug, self.event1.slug), data, follow=True)
            assert doc.select('.alert-success')
            self.event1.settings.flush()
    ...
    def test_email_settings(self):
        with mocker_context() as mocker:
            ...
            doc = self.post_doc('/control/event/%s/%s/settings/email' % (self.orga1.slug, self.event1.slug), data, follow=True)
            assert doc.select('.alert-success')
            self.event1.settings.flush()
    ...
    def test_ticket_settings(self):
        ...
        doc = self.post_doc('/control/event/%s/%s/settings/tickets' % (self.orga1.slug, self.event1.slug), data, follow=True)
        self.event1.settings.flush()
    ...
    def test_create_event_success(self):
        ...
        self.post_doc('/control/events/add', {'event_wizard-current_step': 'copy', 'event_wizard-prefix': 'event_wizard', 'copy-copy_from_event': ''})
    ...
    def test_create_event_copy_success(self):
        with scopes_disabled():
            ...
            self.event1.items.create(name='Early-bird ticket', category=None, default_price=23, tax_rule=tr, admission=True, hidden_if_available=q1)
        ...
        self.post_doc(
            '/control/events/add', {'event_wizard-current_step': 'copy', 'event_wizard-prefix': 'event_wizard', 'copy-copy_from_event': self.event1.pk}
        )
    ...
    def test_create_event_clone_success(self):
        with scopes_disabled():
            tr = self.event1.tax_rules.create(rate=19, name='VAT')
            self.event1.items.create(name='Early-bird ticket', category=None, default_price=23, tax_rule=tr, admission=True)
        self.event1.settings.tax_rate_default = tr
...
class EventDeletionTest(SoupTest):
    ...
    def setUp(self):
        ...
        t = Team.objects.create(organizer=self.orga1, can_create_events=True, can_change_event_settings=True, can_change_items=True)
        t.members.add(self.user)
        t.limit_events.add(self.event1)
        self.ticket = self.event1.items.create(name='Early-bird ticket', category=None, default_price=23, admission=True)
    ...
    def test_delete_orders(self):
        Order.objects.create(
            code='FOO', event=self.event1, email='dummy@dummy.test', status=Order.STATUS_PENDING, datetime=now(), expires=now(), total=14, locale='en'
        )
