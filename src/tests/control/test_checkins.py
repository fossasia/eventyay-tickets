...
def dashboard_env():
    ...
    order_paid = Order.objects.create(
        code='FOO', event=event, email='dummy@dummy.test', status=Order.STATUS_PAID, datetime=now(), expires=now() + timedelta(days=10), total=33, locale='en'
    )
    OrderPosition.objects.create(order=order_paid, item=item_ticket, variation=None, price=Decimal('23'), attendee_name_parts={'full_name': 'Peter'})
...
def test_dashboard_pending_not_count(dashboard_env):
    ...
    OrderPosition.objects.create(order=order_pending, item=dashboard_env[4], variation=None, price=Decimal('23'), attendee_name_parts={'full_name': 'NotPaid'})
    assert '0/2' in c[0]['content']
...
def checkin_list_env():
    ...
    # event
    event = Event.objects.create(organizer=orga, name='Dummy', slug='dummy', date_from=now(), plugins='pretix.plugins.banktransfer,tests.testdummy')
    event.settings.set('ticketoutput_testdummy__enabled', True)
    event.settings.set('attendee_names_asked', True)
    event.settings.set('locales', ['en', 'de'])
    ...
    order_a1 = Order.objects.create(
        code='A1', event=event, email='a1dummy@dummy.test', status=Order.STATUS_PAID, datetime=now(), expires=now() + timedelta(days=10), total=33, locale='en'
    )
    order_a2 = Order.objects.create(
        code='A2', event=event, email='a2dummy@dummy.test', status=Order.STATUS_PAID, datetime=now(), expires=now() + timedelta(days=10), total=23, locale='en'
    )
    order_a3 = Order.objects.create(
        code='A3', event=event, email='a3dummy@dummy.test', status=Order.STATUS_PAID, datetime=now(), expires=now() + timedelta(days=10), total=23, locale='en'
    )

    # order position
    op_pending_ticket = OrderPosition.objects.create(
        order=order_pending, item=item_ticket, variation=None, price=Decimal('23'), attendee_name_parts={'full_name': 'Pending'}
    )
    op_a1_ticket = OrderPosition.objects.create(order=order_a1, item=item_ticket, variation=None, price=Decimal('23'), attendee_name_parts={'full_name': 'A1'})
    op_a1_mascot = OrderPosition.objects.create(order=order_a1, item=item_mascot, variation=None, price=Decimal('10'))
    op_a2_ticket = OrderPosition.objects.create(order=order_a2, item=item_ticket, variation=None, price=Decimal('23'), attendee_name_parts={'full_name': 'A2'})
...
def test_manual_checkins(client, checkin_list_env):
    ...
    client.post('/control/event/dummy/dummy/checkinlists/{}/'.format(checkin_list_env[6].pk), {'checkin': [checkin_list_env[5][3].pk]})
    with scopes_disabled():
        assert checkin_list_env[5][3].checkins.exists()
    assert LogEntry.objects.filter(action_type='pretix.event.checkin', object_id=checkin_list_env[5][3].order.pk).exists()
...
def test_manual_checkins_revert(client, checkin_list_env):
    ...
    with scopes_disabled():
        assert not checkin_list_env[5][3].checkins.exists()
    client.post('/control/event/dummy/dummy/checkinlists/{}/'.format(checkin_list_env[6].pk), {'checkin': [checkin_list_env[5][3].pk]})
    client.post('/control/event/dummy/dummy/checkinlists/{}/'.format(checkin_list_env[6].pk), {'checkin': [checkin_list_env[5][3].pk], 'revert': 'true'})
    with scopes_disabled():
        assert not checkin_list_env[5][3].checkins.exists()
    assert LogEntry.objects.filter(action_type='pretix.event.checkin', object_id=checkin_list_env[5][3].order.pk).exists()
    assert LogEntry.objects.filter(action_type='pretix.event.checkin.reverted', object_id=checkin_list_env[5][3].order.pk).exists()
...
def checkin_list_with_addon_env():
    ...
    item_workshop = Item.objects.create(event=event, name='Workshop', default_price=10, admission=False, category=cat_workshop)
    ...
    op_a1_ticket = OrderPosition.objects.create(order=order_a1, item=item_ticket, variation=None, price=Decimal('23'), attendee_name_parts={'full_name': 'A1'})
    op_a1_workshop = OrderPosition.objects.create(order=order_a1, item=item_workshop, variation=None, price=Decimal('10'), addon_to=op_a1_ticket)
    op_a2_ticket = OrderPosition.objects.create(order=order_a2, item=item_ticket, variation=None, price=Decimal('23'), attendee_name_parts={'full_name': 'A2'})
...
def test_checkins_attendee_name_from_addon_available(client, checkin_list_with_addon_env):
    ...
    qs = response.context['entries']
    item_keys = [q.order.code + str(q.item.name) + (str(q.addon_to.attendee_name) if q.addon_to is not None else str(q.attendee_name)) for q in qs]
...
class CheckinListFormTest(SoupTest):
    ...
    def test_update(self):
        ...
        doc = self.post_doc('/control/event/%s/%s/checkinlists/%s/change' % (self.orga1.slug, self.event1.slug, cl.id), form_data)
        assert doc.select('.alert-success')
        cl.refresh_from_db()
    ...
    def test_delete(self):
        ...
        form_data = extract_form_fields(doc.select('.container-fluid form')[0])
        doc = self.post_doc('/control/event/%s/%s/checkinlists/%s/delete' % (self.orga1.slug, self.event1.slug, cl.id), form_data)
        assert doc.select('.alert-success')
