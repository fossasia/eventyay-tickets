...
class EventShredderTest(SoupTest):
    ...
    def setUp(self):
        ...
        t = Team.objects.create(organizer=self.orga1, can_create_events=True, can_change_event_settings=True, can_change_items=True, can_change_orders=True)
    ...
    def test_shred_simple(self):
        ...
        doc = self.post_doc('/control/event/%s/%s/shredder/export' % (self.orga1.slug, self.event1.slug), {'shredder': 'order_emails'})
        ...
        doc = self.post_doc(
            ...
            {'confirm_code': indexdata['confirm_code'], 'file': doc.select('input[name=file]')[0].attrs['value'], 'slug': self.event1.slug},
        )
    ...
    def test_shred_password_wrong(self):
        ...
        doc = self.post_doc(
            '/control/event/%s/%s/shredder/shred' % (self.orga1.slug, self.event1.slug),
            {'confirm_code': indexdata['confirm_code'], 'file': doc.select('input[name=file]')[0].attrs['value'], 'password': 'test'},
        )
    ...
    def test_shred_confirm_code_wrong(self):
        ...
        doc = self.post_doc(
            '/control/event/%s/%s/shredder/shred' % (self.orga1.slug, self.event1.slug),
            {'confirm_code': indexdata['confirm_code'][::-1] + 'A', 'file': doc.select('input[name=file]')[0].attrs['value'], 'password': 'dummy'},
        )
        assert doc.select('.alert-danger')
        self.order.refresh_from_db()
        assert self.order.email
    ...
    def test_shred_something_happened(self):
        ...
        doc = self.post_doc(
            '/control/event/%s/%s/shredder/shred' % (self.orga1.slug, self.event1.slug),
            {'confirm_code': indexdata['confirm_code'], 'file': doc.select('input[name=file]')[0].attrs['value'], 'password': 'dummy'},
        )
        assert doc.select('.alert-danger')
        self.order.refresh_from_db()
        assert self.order.email
