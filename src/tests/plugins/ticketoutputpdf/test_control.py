...
class TicketLayoutFormTest(SoupTest):
    ...
    def test_delete(self):
        ...
        form_data = extract_form_fields(doc.select('.container-fluid form')[0])
        doc = self.post_doc('/control/event/%s/%s/pdfoutput/%s/delete' % (self.orga1.slug, self.event1.slug, bl1.id), form_data)
        assert doc.select('.alert-success')
    ...
    def test_item_copy(self):
        ...
        self.client.post(
            ...
            {'name_0': 'Intermediate', 'default_price': '23.00', 'tax_rate': '19.00', 'copy_from': str(self.item1.pk), 'has_variations': '1'},
        )
        with scopes_disabled():
