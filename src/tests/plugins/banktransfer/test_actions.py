...
def test_discard(env, client):
    ...
    trans = BankTransaction.objects.create(event=env[0], import_job=job, payer='Foo', state=BankTransaction.STATE_ERROR, amount=0, date='unknown')
...
def test_assign_order(env, client):
    job = BankImportJob.objects.create(event=env[0])
    trans = BankTransaction.objects.create(event=env[0], import_job=job, payer='Foo', state=BankTransaction.STATE_NOMATCH, amount=23, date='unknown')
    client.login(email='dummy@dummy.dummy', password='dummy')
    r = json.loads(
        client.post(
            '/control/event/{}/{}/banktransfer/action/'.format(env[0].organizer.slug, env[0].slug),
...
def test_assign_order_unknown(env, client):
    ...
    r = json.loads(
        client.post(
            '/control/event/{}/{}/banktransfer/action/'.format(env[0].organizer.slug, env[0].slug), {'action_{}'.format(trans.pk): 'assign:FOO'}
        ).content.decode('utf-8')
    )
...
def test_assign_order_amount_incorrect(env, client):
    job = BankImportJob.objects.create(event=env[0])
    trans = BankTransaction.objects.create(event=env[0], import_job=job, payer='Foo', state=BankTransaction.STATE_NOMATCH, amount=12, date='unknown')
    client.login(email='dummy@dummy.dummy', password='dummy')
    r = json.loads(
        client.post(
            '/control/event/{}/{}/banktransfer/action/'.format(env[0].organizer.slug, env[0].slug),
...
def test_comment(env, client):
    ...
    r = json.loads(
        client.post(
            '/control/event/{}/{}/banktransfer/action/'.format(env[0].organizer.slug, env[0].slug), {'action_{}'.format(trans.pk): 'comment:This is my comment'}
        ).content.decode('utf-8')
    )
    assert r['status'] == 'ok'
    trans.refresh_from_db()
...
def test_retry_success(env, client):
    ...
    trans = BankTransaction.objects.create(
        event=env[0], import_job=job, payer='Foo', state=BankTransaction.STATE_DUPLICATE, amount=23, date='unknown', order=env[3]
    )
    client.login(email='dummy@dummy.dummy', password='dummy')
...
def test_retry_canceled(env, client):
    job = BankImportJob.objects.create(event=env[0])
    trans = BankTransaction.objects.create(
        event=env[0], import_job=job, payer='Foo', state=BankTransaction.STATE_ERROR, amount=23, date='unknown', order=env[3]
    )
    client.login(email='dummy@dummy.dummy', password='dummy')
    r = json.loads(
        client.post(
...
def test_assign_order_organizer(env, client):
    ...
    trans = BankTransaction.objects.create(
        organizer=env[0].organizer, import_job=job, payer='Foo', state=BankTransaction.STATE_NOMATCH, amount=23, date='unknown'
    )
    client.login(email='dummy@dummy.dummy', password='dummy')
    r = json.loads(
        client.post(
