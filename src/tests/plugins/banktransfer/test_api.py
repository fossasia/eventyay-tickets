...
def env():
    ...
    o1 = Order.objects.create(code='1Z3AS', event=event, status=Order.STATUS_PENDING, datetime=now(), expires=now() + timedelta(days=10), total=23)
...
def test_api_list(env, client):
    ...
    with mock.patch('django.utils.timezone.now') as mock_now:
        ...
        BankTransaction.objects.create(event=env[0], import_job=job, payer='Foo', state=BankTransaction.STATE_ERROR, amount=0, date='unknown')
    res = copy.copy(RES_JOB)
    res['id'] = job.pk
    res['created'] = testtime.isoformat().replace('+00:00', 'Z')
    client.login(email='dummy@dummy.dummy', password='dummy')
    r = json.loads(client.get('/api/v1/organizers/{}/bankimportjobs/'.format(env[0].organizer.slug)).content.decode('utf-8'))
...
def test_api_detail(env, client):
    ...
    res['created'] = testtime.isoformat().replace('+00:00', 'Z')
    client.login(email='dummy@dummy.dummy', password='dummy')
    r = json.loads(client.get('/api/v1/organizers/{}/bankimportjobs/{}/'.format(env[0].organizer.slug, job.pk)).content.decode('utf-8'))
    assert r == res
