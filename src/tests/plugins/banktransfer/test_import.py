...
def env():
    ...
    o1 = Order.objects.create(
        code='1Z3AS', event=event, email='admin@localhost', status=Order.STATUS_PENDING, datetime=now(), expires=now() + timedelta(days=10), total=23
    )
    o2 = Order.objects.create(code='6789Z', event=event, status=Order.STATUS_CANCELED, datetime=now(), expires=now() + timedelta(days=10), total=23)
    Order.objects.create(code='GS89Z', event=event, status=Order.STATUS_CANCELED, datetime=now(), expires=now() + timedelta(days=10), total=23)
...
def test_mark_paid(env, job):
    djmail.outbox = []
    process_banktransfers(job, [{'payer': 'Karla Kundin', 'reference': 'Bestellung DUMMY1234S', 'date': '2016-01-26', 'amount': '23.00'}])
...
def test_underpaid(env, job):
    djmail.outbox = []
    process_banktransfers(job, [{'payer': 'Karla Kundin', 'reference': 'Bestellung DUMMY1Z3AS', 'date': '2016-01-26', 'amount': '22.50'}])
    env[2].refresh_from_db()
...
def test_in_parts(env, job):
    process_banktransfers(job, [{'payer': 'Karla Kundin', 'reference': 'Bestellung DUMMY1Z3AS', 'date': '2016-01-26', 'amount': '10.00'}])
    process_banktransfers(job, [{'payer': 'Karla Kundin', 'reference': 'Bestellung DUMMY1Z3AS', 'date': '2016-01-26', 'amount': '13.00'}])
    env[2].refresh_from_db()
    assert env[2].status == Order.STATUS_PAID
    with scopes_disabled():
...
def test_overpaid(env, job):
    process_banktransfers(job, [{'payer': 'Karla Kundin', 'reference': 'Bestellung DUMMY1Z3AS', 'date': '2016-01-26', 'amount': '23.50'}])
    env[2].refresh_from_db()
    assert env[2].status == Order.STATUS_PAID
    with scopes_disabled():
        p = env[2].payments.last()
...
def test_ignore_canceled(env, job):
    process_banktransfers(job, [{'payer': 'Karla Kundin', 'reference': 'Bestellung DUMMY6789Z', 'date': '2016-01-26', 'amount': '23.00'}])
...
def test_mark_paid_organizer(env, orga_job):
    process_banktransfers(orga_job, [{'payer': 'Karla Kundin', 'reference': 'Bestellung DUMMY-1234S', 'date': '2016-01-26', 'amount': '23.00'}])
    env[2].refresh_from_db()
    assert env[2].status == Order.STATUS_PAID
...
def test_mark_paid_double_reference(env, orga_job):
    process_banktransfers(orga_job, [{'payer': 'Karla Kundin', 'reference': 'Bestellung DUMMY-1234S DUMMY-1234S', 'date': '2016-01-26', 'amount': '23.00'}])
    env[2].refresh_from_db()
    assert env[2].status == Order.STATUS_PAID
...
def test_mark_paid_organizer_dash_in_slug(env, orga_job):
    env[0].slug = 'foo-bar'
    env[0].save()
    process_banktransfers(orga_job, [{'payer': 'Karla Kundin', 'reference': 'Bestellung FOO-BAR-1234S', 'date': '2016-01-26', 'amount': '23.00'}])
    env[2].refresh_from_db()
    assert env[2].status == Order.STATUS_PAID
...
def test_mark_paid_organizer_varying_order_code_length(env, orga_job):
    ...
    env[2].save()
    process_banktransfers(orga_job, [{'payer': 'Karla Kundin', 'reference': 'Bestellung DUMMY-123412341234', 'date': '2016-01-26', 'amount': '23.00'}])
    env[2].refresh_from_db()
    assert env[2].status == Order.STATUS_PAID
...
def test_mark_paid_organizer_weird_slug(env, orga_job):
    env[0].slug = 'du.m-y'
    env[0].save()
    process_banktransfers(orga_job, [{'payer': 'Karla Kundin', 'reference': 'Bestellung DU.M-Y-1234S', 'date': '2016-01-26', 'amount': '23.00'}])
    env[2].refresh_from_db()
    assert env[2].status == Order.STATUS_PAID
...
def test_wrong_event_organizer(env, orga_job):
    Event.objects.create(organizer=env[0].organizer, name='Wrong', slug='wrong', date_from=now(), plugins='pretix.plugins.banktransfer')
    process_banktransfers(orga_job, [{'payer': 'Karla Kundin', 'reference': 'Bestellung WRONG-1234S', 'date': '2016-01-26', 'amount': '23.00'}])
    env[2].refresh_from_db()
    assert env[2].status == Order.STATUS_PENDING
...
def test_keep_unmatched(env, orga_job):
    process_banktransfers(orga_job, [{'payer': 'Karla Kundin', 'reference': 'No useful reference', 'date': '2016-01-26', 'amount': '23.00'}])
    with scopes_disabled():
...
def test_split_payment_success(env, orga_job):
    o4 = Order.objects.create(code='99999', event=env[0], status=Order.STATUS_PENDING, datetime=now(), expires=now() + timedelta(days=10), total=12)
    process_banktransfers(orga_job, [{'payer': 'Karla Kundin', 'reference': 'Bestellungen DUMMY-1Z3AS DUMMY-99999', 'date': '2016-01-26', 'amount': '35.00'}])
    with scopes_disabled():
        job = BankImportJob.objects.last()
        t = job.transactions.last()
...
def test_split_payment_mismatch(env, orga_job):
    o4 = Order.objects.create(code='99999', event=env[0], status=Order.STATUS_PENDING, datetime=now(), expires=now() + timedelta(days=10), total=12)
    process_banktransfers(orga_job, [{'payer': 'Karla Kundin', 'reference': 'Bestellungen DUMMY-1Z3AS DUMMY-99999', 'date': '2016-01-26', 'amount': '36.00'}])
    with scopes_disabled():
        job = BankImportJob.objects.last()
        t = job.transactions.last()
        assert t.state == BankTransaction.STATE_NOMATCH
