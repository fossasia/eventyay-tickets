...
def test_counter(monkeypatch):
    ...
    pretix_view_requests_total = metrics.Counter('pretix_view_requests_total', 'Total number of HTTP requests made.', ['status_code', 'method', 'url_name'])
...
def test_metrics_view(monkeypatch, client):
    ...
    counter_value = 3
    pretix_view_requests_total = metrics.Counter('pretix_view_requests_total', 'Total number of HTTP requests made.', ['status_code', 'method', 'url_name'])
    fullname = pretix_view_requests_total._construct_metric_identifier('http_requests_total', {'status_code': '200', 'url_name': 'foo', 'method': 'GET'})
