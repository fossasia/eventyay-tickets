...
def test_expiry_days(event):
    ...
    order = _create_order(event, email='dummy@example.org', positions=[], now_dt=today, payment_provider=FreeOrderProvider(event), locale='de')[0]
...
def test_expiry_last_relative(event):
    ...
    event.save()
    event.settings.set('payment_term_last', RelativeDateWrapper(RelativeDate(days_before=2, time=None, base_date_name='date_from', minutes_before=None)))
    order = _create_order(event, email='dummy@example.org', positions=[], now_dt=today, payment_provider=FreeOrderProvider(event), locale='de')[0]
    assert (order.expires - today).days == 3
...
def test_expiry_last_relative_subevents(event):
    ...
    ticket = Item.objects.create(event=event, name='Early-bird ticket', tax_rule=tr7, default_price=Decimal('23.00'), admission=True)
    ...
    cp1 = CartPosition.objects.create(item=ticket, price=23, expires=now() + timedelta(days=1), subevent=se1, event=event, cart_id='123')
    cp2 = CartPosition.objects.create(item=ticket, price=23, expires=now() + timedelta(days=1), subevent=se2, event=event, cart_id='123')

    event.settings.set('payment_term_last', RelativeDateWrapper(RelativeDate(days_before=2, time=None, base_date_name='date_from', minutes_before=None)))
    order = _create_order(event, email='dummy@example.org', positions=[cp1, cp2], now_dt=today, payment_provider=FreeOrderProvider(event), locale='de')[0]
    assert (order.expires - today).days == 6
    ...
    def setUp(self):
        ...
        with scope(organizer=self.o):
            self.event = Event.objects.create(
                organizer=self.o, name='Dummy', slug='dummy', date_from=now() + timedelta(days=2), plugins='pretix.plugins.banktransfer'
            )
            ...
            self.ticket = Item.objects.create(event=self.event, name='Early-bird ticket', default_price=Decimal('23.00'), admission=True)
            self.op1 = OrderPosition.objects.create(
                order=self.order, item=self.ticket, variation=None, price=Decimal('23.00'), attendee_name_parts={'full_name': 'Peter'}, positionid=1
            )
            djmail.outbox = []
...
class OrderCancelTests(TestCase):
    def setUp(self):
        ...
        with scope(organizer=self.o):
            ...
            self.op2 = OrderPosition.objects.create(
                order=self.order, item=self.ticket, variation=None, price=Decimal('23.00'), attendee_name_parts={'full_name': 'Dieter'}, positionid=2
            )
    ...
    def test_auto_refund_possible(self):
        p1 = self.order.payments.create(amount=Decimal('46.00'), state=OrderPayment.PAYMENT_STATE_CONFIRMED, provider='testdummy_partialrefund')
    ...
    def test_auto_refund_possible_giftcard(self):
        ...
        p1 = self.order.payments.create(
            amount=Decimal('46.00'), state=OrderPayment.PAYMENT_STATE_CONFIRMED, provider='giftcard', info='{"gift_card": %d}' % gc.pk
        )
        cancel_order(self.order.pk, cancellation_fee=2, try_auto_refund=True)
        r = self.order.refunds.get()
        assert r.state == OrderRefund.REFUND_STATE_DONE
    ...
    def test_auto_refund_possible_issued_giftcard(self):
        ...
        self.order.payments.create(amount=Decimal('46.00'), state=OrderPayment.PAYMENT_STATE_CONFIRMED, provider='testdummy_partialrefund')
        cancel_order(self.order.pk, cancellation_fee=2, try_auto_refund=True)
        r = self.order.refunds.get()
        assert r.state == OrderRefund.REFUND_STATE_DONE
    ...
    def test_auto_refund_impossible(self):
        self.order.payments.create(amount=Decimal('46.00'), state=OrderPayment.PAYMENT_STATE_CONFIRMED, provider='testdummy_fullrefund')
        cancel_order(self.order.pk, cancellation_fee=2, try_auto_refund=True)
...
class OrderChangeManagerTests(TestCase):
    def setUp(self):
        ...
        self.o = Organizer.objects.create(name='Dummy', slug='dummy')
        with scope(organizer=self.o):
            self.event = Event.objects.create(organizer=self.o, name='Dummy', slug='dummy', date_from=now(), plugins='pretix.plugins.banktransfer')
            self.order = Order.objects.create(
                code='FOO',
                event=self.event,
                email='dummy@dummy.test',
            ...
            self.order.payments.create(provider='banktransfer', state=OrderPayment.PAYMENT_STATE_CREATED, amount=self.order.total)
            ...
            self.stalls = Item.objects.create(event=self.event, name='Stalls', tax_rule=self.tr7, default_price=Decimal('23.00'), admission=True)
    ...
    def _enable_reverse_charge(self):
        ...
        self.tr19.save()
        return InvoiceAddress.objects.create(order=self.order, is_business=True, vat_id='ATU1234567', vat_id_validated=True, country=Country('AT'))
    ...
    def test_split_invoice_address(self):
        ia = InvoiceAddress.objects.create(
            order=self.order, is_business=True, vat_id='ATU1234567', vat_id_validated=True, country=Country('AT'), company='Sample'
        )
...
def test_autocheckin(clist_autocheckin, event):
    ...
    tr7 = event.tax_rules.create(rate=Decimal('17.00'))
    ticket = Item.objects.create(event=event, name='Early-bird ticket', tax_rule=tr7, default_price=Decimal('23.00'), admission=True)
    cp1 = CartPosition.objects.create(item=ticket, price=23, expires=now() + timedelta(days=1), event=event, cart_id='123')
    order = _create_order(event, email='dummy@example.org', positions=[cp1], now_dt=today, payment_provider=FreeOrderProvider(event), locale='de')[0]
...
def test_saleschannel_testmode_restriction(event):
    ...
    order = _create_order(
        event, email='dummy@example.org', positions=[cp1], now_dt=today, payment_provider=FreeOrderProvider(event), locale='de', sales_channel='web'
    )[0]
...
def test_giftcard_multiple(event):
    ticket = Item.objects.create(event=event, name='Early-bird ticket', default_price=Decimal('23.00'), admission=True)
    cp1 = CartPosition.objects.create(item=ticket, price=23, expires=now() + timedelta(days=1), event=event, cart_id='123')
    ...
    order = _create_order(
        event, email='dummy@example.org', positions=[cp1], now_dt=now(), payment_provider=BankTransfer(event), locale='de', gift_cards=[gc1.pk, gc2.pk]
    )[0]
...
def test_giftcard_partial(event):
    ...
    gc1.transactions.create(value=12)
    order = _create_order(
        event, email='dummy@example.org', positions=[cp1], now_dt=now(), payment_provider=BankTransfer(event), locale='de', gift_cards=[gc1.pk]
    )[0]
...
def test_giftcard_invalid_currency(event):
    ...
    gc1.transactions.create(value=12)
    with pytest.raises(OrderError):
        _create_order(event, email='dummy@example.org', positions=[cp1], now_dt=now(), payment_provider=BankTransfer(event), locale='de', gift_cards=[gc1.pk])[
            0
        ]
...
def test_giftcard_swap(event):
    ticket = Item.objects.create(event=event, name='Early-bird ticket', issue_giftcard=True, default_price=Decimal('23.00'), admission=True)
    cp1 = CartPosition.objects.create(item=ticket, price=23, expires=now() + timedelta(days=1), event=event, cart_id='123')
    gc1 = event.organizer.issued_gift_cards.create(currency='EUR', testmode=False)
    gc1.transactions.create(value=12)
    with pytest.raises(OrderError):
...
def test_issue_when_paid_and_changed(event):
    ...
    q.items.add(ticket)
    order = _create_order(event, email='dummy@example.org', positions=[cp1], now_dt=now(), payment_provider=BankTransfer(event), locale='de', gift_cards=[])[0]
...
class OrderReactivateTest(TestCase):
    def setUp(self):
        ...
        with scope(organizer=self.o):
            ...
            self.stalls = Item.objects.create(event=self.event, name='Stalls', default_price=Decimal('23.00'), admission=True)
            self.plan = SeatingPlan.objects.create(name='Plan', organizer=self.o, layout='{}')
            self.event.seat_category_mappings.create(layout_category='Stalls', product=self.stalls)
            self.quota = self.event.quotas.create(name='Test', size=None)
            self.quota.items.add(self.stalls)
