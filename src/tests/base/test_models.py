...
class BaseQuotaTestCase(TestCase):
    def setUp(self):
        ...
        self.event = Event.objects.create(organizer=self.o, name='Dummy', slug='dummy', date_from=now(), plugins='tests.testdummy')
...
class QuotaTestCase(BaseQuotaTestCase):
    ...
    def test_ordered(self):
        ...
        order = Order.objects.create(event=self.event, status=Order.STATUS_PENDING, expires=now() + timedelta(days=3), total=4)
        OrderPosition.objects.create(order=order, item=self.item1, price=2)
    ...
    def test_reserved(self):
        ...
        cp = CartPosition.objects.create(event=self.event, item=self.item1, price=2, expires=now() + timedelta(days=3))
        ...
        self.quota.items.add(self.item2)
        self.quota.variations.add(self.var1)
        cp = CartPosition.objects.create(event=self.event, item=self.item2, variation=self.var1, price=2, expires=now() + timedelta(days=3))
        self.assertEqual(self.item1.check_quotas(), (Quota.AVAILABILITY_RESERVED, 0))
    ...
    def test_ignore_quotas(self):
        ...
        self.assertEqual(self.item1.check_quotas(ignored_quotas=[self.quota, quota2]), (Quota.AVAILABILITY_OK, sys.maxsize))

    @classscope(attr='o')
    def test_unlimited(self):
        self.quota.items.add(self.item1)
        order = Order.objects.create(event=self.event, status=Order.STATUS_PAID, expires=now() + timedelta(days=3), total=2)
        OrderPosition.objects.create(order=order, item=self.item1, price=2)
        OrderPosition.objects.create(order=order, item=self.item1, price=2)
        self.assertEqual(self.item1.check_quotas(), (Quota.AVAILABILITY_GONE, 0))
    ...
    def test_voucher_quota_multiuse_multiproduct(self):
        ...
        Voucher.objects.create(item=self.item2, variation=self.var2, event=self.event, block_quota=True, max_usages=5, redeemed=2)
        Voucher.objects.create(item=self.item2, variation=self.var2, event=self.event, block_quota=True, max_usages=5, redeemed=2)
        qa = QuotaAvailability(full_results=True)
        qa.queue(self.quota)
        qa.compute()
    ...
    def test_voucher_quota_expiring_soon(self):
        ...
        self.quota.size = 1
        self.quota.save()
        Voucher.objects.create(quota=self.quota, event=self.event, valid_until=now() + timedelta(days=5), block_quota=True)
        self.assertEqual(self.var1.check_quotas(), (Quota.AVAILABILITY_ORDERED, 0))
    ...
    def test_blocking_voucher_in_cart(self):
        self.quota.items.add(self.item1)
        v = Voucher.objects.create(quota=self.quota, event=self.event, valid_until=now() + timedelta(days=5), block_quota=True)
        CartPosition.objects.create(event=self.event, item=self.item1, price=2, expires=now() + timedelta(days=3), voucher=v)
    ...
    def test_waitinglist_variation_fulfilled(self):
        ...
        WaitingListEntry.objects.create(event=self.event, item=self.item2, variation=self.var1, email='foo@bar.com', voucher=v)
    ...
    def test_subevent_isolation(self):
        ...
        # Create orders
        order = Order.objects.create(event=self.event, status=Order.STATUS_PAID, expires=now() + timedelta(days=3), total=6)
        ...
        order = Order.objects.create(event=self.event, status=Order.STATUS_PENDING, expires=now() + timedelta(days=3), total=8)
        OrderPosition.objects.create(order=order, item=self.item1, subevent=se1, price=2)
        OrderPosition.objects.create(order=order, item=self.item1, subevent=se1, price=2)
        OrderPosition.objects.create(order=order, item=self.item1, subevent=se1, price=2)
        OrderPosition.objects.create(order=order, item=self.item1, subevent=se2, price=2)

        Voucher.objects.create(item=self.item1, event=self.event, valid_until=now() + timedelta(days=5), block_quota=True, max_usages=6, subevent=se1)
        Voucher.objects.create(item=self.item1, event=self.event, valid_until=now() + timedelta(days=5), block_quota=True, max_usages=4, subevent=se2)

        for i in range(8):
            CartPosition.objects.create(event=self.event, item=self.item1, price=2, subevent=se1, expires=now() + timedelta(days=3))

        for i in range(5):
            CartPosition.objects.create(event=self.event, item=self.item1, price=2, subevent=se2, expires=now() + timedelta(days=3))
...
class BundleQuotaTestCase(BaseQuotaTestCase):
    def setUp(self):
        ...
        self.quota.items.add(self.item2)
        self.quota.variations.add(self.var1)
        self.bundle1 = ItemBundle.objects.create(base_item=self.item1, bundled_item=self.trans, designated_price=1.5, count=1)
        self.bundle2 = ItemBundle.objects.create(base_item=self.item2, bundled_item=self.trans, designated_price=1.5, count=1)
...
class WaitingListTestCase(BaseQuotaTestCase):
    ...
    def test_duplicate(self):
        w1 = WaitingListEntry.objects.create(event=self.event, item=self.item2, variation=self.var1, email='foo@bar.com')
        w1.clean()
    ...
    def test_duplicate_of_successful(self):
        v = Voucher.objects.create(quota=self.quota, event=self.event, block_quota=True, redeemed=1)
        w1 = WaitingListEntry.objects.create(event=self.event, item=self.item2, variation=self.var1, email='foo@bar.com', voucher=v)
        w1.clean()
        w2 = WaitingListEntry(event=self.event, item=self.item2, variation=self.var1, email='foo@bar.com')
        w2.clean()
...
class VoucherTestCase(BaseQuotaTestCase):
    ...
    def test_voucher_reuse(self):
        ...
        cart = CartPosition.objects.create(event=self.event, item=self.item1, price=self.item1.default_price, expires=now() + timedelta(days=3), voucher=v)
        self.assertTrue(v.is_active())
        self.assertTrue(v.is_in_cart())
        self.assertFalse(v.is_ordered())
    ...
    def test_calculate_budget_used(self):
        ...
        OrderPosition.objects.create(order=order, item=self.item1, voucher=v, price=Decimal('20.00'), price_before_voucher=Decimal('23.00'))
...
class OrderTestCase(BaseQuotaTestCase):
    ...
    def test_paid_expired_after_last_date_subevent_relative(self):
        ...
        self.event.settings.set(
            'payment_term_last', RelativeDateWrapper(RelativeDate(days_before=2, time=None, base_date_name='date_from', minutes_before=None))
        )
    ...
    def test_can_modify_answers_subevent(self):
        ...
        self.op2.save()
        self.event.settings.set(
            'last_order_modification_date', RelativeDateWrapper(RelativeDate(days_before=2, time=None, base_date_name='date_from', minutes_before=None))
        )
        assert self.order.can_modify_answers
    ...
    def test_ticket_download_date_relative(self):
        ...
        self.event.save()
        self.event.settings.set(
            'ticket_download_date', RelativeDateWrapper(RelativeDate(days_before=2, time=None, base_date_name='date_from', minutes_before=None))
        )
    ...
    def test_can_cancel_order(self):
        item1 = Item.objects.create(event=self.event, name='Ticket', default_price=23, admission=True, allow_cancel=True)
    ...
    def test_can_cancel_order_with_giftcard(self):
        item1 = Item.objects.create(event=self.event, name='Ticket', default_price=23, admission=True, allow_cancel=True, issue_giftcard=True)
    ...
    def test_can_cancel_checked_in(self):
        ...
        self.event.settings.cancel_allow_user_paid = True
        assert self.order.user_cancel_allowed
        Checkin.objects.create(position=self.order.positions.first(), list=CheckinList.objects.create(event=self.event, name='Default'))
        assert not self.order.user_cancel_allowed

    @classscope(attr='o')
    def test_can_cancel_order_multiple(self):
        item1 = Item.objects.create(event=self.event, name='Ticket', default_price=23, admission=True, allow_cancel=True)
        item2 = Item.objects.create(event=self.event, name='Ticket', default_price=23, admission=True, allow_cancel=True)
        OrderPosition.objects.create(order=self.order, item=item1, variation=None, price=23)
    ...
    def test_can_not_cancel_order(self):
        item1 = Item.objects.create(event=self.event, name='Ticket', default_price=23, admission=True, allow_cancel=False)
        OrderPosition.objects.create(order=self.order, item=item1, variation=None, price=23)
    ...
    def test_user_cancel_relative_deadline_unpaid_no_subevents(self):
        ...
        self.event.settings.set(
            ...
            RelativeDateWrapper(RelativeDate(days_before=2, time=datetime.time(14, 0, 0), base_date_name='date_from', minutes_before=None)),
        )
        self.order = Order.objects.get(pk=self.order.pk)
        assert self.order.user_cancel_deadline > now()
        assert self.order.user_cancel_allowed
        self.event.settings.set(
            'cancel_allow_user_until',
            RelativeDateWrapper(RelativeDate(days_before=4, time=datetime.time(14, 0, 0), base_date_name='date_from', minutes_before=None)),
        )
        self.order = Order.objects.get(pk=self.order.pk)
        assert self.order.user_cancel_deadline < now()
        assert not self.order.user_cancel_allowed
    ...
    def test_paid_order_underpaid(self):
        self.order.status = Order.STATUS_PAID
        self.order.save()
        self.order.payments.create(amount=Decimal('46.00'), state=OrderPayment.PAYMENT_STATE_CONFIRMED, provider='manual')
        self.order.refunds.create(amount=Decimal('10.00'), state=OrderRefund.REFUND_STATE_DONE, provider='manual')
    ...
    def test_paid_order_external_refund(self):
        ...
        self.order.save()
        self.order.payments.create(amount=Decimal('46.00'), state=OrderPayment.PAYMENT_STATE_CONFIRMED, provider='manual')
        self.order.refunds.create(amount=Decimal('10.00'), state=OrderRefund.REFUND_STATE_EXTERNAL, provider='manual')
    ...
    def test_pending_order_pending_refund(self):
        ...
        self.order.save()
        self.order.payments.create(amount=Decimal('46.00'), state=OrderPayment.PAYMENT_STATE_CONFIRMED, provider='manual')
        self.order.refunds.create(amount=Decimal('46.00'), state=OrderRefund.REFUND_STATE_CREATED, provider='manual')
        assert self.order.pending_sum == Decimal('0.00')
        o = Order.annotate_overpayments(Order.objects.all()).first()
        assert not o.is_underpaid
        assert not o.is_overpaid
    ...
    def test_paid_order_overpaid(self):
        self.order.status = Order.STATUS_PAID
        self.order.save()
        self.order.payments.create(amount=Decimal('66.00'), state=OrderPayment.PAYMENT_STATE_CONFIRMED, provider='manual')
        self.order.refunds.create(amount=Decimal('10.00'), state=OrderRefund.REFUND_STATE_DONE, provider='manual')
        assert self.order.pending_sum == Decimal('-10.00')
        o = Order.annotate_overpayments(Order.objects.all()).first()
        assert not o.is_underpaid
    ...
    def test_pending_order_overpaid(self):
        ...
        self.order.save()
        self.order.payments.create(amount=Decimal('56.00'), state=OrderPayment.PAYMENT_STATE_CONFIRMED, provider='manual')
        self.order.refunds.create(amount=Decimal('10.00'), state=OrderRefund.REFUND_STATE_DONE, provider='manual')
        assert self.order.pending_sum == Decimal('0.00')
        o = Order.annotate_overpayments(Order.objects.all()).first()
        assert not o.is_underpaid
    ...
    def test_propose_auto_refunds(self):
        p1 = self.order.payments.create(amount=Decimal('23.00'), state=OrderPayment.PAYMENT_STATE_CONFIRMED, provider='testdummy_fullrefund')
        p2 = self.order.payments.create(amount=Decimal('10.00'), state=OrderPayment.PAYMENT_STATE_CONFIRMED, provider='testdummy_partialrefund')
        self.order.payments.create(amount=Decimal('13.00'), state=OrderPayment.PAYMENT_STATE_CONFIRMED, provider='testdummy')
...
class EventTest(TestCase):
    ...
    def test_event_end_before_start(self):
        event = Event(organizer=self.organizer, name='Dummy', slug='dummy', date_from=now(), date_to=now() - timedelta(hours=1))
    ...
    def test_presale_end_before_start(self):
        event = Event(organizer=self.organizer, name='Dummy', slug='dummy', presale_start=now(), presale_end=now() - timedelta(hours=1))
        with self.assertRaises(ValidationError) as context:
            event.clean()
    ...
    def test_slug_validation(self):
        event = Event(organizer=self.organizer, name='Download', slug='download', date_from=datetime.datetime(2013, 12, 26, tzinfo=datetime.timezone.utc))
        with self.assertRaises(ValidationError) as context:
            event.full_clean()
...
class SubEventTest(TestCase):
    def setUp(self):
        self.organizer = Organizer.objects.create(name='Dummy', slug='dummy')
        self.event = Event.objects.create(
            organizer=self.organizer, name='Dummy', slug='dummy', date_from=now(), date_to=now() - timedelta(hours=1), has_subevents=True
        )
...
class CheckinListTestCase(TestCase):
    def setUp(self):
        ...
        with scope(organizer=self.organizer):
            ...
            self.cl_all_pending = self.event.checkin_lists.create(name='Z Pending', all_products=True, include_pending=True)
...
class SeatingTestCase(TestCase):
    ...
    def test_blocked_in_proximity(self):
        ...
        OrderPosition.objects.create(order=o, item=self.ticket, variation=None, price=Decimal('12'), seat=self.seat_a1)
    ...
    def test_cart_active(self):
        CartPosition.objects.create(event=self.event, cart_id='a', item=self.ticket, seat=self.seat_a1, price=23, expires=now() + timedelta(minutes=10))
        assert set(self.event.free_seats()) == {self.seat_a2}
        assert not self.seat_a1.is_available()
    ...
    def test_subevent_order_pending(self):
        ...
        OrderPosition.objects.create(order=o, item=self.ticket, variation=None, price=Decimal('12'), seat=self.seat_a1, subevent=se1)
    ...
    def test_subevent_cart_active(self):
        ...
        CartPosition.objects.create(
            event=self.event, cart_id='a', item=self.ticket, seat=self.seat_a1, price=23, expires=now() + timedelta(minutes=10), subevent=se1
        )
        assert set(se1.free_seats()) == set()
        assert not self.seat_a1.is_available()
    ...
    def test_voucher_active(self):
        Voucher.objects.create(event=self.event, code='a', item=self.ticket, seat=self.seat_a1, valid_until=now() + timedelta(minutes=10))
        assert set(self.event.free_seats()) == {self.seat_a2}
        assert not self.seat_a1.is_available()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (Question.TYPE_DATETIME, '2018-01-16T15:20:00', datetime.datetime(2018, 1, 16, 15, 20, 0, tzinfo=pytz.timezone('Europe/Berlin'))),
        (Question.TYPE_DATETIME, '2018-01-16T14:20:00Z', datetime.datetime(2018, 1, 16, 14, 20, 0, tzinfo=pytz.UTC).astimezone(pytz.timezone('Europe/Berlin'))),
        (Question.TYPE_DATETIME, '2018-01-16T15:20:00', datetime.datetime(2018, 1, 16, 15, 20, 0, tzinfo=pytz.timezone('Europe/Berlin'))),
...
def test_subevent_date_updates_order_date():
    ...
    organizer = Organizer.objects.create(name='Dummy', slug='dummy')
    with scope(organizer=organizer):
        event = Event.objects.create(organizer=organizer, name='Dummy', slug='dummy', date_from=now(), date_to=now() - timedelta(hours=1), has_subevents=True)
