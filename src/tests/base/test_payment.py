...
def test_availability_date_relative(event):
    ...
    event.save()
    prov = DummyPaymentProvider(event)
    prov.settings.set('_availability_date', RelativeDateWrapper(RelativeDate(days_before=2, time=None, base_date_name='date_from', minutes_before=None)))
...
def test_availability_date_cart_relative_subevents(event):
    ...
    CartPosition.objects.create(item=ticket, price=23, expires=now() + datetime.timedelta(days=1), subevent=se1, event=event, cart_id='123')
    CartPosition.objects.create(item=ticket, price=23, expires=now() + datetime.timedelta(days=1), subevent=se2, event=event, cart_id='123')

    prov = DummyPaymentProvider(event)
    prov.settings.set('_availability_date', RelativeDateWrapper(RelativeDate(days_before=3, time=None, base_date_name='date_from', minutes_before=None)))
    assert prov._is_still_available(cart_id='123')

    prov.settings.set('_availability_date', RelativeDateWrapper(RelativeDate(days_before=4, time=None, base_date_name='date_from', minutes_before=None)))
...
def test_availability_date_order_relative_subevents(event):
    ...
    OrderPosition.objects.create(
        order=order, item=ticket, variation=None, subevent=se1, price=Decimal('23.00'), attendee_name_parts={'full_name': 'Peter'}, positionid=1
    )
    OrderPosition.objects.create(
        order=order, item=ticket, variation=None, subevent=se2, price=Decimal('23.00'), attendee_name_parts={'full_name': 'Dieter'}, positionid=2
    )
