...
def event():
    ...
    event = Event.objects.create(organizer=o, name='Dummy', slug='dummy', date_from=now(), plugins='pretix.plugins.banktransfer,pretix.plugins.ticketoutputpdf')
...
def test_email_shredder(event, order):
    l1 = order.log_action('pretix.event.order.email.expired', data={'recipient': 'dummy@dummy.test', 'message': 'Hello Peter@,', 'subject': 'Foo'})
    l2 = order.log_action(
...
def test_waitinglist_shredder(event, item):
    ...
    q.items.add(item)
    wle = event.waitinglistentries.create(item=item, email='foo@example.org', name_parts={'_legacy': 'Peter'}, phone='+49891234567')
    wle.send_voucher()
...
def test_attendee_name_shredder(event, order):
    l1 = order.log_action(
        'pretix.event.order.modified', data={'data': [{'attendee_name': 'Peter', 'question_1': 'Test', 'company': 'Foobar'}], 'invoice_data': {'name': 'Foo'}}
    )

    s = AttendeeInfoShredder(event)
    f = list(s.generate_files())
    assert json.loads(f[0][2]) == {
        '{}-{}'.format(order.code, 1): {'name': 'Peter', 'company': 'Foobar', 'street': None, 'zipcode': None, 'city': None, 'country': None, 'state': None}
    }
    s.shred_data()
    order.refresh_from_db()
...
def test_invoice_address_shredder(event, order):
    ...
    ia = InvoiceAddress.objects.create(company='Acme Company', street='221B Baker Street', zipcode='12345', city='London', country='UK', order=order)
...
def test_question_answer_shredder(event, order, question):
    ...
    l1 = order.log_action(
        'pretix.event.order.modified',
        data={
            'data': [{'attendee_name': 'Hans', 'question_%d' % question.pk: [{'id': opt.pk, 'type': 'QuestionOption'}]}],
        },
    )
...
def test_invoice_shredder(event, order):
    InvoiceAddress.objects.create(company='Acme Company', street='221B Baker Street', zipcode='12345', city='London', country='UK', order=order)
...
def test_payment_info_shredder(event, order):
    order.payments.create(
        info=json.dumps({'reference': 'Verwendungszweck 1', 'date': '2018-05-01', 'payer': 'Hans', 'trans_id': 12}),
    ...
    order.refresh_from_db()
    assert order.payments.first().info_data == {'_shredded': True, 'reference': '█', 'date': '2018-05-01', 'payer': '█', 'trans_id': 12}
