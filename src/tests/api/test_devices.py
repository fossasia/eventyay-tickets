...
def test_device_create(token_client, organizer, event):
    resp = token_client.post('/api/v1/organizers/{}/devices/'.format(organizer.slug), TEST_DEVICE_CREATE_PAYLOAD, format='json')
...
def test_device_update(token_client, organizer, event, device):
    resp = token_client.patch(
        '/api/v1/organizers/{}/devices/{}/'.format(organizer.slug, device.device_id), {'name': 'bla', 'hardware_brand': 'Foo'}, format='json'
    )
    assert resp.status_code == 200
