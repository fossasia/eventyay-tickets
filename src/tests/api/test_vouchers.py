...
def test_voucher_list(token_client, organizer, event, voucher, item, quota, subevent):
    ...
    resp = token_client.get(
        '/api/v1/organizers/{}/events/{}/vouchers/?code={}'.format(
            organizer.slug, event.slug, voucher.code
        )
    )
    assert [res] == resp.data['results']
    resp = token_client.get(
        '/api/v1/organizers/{}/events/{}/vouchers/?code=ABC'.format(
    ...
    resp = token_client.get(
        '/api/v1/organizers/{}/events/{}/vouchers/?max_usages=1'.format(
            organizer.slug, event.slug
        )
    )
    assert [res] == resp.data['results']
    resp = token_client.get(
        '/api/v1/organizers/{}/events/{}/vouchers/?max_usages=2'.format(
            organizer.slug, event.slug
        )
    )
    assert [] == resp.data['results']
    ...
    resp = token_client.get(
        '/api/v1/organizers/{}/events/{}/vouchers/?redeemed=0'.format(
            organizer.slug, event.slug
        )
    )
    assert [res] == resp.data['results']
    resp = token_client.get(
        '/api/v1/organizers/{}/events/{}/vouchers/?redeemed=1'.format(
            organizer.slug, event.slug
        )
    )
    assert [] == resp.data['results']
    ...
    resp = token_client.get(
        '/api/v1/organizers/{}/events/{}/vouchers/?block_quota=false'.format(
            organizer.slug, event.slug
        )
    )
    assert [res] == resp.data['results']
    resp = token_client.get(
        '/api/v1/organizers/{}/events/{}/vouchers/?block_quota=true'.format(
            organizer.slug, event.slug
        )
    )
    assert [] == resp.data['results']
    ...
    resp = token_client.get(
        '/api/v1/organizers/{}/events/{}/vouchers/?allow_ignore_quota=false'.format(
            organizer.slug, event.slug
        )
    )
    assert [res] == resp.data['results']
    resp = token_client.get(
        '/api/v1/organizers/{}/events/{}/vouchers/?allow_ignore_quota=true'.format(
            organizer.slug, event.slug
        )
    )
    assert [] == resp.data['results']
    ...
    resp = token_client.get(
        '/api/v1/organizers/{}/events/{}/vouchers/?price_mode=set'.format(
            organizer.slug, event.slug
        )
    )
    assert [res] == resp.data['results']
    resp = token_client.get(
        '/api/v1/organizers/{}/events/{}/vouchers/?price_mode=percent'.format(
            organizer.slug, event.slug
        )
    )
    assert [] == resp.data['results']
    ...
    resp = token_client.get(
        '/api/v1/organizers/{}/events/{}/vouchers/?value=12.00'.format(
            organizer.slug, event.slug
        )
    )
    assert [res] == resp.data['results']
    resp = token_client.get(
        '/api/v1/organizers/{}/events/{}/vouchers/?value=10.00'.format(
            organizer.slug, event.slug
        )
    )
    assert [] == resp.data['results']
    ...
    resp = token_client.get(
        '/api/v1/organizers/{}/events/{}/vouchers/?item={}'.format(
    ...
    resp = token_client.get(
        '/api/v1/organizers/{}/events/{}/vouchers/?variation={}'.format(
    ...
    res['quota'] = quota.pk
    resp = token_client.get(
        '/api/v1/organizers/{}/events/{}/vouchers/?quota={}'.format(
    ...
    resp = token_client.get(
        '/api/v1/organizers/{}/events/{}/vouchers/?quota={}'.format(
            organizer.slug, event.slug, q2.pk
        )
    )
    assert [] == resp.data['results']

    resp = token_client.get(
        '/api/v1/organizers/{}/events/{}/vouchers/?tag=Foo'.format(
            organizer.slug, event.slug
        )
    )
    assert [res] == resp.data['results']
    resp = token_client.get(
        '/api/v1/organizers/{}/events/{}/vouchers/?tag=bar'.format(
            organizer.slug, event.slug
        )
    )
    assert [] == resp.data['results']
    ...
    resp = token_client.get(
        '/api/v1/organizers/{}/events/{}/vouchers/?active=true'.format(
            organizer.slug, event.slug
        )
    )
    assert [res] == resp.data['results']
    resp = token_client.get(
        '/api/v1/organizers/{}/events/{}/vouchers/?active=false'.format(
            organizer.slug, event.slug
        )
    )
    assert [] == resp.data['results']
    ...
    voucher.redeemed = 0
    voucher.valid_until = (timezone.now() - datetime.timedelta(days=1)).replace(
        microsecond=0
    )
    voucher.save()
    ...
    resp = token_client.get(
        '/api/v1/organizers/{}/events/{}/vouchers/?subevent={}'.format(
...
def test_voucher_detail(token_client, organizer, event, voucher, item):
    ...
    resp = token_client.get(
        '/api/v1/organizers/{}/events/{}/vouchers/{}/'.format(
            organizer.slug, event.slug, voucher.pk
        )
    )
    assert resp.status_code == 200
...
@pytest.mark.django_db
def test_create_blocking_item_voucher_quota_free(
    token_client, organizer, event, item, quota
):
    v = create_voucher(
        token_client, organizer, event, data={'item': item.pk, 'block_quota': True}
    )
    assert v.block_quota


@pytest.mark.django_db
def test_create_blocking_item_voucher_quota_full(
    token_client, organizer, event, item, quota
):
    quota.size = 0
    quota.save()
...
@pytest.mark.django_db
def test_create_blocking_item_voucher_quota_full_invalid(
    token_client, organizer, event, item, quota
):
    quota.size = 0
    quota.save()
...
@pytest.mark.django_db
def test_create_blocking_variation_voucher_quota_free(
    token_client, organizer, event, item, quota
):
    with scopes_disabled():
        variation = item.variations.create(value='XL')
...
@pytest.mark.django_db
def test_create_blocking_variation_voucher_quota_full(
    token_client, organizer, event, item, quota
):
    with scopes_disabled():
        variation = item.variations.create(value='XL')
...
@pytest.mark.django_db
def test_create_blocking_quota_voucher_quota_free(
    token_client, organizer, event, quota
):
    create_voucher(
        token_client,
        organizer,
...
@pytest.mark.django_db
def test_create_blocking_quota_voucher_quota_full(
    token_client, organizer, event, quota
):
    quota.size = 0
    quota.save()
...
@pytest.mark.django_db
def test_subevent_blocking_quota_free(
...
@pytest.mark.django_db
def test_subevent_blocking_quota_full(
    token_client, organizer, event, item, quota, subevent
):
    with scopes_disabled():
        se2 = event.subevents.create(name='Bar', date_from=now())
...
def change_voucher(
    token_client, organizer, event, voucher, data, expected_failure=False
):
...
def test_change_non_blocking_voucher(token_client, organizer, event, item, quota):
    with scopes_disabled():
        v = event.vouchers.create(item=item)
    change_voucher(
        token_client, organizer, event, v, data={'quota': quota.pk, 'item': None}
    )
    assert v.item is None
    assert v.quota == quota
...
def test_change_voucher_reduce_max_usages(token_client, organizer, event, item, quota):
    ...
    change_voucher(
        token_client, organizer, event, v, data={'max_usages': 2}, expected_failure=True
    )
    assert v.max_usages == 5


@pytest.mark.django_db
def test_change_blocking_voucher_unchanged_quota_full(
    token_client, organizer, event, item, quota
):
    quota.size = 0
    quota.save()
...
@pytest.mark.django_db
def test_change_voucher_to_blocking_quota_full(
    token_client, organizer, event, item, quota
):
    quota.size = 0
    quota.save()
...
@pytest.mark.django_db
def test_change_voucher_to_blocking_quota_free(
    token_client, organizer, event, item, quota
):
    with scopes_disabled():
        v = event.vouchers.create(item=item)
...
@pytest.mark.django_db
def test_change_voucher_validity_to_valid_quota_full(
    token_client, organizer, event, item, quota
):
    quota.size = 0
    quota.save()
    with scopes_disabled():
        v = event.vouchers.create(
            item=item, valid_until=now() - datetime.timedelta(days=3), block_quota=True
        )
    change_voucher(
        token_client,
        organizer,
...
@pytest.mark.django_db
def test_change_voucher_validity_to_valid_quota_free(
    token_client, organizer, event, item, quota
):
    with scopes_disabled():
        v = event.vouchers.create(
...
@pytest.mark.django_db
def test_change_item_of_blocking_voucher_quota_free(
    token_client, organizer, event, item, quota
):
    with scopes_disabled():
...
@pytest.mark.django_db
def test_change_item_of_blocking_voucher_quota_full(
    token_client, organizer, event, item, quota
):
    with scopes_disabled():
        ticket2 = event.items.create(name='Late-bird ticket', default_price=23)
...
@pytest.mark.django_db
def test_change_variation_of_blocking_voucher_without_quota_change(
    token_client, organizer, event
):
    with scopes_disabled():
        shirt = event.items.create(name='Shirt', default_price=23)
...
@pytest.mark.django_db
def test_change_variation_of_blocking_voucher_quota_full(
    token_client, organizer, event
):
    with scopes_disabled():
        shirt = event.items.create(name='Shirt', default_price=23)
...
def test_change_quota_of_blocking_voucher_quota_full(token_client, organizer, event):
    with scopes_disabled():
        ...
        v = event.vouchers.create(quota=qs, block_quota=True)
    change_voucher(
        token_client, organizer, event, v, data={'quota': qm.pk}, expected_failure=True
    )


@pytest.mark.django_db
def test_change_item_of_blocking_voucher_without_quota_change(
    token_client, organizer, event, item, quota
):
    with scopes_disabled():
        quota.size = 0
...
@pytest.mark.django_db
def test_change_subevent_blocking_quota_free(
    token_client, organizer, event, item, quota, subevent
):
    with scopes_disabled():
        quota.subevent = subevent
...
@pytest.mark.django_db
def test_change_subevent_blocking_quota_full(
    token_client, organizer, event, item, quota, subevent
):
    with scopes_disabled():
        quota.subevent = subevent
...
def test_delete_voucher(token_client, organizer, event, quota):
    ...
    resp = token_client.delete(
        '/api/v1/organizers/{}/events/{}/vouchers/{}/'.format(
            organizer.slug, event.slug, v.pk
        ),
    )
...
@pytest.mark.django_db
def test_create_multiple_vouchers_duplicate_seat(
    token_client, organizer, event, item, seat1, seatingplan
):
    resp = token_client.post(
        '/api/v1/organizers/{}/events/{}/vouchers/batch_create/'.format(
            organizer.slug, event.slug
...
@pytest.mark.django_db
def test_seat_seat_productmissing(
    token_client, organizer, event, seatingplan, seat1, item, quota
):
    with scopes_disabled():
        v = event.vouchers.create(quota=quota)
    change_voucher(
        token_client, organizer, event, v, data={'seat': 'A1'}, expected_failure=True
    )


@pytest.mark.django_db
def test_seat_seat_productwrong(
    token_client, organizer, event, seatingplan, seat1, item, quota
):
    with scopes_disabled():
...
def test_set_seat_subevent(
    ...
    with scopes_disabled():
        ...
        se2 = event.subevents.create(
            name='Baz', date_from=datetime.datetime(2017, 12, 27, 10, 0, 0, tzinfo=UTC)
        )
        seat1 = event.seats.create(
            seat_number='A1', product=item, seat_guid='A1', subevent=se1
        )
...
@pytest.mark.django_db
def test_set_seat_subevent_required(
    token_client, organizer, event, seatingplan, seat1, item, quota
):
    with scopes_disabled():
        event.has_subevents = True
...
@pytest.mark.django_db
def test_set_seat_subevent_invalid(
    token_client, organizer, event, seatingplan, seat1, item, quota
):
    with scopes_disabled():
        event.has_subevents = True
