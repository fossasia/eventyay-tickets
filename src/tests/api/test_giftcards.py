...
TEST_GC_RES = {'id': 1, 'secret': 'ABCDEF', 'value': '23.00', 'testmode': False, 'expires': None, 'conditions': None, 'currency': 'EUR'}
...
def test_giftcard_create(token_client, organizer, event):
    resp = token_client.post('/api/v1/organizers/{}/giftcards/'.format(organizer.slug), TEST_GIFTCARD_CREATE_PAYLOAD, format='json')
...
def test_giftcard_transact(token_client, organizer, event, giftcard):
    ...
    resp = token_client.post(
        '/api/v1/organizers/{}/giftcards/{}/transact/'.format(organizer.slug, giftcard.pk), {'value': '10.00', 'text': 'bla'}, format='json'
    )
    assert resp.status_code == 200
    giftcard.refresh_from_db()
