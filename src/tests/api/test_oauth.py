...
def admin_team(organizer):
    return Team.objects.create(organizer=organizer, can_change_teams=True, name='Admin team', all_events=True, can_create_events=True)
...
def test_authorize_require_login(client, application: OAuthApplication):
    ...
    resp = client.get('/api/v1/oauth/authorize?client_id=%s&redirect_uri=%s' % (application.client_id, quote('https://example.org')))
...
def test_authorize_missing_response_type(client, admin_user, application: OAuthApplication):
    application, secret = application
    client.login(email='dummy@dummy.dummy', password='dummy')
    resp = client.get('/api/v1/oauth/authorize?client_id=%s&redirect_uri=%s' % (application.client_id, quote(application.redirect_uris)))
    assert resp.status_code == 302
...
def test_authorize_require_organizer(client, admin_user, organizer, application: OAuthApplication):
    application, secret = application
    client.login(email='dummy@dummy.dummy', password='dummy')
    resp = client.get('/api/v1/oauth/authorize?client_id=%s&redirect_uri=%s&response_type=code' % (application.client_id, quote(application.redirect_uris)))
    assert resp.status_code == 200
    resp = client.post(
        '/api/v1/oauth/authorize?client_id=%s&redirect_uri=%s&response_type=code' % (application.client_id, quote(application.redirect_uris)),
        data={
...
def test_authorize_disallow_response_token(client, admin_user, organizer, application: OAuthApplication):
    application, secret = application
    client.login(email='dummy@dummy.dummy', password='dummy')
    resp = client.get('/api/v1/oauth/authorize?client_id=%s&redirect_uri=%s&response_type=token' % (application.client_id, quote(application.redirect_uris)))
    assert resp.status_code == 302
...
def test_authorize_state(client, admin_user, organizer, application: OAuthApplication):
    ...
    client.login(email='dummy@dummy.dummy', password='dummy')
    resp = client.get(
        '/api/v1/oauth/authorize?client_id=%s&redirect_uri=%s&response_type=code&state=asdadf' % (application.client_id, quote(application.redirect_uris))
    )
    assert resp.status_code == 200
    resp = client.post(
        '/api/v1/oauth/authorize',
...
def test_allow_profile_only(client, admin_user, organizer, application: OAuthApplication):
    ...
    client.login(email='dummy@dummy.dummy', password='dummy')
    resp = client.get(
        '/api/v1/oauth/authorize?client_id=%s&redirect_uri=%s&response_type=code&scope=profile' % (application.client_id, quote(application.redirect_uris))
    )
    assert resp.status_code == 200
    resp = client.post(
        '/api/v1/oauth/authorize',
