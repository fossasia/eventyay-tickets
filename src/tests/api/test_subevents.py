...
def test_subevent_list(token_client, organizer, event, subevent):
    ...
    resp = token_client.get('/api/v1/organizers/{}/events/{}/subevents/?active=false'.format(organizer.slug, event.slug))
    assert [res] == resp.data['results']
    resp = token_client.get('/api/v1/organizers/{}/events/{}/subevents/?active=true'.format(organizer.slug, event.slug))
    assert [] == resp.data['results']

    resp = token_client.get('/api/v1/organizers/{}/events/{}/subevents/?event__live=false'.format(organizer.slug, event.slug))
    assert [res] == resp.data['results']
    resp = token_client.get('/api/v1/organizers/{}/events/{}/subevents/?event__live=true'.format(organizer.slug, event.slug))
    assert [] == resp.data['results']

    resp = token_client.get('/api/v1/organizers/{}/events/{}/subevents/?ends_after=2017-12-27T09:59:59Z'.format(organizer.slug, event.slug))
    assert [res] == resp.data['results']
    resp = token_client.get('/api/v1/organizers/{}/events/{}/subevents/?ends_after=2017-12-27T10:01:01Z'.format(organizer.slug, event.slug))
    assert [] == resp.data['results']
...
def test_subevent_update(token_client, organizer, event, subevent, item, item2, meta_prop, variations, variations2):
    ...
    resp = token_client.patch('/api/v1/organizers/{}/events/{}/subevents/{}/'.format(organizer.slug, event.slug, subevent.pk), {'meta_data': {}}, format='json')
    assert resp.status_code == 200
    with scopes_disabled():
        assert not subevent.meta_values.filter(property__name=meta_prop.name).exists()

    resp = token_client.patch(
        '/api/v1/organizers/{}/events/{}/subevents/{}/'.format(organizer.slug, event.slug, subevent.pk), {'meta_data': {'test': 'test'}}, format='json'
    )
    assert resp.status_code == 400
...
def test_subevent_detail(token_client, organizer, event, subevent):
    ...
    res['id'] = subevent.pk
    res['last_modified'] = subevent.last_modified.isoformat().replace('+00:00', 'Z')
    resp = token_client.get('/api/v1/organizers/{}/events/{}/subevents/{}/'.format(organizer.slug, event.slug, subevent.pk))
    assert resp.status_code == 200
...
def test_subevent_delete(token_client, organizer, event, subevent):
    resp = token_client.delete('/api/v1/organizers/{}/events/{}/subevents/{}/'.format(organizer.slug, event.slug, subevent.pk))
...
def test_remove_seating(token_client, organizer, event, item, seatingplan, subevent):
    ...
    resp = token_client.patch(
        '/api/v1/organizers/{}/events/{}/subevents/{}/'.format(organizer.slug, event.slug, subevent.pk), {'seating_plan': None}, format='json'
    )
    assert resp.status_code == 200
    subevent.refresh_from_db()
