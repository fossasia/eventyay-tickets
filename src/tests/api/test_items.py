...
TEST_CATEGORY_RES = {'name': {'en': 'Tickets'}, 'description': {'en': ''}, 'internal_name': None, 'position': 0, 'is_addon': False}
...
def test_category_list(token_client, organizer, event, team, category):
    ...
    resp = token_client.get('/api/v1/organizers/{}/events/{}/categories/?is_addon=false'.format(organizer.slug, event.slug))
    assert resp.status_code == 200
    assert [res] == resp.data['results']
    resp = token_client.get('/api/v1/organizers/{}/events/{}/categories/?is_addon=true'.format(organizer.slug, event.slug))
    assert resp.status_code == 200
    ...
    assert lmd
    time.sleep(1)
    resp = token_client.get('/api/v1/organizers/{}/events/{}/categories/'.format(organizer.slug, event.slug), HTTP_IF_MODIFIED_SINCE=lmd)
...
def test_category_detail(token_client, organizer, event, team, category):
    res = dict(TEST_CATEGORY_RES)
    res['id'] = category.pk
    resp = token_client.get('/api/v1/organizers/{}/events/{}/categories/{}/'.format(organizer.slug, event.slug, category.pk))
    assert resp.status_code == 200
...
def test_category_create(token_client, organizer, event, team):
    resp = token_client.post(
        ...
        {'name': {'en': 'Tickets'}, 'description': {'en': ''}, 'position': 0, 'is_addon': False},
        format='json',
    )
...
def test_category_delete(token_client, organizer, event, category3, item):
    resp = token_client.delete('/api/v1/organizers/{}/events/{}/categories/{}/'.format(organizer.slug, event.slug, category3.pk))
...
def test_item_list(token_client, organizer, event, team, item):
    ...
    resp = token_client.get('/api/v1/organizers/{}/events/{}/items/?category={}'.format(organizer.slug, event.slug, cat.pk))
    assert resp.status_code == 200
    assert [] == resp.data['results']
...
def test_item_create_with_variation(token_client, organizer, event, item, category, taxrule):
    resp = token_client.post(
        '/api/v1/organizers/{}/events/{}/items/'.format(organizer.slug, event.slug),
        {
            ...
            'has_variations': True,
            'variations': [
                {'value': {'de': 'Kommentar', 'en': 'Comment'}, 'active': True, 'description': None, 'position': 0, 'default_price': None, 'price': '23.00'}
            ],
        },
        format='json',
    )
...
def test_item_create_with_addon(token_client, organizer, event, item, category, category2, taxrule):
    resp = token_client.post(
        '/api/v1/organizers/{}/events/{}/items/'.format(organizer.slug, event.slug),
        {
            ...
            'checkin_attention': False,
            'has_variations': True,
            'addons': [{'addon_category': category.pk, 'min_count': 0, 'max_count': 10, 'position': 0, 'multi_allowed': False, 'price_included': True}],
        },
        format='json',
    )
    assert resp.status_code == 201
    ...
    resp = token_client.post(
        '/api/v1/organizers/{}/events/{}/items/'.format(organizer.slug, event.slug),
        {
            ...
            'checkin_attention': False,
            'has_variations': True,
            'addons': [{'addon_category': category2.pk, 'min_count': 0, 'max_count': 10, 'position': 0, 'multi_allowed': False, 'price_included': True}],
        },
        format='json',
    )
    assert resp.status_code == 400
    ...
    resp = token_client.post(
        '/api/v1/organizers/{}/events/{}/items/'.format(organizer.slug, event.slug),
        {
            ...
            'checkin_attention': False,
            'has_variations': True,
            'addons': [{'addon_category': category.pk, 'min_count': 110, 'max_count': 10, 'position': 0, 'multi_allowed': False, 'price_included': True}],
        },
        format='json',
    )
    assert resp.status_code == 400
    ...
    resp = token_client.post(
        '/api/v1/organizers/{}/events/{}/items/'.format(organizer.slug, event.slug),
        {
            ...
            'checkin_attention': False,
            'has_variations': True,
            'addons': [{'addon_category': category.pk, 'min_count': -1, 'max_count': 10, 'position': 0, 'multi_allowed': False, 'price_included': True}],
        },
        format='json',
    )
    assert resp.status_code == 400
...
def test_item_update(token_client, organizer, event, item, category, item2, category2, taxrule2):
    resp = token_client.patch(
        '/api/v1/organizers/{}/events/{}/items/{}/'.format(organizer.slug, event.slug, item.pk), {'min_per_order': 1, 'max_per_order': 2}, format='json'
    )
    assert resp.status_code == 200
    with scopes_disabled():
    ...
    resp = token_client.patch(
        '/api/v1/organizers/{}/events/{}/items/{}/'.format(organizer.slug, event.slug, item.pk), {'min_per_order': 10, 'max_per_order': 2}, format='json'
    )
    assert resp.status_code == 400
    ...
    resp = token_client.patch(
        '/api/v1/organizers/{}/events/{}/items/{}/'.format(organizer.slug, event.slug, item.pk), {'category': category2.pk}, format='json'
    )
    assert resp.status_code == 400
    assert resp.content.decode() == '{"category":["The item\'s category must belong to the same event as the item."]}'

    resp = token_client.patch('/api/v1/organizers/{}/events/{}/items/{}/'.format(organizer.slug, event.slug, item.pk), {'tax_rule': taxrule2.pk}, format='json')
    assert resp.status_code == 400
    ...
    assert resp.status_code == 400
    assert (
        resp.content.decode() == '{"non_field_errors":["Updating add-ons, bundles, or variations via PATCH/PUT is not supported. Please use '
    ...
    resp = token_client.patch(
        '/api/v1/organizers/{}/events/{}/items/{}/'.format(organizer.slug, event.slug, item.pk), {'meta_data': {'day': 'Friday'}}, format='json'
    )
    assert resp.status_code == 200
    with scopes_disabled():
    ...
    resp = token_client.patch(
        '/api/v1/organizers/{}/events/{}/items/{}/'.format(organizer.slug, event.slug, item.pk), {'meta_data': {'foo': 'bar'}}, format='json'
    )
    assert resp.status_code == 400
...
TEST_VARIATIONS_UPDATE = {'value': {'en': 'ChildC2'}, 'active': True, 'description': None, 'position': 1, 'default_price': '20.0', 'original_price': None}
...
def test_variations_create(token_client, organizer, event, item, variation):
    resp = token_client.post(
        ...
        {'value': {'en': 'ChildC2'}, 'active': True, 'description': None, 'position': 1, 'default_price': None, 'original_price': '23.42', 'price': 23.0},
        format='json',
    )
    assert resp.status_code == 201
    with scopes_disabled():
...
def test_variations_create_not_allowed(token_client, organizer, event, item):
    resp = token_client.post(
        '/api/v1/organizers/{}/events/{}/items/{}/variations/'.format(organizer.slug, event.slug, item.pk),
        {'value': {'en': 'ChildC2'}, 'active': True, 'description': None, 'position': 1, 'default_price': None, 'price': 23.0},
        format='json',
    )
    assert resp.status_code == 403
    assert (
...
def test_variations_update(token_client, organizer, event, item, item3, variation):
    ...
    resp = token_client.patch(
        ...
        {'value': {'en': 'ChildC2'}, 'position': 1, 'default_price': '20.00', 'original_price': '50.00'},
        format='json',
    )
    assert resp.status_code == 200
    assert res == resp.data
    ...
    resp = token_client.patch(
        '/api/v1/organizers/{}/events/{}/items/{}/variations/{}/'.format(organizer.slug, event.slug, item3.pk, variation.pk), {'position': 1}, format='json'
    )
    assert resp.status_code == 404
...
TEST_BUNDLE_RES = {'bundled_item': 0, 'bundled_variation': None, 'count': 1, 'designated_price': '2.00'}
...
def test_bundles_list(token_client, organizer, event, item, bundle, item3):
    ...
    resp = token_client.get('/api/v1/organizers/{}/events/{}/items/{}/bundles/'.format(organizer.slug, event.slug, item.pk))
    assert resp.status_code == 200
...
def test_bundles_detail(token_client, organizer, event, item, bundle, item3):
    ...
    res['id'] = bundle.pk
    res['bundled_item'] = item3.pk
    resp = token_client.get('/api/v1/organizers/{}/events/{}/items/{}/bundles/{}/'.format(organizer.slug, event.slug, item.pk, bundle.pk))
    assert resp.status_code == 200
    assert res == resp.data
...
def test_bundles_delete(token_client, organizer, event, item, bundle):
    resp = token_client.delete('/api/v1/organizers/{}/events/{}/items/{}/bundles/{}/'.format(organizer.slug, event.slug, item.pk, bundle.pk))
    assert resp.status_code == 204
    with scopes_disabled():
...
TEST_ADDONS_RES = {'min_count': 0, 'max_count': 10, 'position': 1, 'multi_allowed': False, 'price_included': False}
...
def test_addons_create(token_client, organizer, event, item, category, category2):
    resp = token_client.post(
        ...
        {'addon_category': category.pk, 'min_count': 0, 'max_count': 10, 'position': 1, 'multi_allowed': False, 'price_included': False},
        format='json',
    )
    assert resp.status_code == 201
    with scopes_disabled():
    ...
    resp = token_client.post(
        '/api/v1/organizers/{}/events/{}/items/{}/addons/'.format(organizer.slug, event.slug, item.pk),
        {'addon_category': category.pk, 'min_count': 10, 'max_count': 20, 'position': 2, 'multi_allowed': False, 'price_included': False},
        format='json',
    )
    assert resp.status_code == 400
    ...
    resp = token_client.post(
        '/api/v1/organizers/{}/events/{}/items/{}/addons/'.format(organizer.slug, event.slug, item.pk),
        {'addon_category': category2.pk, 'min_count': 10, 'max_count': 20, 'position': 2, 'multi_allowed': False, 'price_included': False},
        format='json',
    )
    assert resp.status_code == 400
    assert resp.content.decode() == '{"addon_category":["The add-on\'s category must belong to the same event as the item."]}'
...
def test_addons_update(token_client, organizer, event, item, addon):
    ...
    assert resp.status_code == 400
    assert resp.content.decode() == '{"non_field_errors":["The maximum count needs to be greater than the minimum count."]}'
...
def test_quota_list(token_client, organizer, event, quota, item, subevent):
    ...
    resp = token_client.get('/api/v1/organizers/{}/events/{}/quotas/?subevent={}'.format(organizer.slug, event.slug, subevent.pk))
    assert [res] == resp.data['results']
    with scopes_disabled():
        se2 = event.subevents.create(name='Foobar', date_from=datetime(2017, 12, 27, 10, 0, 0, tzinfo=UTC))
    resp = token_client.get('/api/v1/organizers/{}/events/{}/quotas/?subevent={}'.format(organizer.slug, event.slug, se2.pk))
    assert [] == resp.data['results']
...
def test_quota_create(token_client, organizer, event, event2, item):
    resp = token_client.post(
        ...
        {'name': 'Ticket Quota', 'size': 200, 'items': [item.pk], 'variations': [], 'subevent': None},
        format='json',
    )
    assert resp.status_code == 201
    with scopes_disabled():
...
def test_quota_create_with_variations(token_client, organizer, event, item, variations, variations2):
    resp = token_client.post(
        '/api/v1/organizers/{}/events/{}/quotas/'.format(organizer.slug, event.slug),
        {'name': 'Ticket Quota', 'size': 200, 'items': [item.pk], 'variations': [variations[0].pk], 'subevent': None},
        format='json',
    )
    assert resp.status_code == 201
    ...
    resp = token_client.post(
        '/api/v1/organizers/{}/events/{}/quotas/'.format(organizer.slug, event.slug),
        {'name': 'Ticket Quota', 'size': 200, 'items': [item.pk], 'variations': [100], 'subevent': None},
        format='json',
    )
    assert resp.status_code == 400
    ...
    resp = token_client.post(
        '/api/v1/organizers/{}/events/{}/quotas/'.format(organizer.slug, event.slug),
        {'name': 'Ticket Quota', 'size': 200, 'items': [item.pk], 'variations': [variations[0].pk, variations2[0].pk], 'subevent': None},
        format='json',
    )
    assert resp.status_code == 400
...
def test_quota_create_with_subevent(token_client, organizer, event, event3, item, variations, subevent, subevent2):
    resp = token_client.post(
        '/api/v1/organizers/{}/events/{}/quotas/'.format(organizer.slug, event.slug),
        {'name': 'Ticket Quota', 'size': 200, 'items': [item.pk], 'variations': [variations[0].pk], 'subevent': subevent.pk},
        format='json',
    )
    assert resp.status_code == 201
    ...
    resp = token_client.post(
        '/api/v1/organizers/{}/events/{}/quotas/'.format(organizer.slug, event.slug),
        {'name': 'Ticket Quota', 'size': 200, 'items': [item.pk], 'variations': [variations[0].pk], 'subevent': subevent2.pk},
        format='json',
    )
    assert resp.status_code == 400
    ...
    resp = token_client.post(
        ...
        {'name': 'Ticket Quota', 'size': 200, 'items': [], 'variations': [], 'subevent': subevent2.pk},
        format='json',
    )
    assert resp.status_code == 400
    assert resp.content.decode() == '{"non_field_errors":["The subevent does not belong to this event."]}'
...
def test_quota_availability(token_client, organizer, event, quota, item):
    resp = token_client.get('/api/v1/organizers/{}/events/{}/quotas/{}/availability/'.format(organizer.slug, event.slug, quota.pk))
    assert resp.status_code == 200
    assert {
        'blocking_vouchers': 0,
...
def question(event, item):
    q = event.questions.create(question='T-Shirt size', type='C', identifier='ABC', help_text='This is an example question')
    q.items.add(item)
...
def test_question_list(token_client, organizer, event, question, item):
    ...
    resp = token_client.get('/api/v1/organizers/{}/events/{}/questions/?required=false'.format(organizer.slug, event.slug))
    assert [res] == resp.data['results']
    resp = token_client.get('/api/v1/organizers/{}/events/{}/questions/?ask_during_checkin=false'.format(organizer.slug, event.slug))
    assert [res] == resp.data['results']
    resp = token_client.get('/api/v1/organizers/{}/events/{}/questions/?identifier=ABC'.format(organizer.slug, event.slug))
    assert [res] == resp.data['results']
    resp = token_client.get('/api/v1/organizers/{}/events/{}/questions/?required=true'.format(organizer.slug, event.slug))
    assert [] == resp.data['results']
    resp = token_client.get('/api/v1/organizers/{}/events/{}/questions/?ask_during_checkin=true'.format(organizer.slug, event.slug))
    assert [] == resp.data['results']
    resp = token_client.get('/api/v1/organizers/{}/events/{}/questions/?identifier=DEF'.format(organizer.slug, event.slug))
    assert [] == resp.data['results']
...
def test_question_detail(token_client, organizer, event, question, item):
    ...
    resp = token_client.get('/api/v1/organizers/{}/events/{}/questions/{}/'.format(organizer.slug, event.slug, question.pk))
    assert resp.status_code == 200
    assert res == resp.data
...
def test_question_create(token_client, organizer, event, event2, item):
    resp = token_client.post(
        ...
        {'question': "What's your name?", 'type': 'S', 'required': True, 'items': [item.pk], 'position': 0, 'ask_during_checkin': False, 'identifier': None},
        format='json',
    )
    assert resp.status_code == 201
    with scopes_disabled():
...
def test_question_update_circular_dependency(token_client, organizer, event, question):
    ...
    resp = token_client.patch(
        '/api/v1/organizers/{}/events/{}/questions/{}/'.format(organizer.slug, event.slug, question.pk), {'dependency_question': q2.pk}, format='json'
    )
    assert resp.status_code == 400
...
def test_question_self_dependency(token_client, organizer, event, question):
    resp = token_client.patch(
        '/api/v1/organizers/{}/events/{}/questions/{}/'.format(organizer.slug, event.slug, question.pk), {'dependency_question': question.pk}, format='json'
    )
    assert resp.status_code == 400
...
def test_question_update_options(token_client, organizer, event, question, item):
    resp = token_client.patch('/api/v1/organizers/{}/events/{}/questions/{}/'.format(organizer.slug, event.slug, question.pk), {'options': []}, format='json')
    assert resp.status_code == 400
...
def test_question_update_dependency_values(token_client, organizer, event, question):
    resp = token_client.patch(
        '/api/v1/organizers/{}/events/{}/questions/{}/'.format(organizer.slug, event.slug, question.pk), {'dependency_values': ['a', 'b']}, format='json'
    )
    assert resp.status_code == 200
...
def test_question_update_dependency_value_legacy(token_client, organizer, event, question):
    resp = token_client.patch(
        '/api/v1/organizers/{}/events/{}/questions/{}/'.format(organizer.slug, event.slug, question.pk), {'dependency_value': 'a'}, format='json'
    )
    assert resp.status_code == 200
    question.refresh_from_db()
...
def test_options_list(token_client, organizer, event, question, option):
    ...
    res['id'] = option.pk
    resp = token_client.get('/api/v1/organizers/{}/events/{}/questions/{}/options/'.format(organizer.slug, event.slug, question.pk))
    assert resp.status_code == 200
...
def test_options_detail(token_client, organizer, event, question, option):
    res = dict(TEST_OPTIONS_RES)
    res['id'] = option.pk
    resp = token_client.get('/api/v1/organizers/{}/events/{}/questions/{}/options/{}/'.format(organizer.slug, event.slug, question.pk, option.pk))
    assert resp.status_code == 200
    assert res == resp.data
...
def test_options_delete(token_client, organizer, event, question, option):
    resp = token_client.delete('/api/v1/organizers/{}/events/{}/questions/{}/options/{}/'.format(organizer.slug, event.slug, question.pk, option.pk))
    assert resp.status_code == 204
    with scopes_disabled():
