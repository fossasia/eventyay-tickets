...
def test_wle_list(token_client, organizer, event, wle, item, subevent):
    ...
    resp = token_client.get('/api/v1/organizers/{}/events/{}/waitinglistentries/?item={}'.format(organizer.slug, event.slug, item.pk))
    assert [res] == resp.data['results']
    resp = token_client.get('/api/v1/organizers/{}/events/{}/waitinglistentries/?item={}'.format(organizer.slug, event.slug, i2.pk))
    assert [] == resp.data['results']

    resp = token_client.get('/api/v1/organizers/{}/events/{}/waitinglistentries/?variation={}'.format(organizer.slug, event.slug, var.pk))
    assert [res] == resp.data['results']
    resp = token_client.get('/api/v1/organizers/{}/events/{}/waitinglistentries/?variation={}'.format(organizer.slug, event.slug, var2.pk))
    assert [] == resp.data['results']

    resp = token_client.get('/api/v1/organizers/{}/events/{}/waitinglistentries/?email=waiting@example.org'.format(organizer.slug, event.slug))
    assert [res] == resp.data['results']
    resp = token_client.get('/api/v1/organizers/{}/events/{}/waitinglistentries/?email=foo@bar.sample'.format(organizer.slug, event.slug))
    assert [] == resp.data['results']

    resp = token_client.get('/api/v1/organizers/{}/events/{}/waitinglistentries/?locale=en'.format(organizer.slug, event.slug))
    assert [res] == resp.data['results']
    resp = token_client.get('/api/v1/organizers/{}/events/{}/waitinglistentries/?locale=de'.format(organizer.slug, event.slug))
    assert [] == resp.data['results']

    resp = token_client.get('/api/v1/organizers/{}/events/{}/waitinglistentries/?has_voucher=false'.format(organizer.slug, event.slug))
    assert [res] == resp.data['results']
    resp = token_client.get('/api/v1/organizers/{}/events/{}/waitinglistentries/?has_voucher=true'.format(organizer.slug, event.slug))
    assert [] == resp.data['results']
    ...
    resp = token_client.get('/api/v1/organizers/{}/events/{}/waitinglistentries/?subevent={}'.format(organizer.slug, event.slug, subevent.pk))
    assert [res] == resp.data['results']
    with scopes_disabled():
        se2 = event.subevents.create(name='Foobar', date_from=datetime.datetime(2017, 12, 27, 10, 0, 0, tzinfo=UTC))
    resp = token_client.get('/api/v1/organizers/{}/events/{}/waitinglistentries/?subevent={}'.format(organizer.slug, event.slug, se2.pk))
    assert [] == resp.data['results']
...
def test_wle_detail(token_client, organizer, event, wle, item):
    ...
    res['id'] = wle.pk
    res['item'] = item.pk
    resp = token_client.get('/api/v1/organizers/{}/events/{}/waitinglistentries/{}/'.format(organizer.slug, event.slug, wle.pk))
    assert resp.status_code == 200
...
def create_wle(token_client, organizer, event, data, expected_failure=False):
    resp = token_client.post('/api/v1/organizers/{}/events/{}/waitinglistentries/'.format(organizer.slug, event.slug), data=data, format='json')
    if expected_failure:
...
def test_wle_create_success(token_client, organizer, event, item, quota):
    w = create_wle(
        ...
        organizer,
        event,
        data={'email': 'test@eventyay.com', 'item': item.pk, 'variation': None, 'locale': 'en', 'subevent': None},
...
def test_wle_require_fields(token_client, organizer, event, item, quota):
    create_wle(token_client, organizer, event, data={'item': item.pk, 'variation': None, 'locale': 'en', 'subevent': None}, expected_failure=True)
    create_wle(token_client, organizer, event, data={'email': 'test@eventyay.com', 'variation': None, 'locale': 'en', 'subevent': None}, expected_failure=True)
    with scopes_disabled():
    ...
    create_wle(
        ...
        organizer,
        event,
        data={'email': 'test@eventyay.com', 'item': item.pk, 'variation': v.pk, 'locale': 'en', 'subevent': None},
        expected_failure=True,
    )
...
def change_wle(token_client, organizer, event, wle, data, expected_failure=False):
    resp = token_client.patch('/api/v1/organizers/{}/events/{}/waitinglistentries/{}/'.format(organizer.slug, event.slug, wle.pk), data=data, format='json')
    if expected_failure:
...
def test_wle_change_to_unavailable_item(token_client, organizer, event, item, wle, quota):
    ...
    q.items.add(i)
    q.variations.add(v)
    change_wle(token_client, organizer, event, wle, data={'item': i.pk, 'variation': v.pk}, expected_failure=False)
    assert wle.item == i
...
def test_wle_send_voucher(token_client, organizer, event, item, wle, quota):
    ...
    resp = token_client.post(
        '/api/v1/organizers/{}/events/{}/waitinglistentries/{}/send_voucher/'.format(organizer.slug, event.slug, wle.pk), data={}, format='json'
    )
    assert resp.status_code == 204
    wle.refresh_from_db()
    assert wle.voucher
