...
def test_list_list(token_client, organizer, event, clist, item, subevent):
    ...
    resp = token_client.get('/api/v1/organizers/{}/events/{}/checkinlists/?subevent={}'.format(organizer.slug, event.slug, subevent.pk))
    assert [res] == resp.data['results']
    resp = token_client.get('/api/v1/organizers/{}/events/{}/checkinlists/?subevent_match={}'.format(organizer.slug, event.slug, subevent.pk))
    assert [res] == resp.data['results']
    with scopes_disabled():
        se2 = event.subevents.create(name='Foobar', date_from=datetime.datetime(2017, 12, 27, 10, 0, 0, tzinfo=UTC))
    resp = token_client.get('/api/v1/organizers/{}/events/{}/checkinlists/?subevent={}'.format(organizer.slug, event.slug, se2.pk))
    assert [] == resp.data['results']
    resp = token_client.get('/api/v1/organizers/{}/events/{}/checkinlists/?subevent_match={}'.format(organizer.slug, event.slug, se2.pk))
    assert [] == resp.data['results']
...
def test_list_detail(token_client, organizer, event, clist, item):
    ...
    res['limit_products'] = [item.pk]
    res['auto_checkin_sales_channels'] = []
    resp = token_client.get('/api/v1/organizers/{}/events/{}/checkinlists/{}/'.format(organizer.slug, event.slug, clist.pk))
    assert resp.status_code == 200
...
def test_list_create(token_client, organizer, event, item, item_on_wrong_event):
    resp = token_client.post(
        ...
        {'name': 'VIP', 'limit_products': [item.pk], 'all_products': False, 'subevent': None, 'rules': {'==': [0, 1]}},
        format='json',
    )
    ...
    resp = token_client.post(
        '/api/v1/organizers/{}/events/{}/checkinlists/'.format(organizer.slug, event.slug),
        {'name': 'VIP', 'limit_products': [item.pk], 'all_products': False, 'subevent': None, 'auto_checkin_sales_channels': ['web']},
        format='json',
    )
    assert resp.status_code == 201
    with scopes_disabled():
    ...
    resp = token_client.post(
        '/api/v1/organizers/{}/events/{}/checkinlists/'.format(organizer.slug, event.slug),
        {'name': 'VIP', 'limit_products': [item_on_wrong_event.pk], 'all_products': True, 'subevent': None},
        format='json',
    )
...
def test_list_create_with_subevent(token_client, organizer, event, event3, item, subevent, subevent2):
    resp = token_client.post(
        '/api/v1/organizers/{}/events/{}/checkinlists/'.format(organizer.slug, event.slug),
        {'name': 'VIP', 'limit_products': [item.pk], 'all_products': True, 'subevent': subevent.pk},
        format='json',
    )
    assert resp.status_code == 201
    ...
    resp = token_client.post(
        '/api/v1/organizers/{}/events/{}/checkinlists/'.format(organizer.slug, event.slug),
        {'name': 'VIP', 'limit_products': [item.pk], 'all_products': True, 'subevent': subevent.pk, 'auto_checkin_sales_channels': ['web']},
        format='json',
    )
    assert resp.status_code == 201
    with scopes_disabled():
    ...
    resp = token_client.post(
        '/api/v1/organizers/{}/events/{}/checkinlists/'.format(organizer.slug, event.slug),
        {'name': 'VIP', 'limit_products': [item.pk], 'all_products': True, 'subevent': None},
        format='json',
    )
    assert resp.status_code == 201
    ...
    resp = token_client.post(
        '/api/v1/organizers/{}/events/{}/checkinlists/'.format(organizer.slug, event.slug),
        {'name': 'VIP', 'limit_products': [], 'all_products': True, 'subevent': subevent2.pk},
        format='json',
    )
    assert resp.status_code == 400
...
def test_list_all_items_positions(token_client, organizer, event, clist, clist_all, item, other_item, order):
    ...
    # All items
    resp = token_client.get('/api/v1/organizers/{}/events/{}/checkinlists/{}/positions/?ordering=positionid'.format(organizer.slug, event.slug, clist_all.pk))
    assert resp.status_code == 200
    ...
    assert resp.status_code == 200
    assert [p1, p2] == resp.data['results']
    resp = token_client.get('/api/v1/organizers/{}/events/{}/checkinlists/{}/positions/?has_checkin=1'.format(organizer.slug, event.slug, clist_all.pk))
    assert resp.status_code == 200
    assert [] == resp.data['results']
    ...
    # Only not checked in
    resp = token_client.get('/api/v1/organizers/{}/events/{}/checkinlists/{}/positions/?has_checkin=0'.format(organizer.slug, event.slug, clist_all.pk))
    assert resp.status_code == 200
    assert [p2] == resp.data['results']

    # Order by checkin
    resp = token_client.get(
        '/api/v1/organizers/{}/events/{}/checkinlists/{}/positions/?ordering=-last_checked_in'.format(organizer.slug, event.slug, clist_all.pk)
    )
    assert resp.status_code == 200
    assert [p1, p2] == resp.data['results']
    resp = token_client.get(
    ...
    resp = token_client.get(
        '/api/v1/organizers/{}/events/{}/checkinlists/{}/positions/?ordering=-attendee_name'.format(organizer.slug, event.slug, clist_all.pk)
    )
    assert resp.status_code == 200
    assert [p1, p2] == resp.data['results']
    resp = token_client.get(
    ...
    order.save()
    resp = token_client.get('/api/v1/organizers/{}/events/{}/checkinlists/{}/positions/'.format(organizer.slug, event.slug, clist_all.pk))
    assert resp.status_code == 200
    assert [] == resp.data['results']

    resp = token_client.get('/api/v1/organizers/{}/events/{}/checkinlists/{}/positions/?ignore_status=true'.format(organizer.slug, event.slug, clist_all.pk))
    assert resp.status_code == 200
    p1['order__status'] = 'n'
    p2['order__status'] = 'n'
    assert [p2, p1] == resp.data['results']
...
def test_list_limited_items_positions(token_client, organizer, event, clist, item, order):
    ...
    # All items
    resp = token_client.get('/api/v1/organizers/{}/events/{}/checkinlists/{}/positions/?ordering=positionid'.format(organizer.slug, event.slug, clist.pk))
    assert resp.status_code == 200
    assert [p1] == resp.data['results']
...
def test_list_limited_items_position_detail(token_client, organizer, event, clist, item, order):
    ...
    # All items
    resp = token_client.get('/api/v1/organizers/{}/events/{}/checkinlists/{}/positions/{}/'.format(organizer.slug, event.slug, clist.pk, p1['id']))
    assert resp.status_code == 200
    assert p1 == resp.data
...
def test_status(token_client, organizer, event, clist_all, item, other_item, order):
    ...
    assert resp.data['items'] == [
        ...
        {'name': other_item.name, 'id': other_item.pk, 'checkin_count': 0, 'admission': False, 'position_count': 1, 'variations': []},
    ]
...
def test_custom_datetime(token_client, organizer, clist, event, order):
    ...
    resp = token_client.post(
        '/api/v1/organizers/{}/events/{}/checkinlists/{}/positions/{}/redeem/'.format(organizer.slug, event.slug, clist.pk, p),
...
def test_name_fallback(token_client, organizer, clist, event, order):
    ...
    op.save()
    resp = token_client.post(
        '/api/v1/organizers/{}/events/{}/checkinlists/{}/positions/{}/redeem/'.format(organizer.slug, event.slug, clist.pk, op.pk), {}, format='json'
    )
    assert resp.status_code == 201
    assert resp.data['status'] == 'ok'
...
def test_by_secret(token_client, organizer, clist, event, order):
    ...
    resp = token_client.post(
        '/api/v1/organizers/{}/events/{}/checkinlists/{}/positions/{}/redeem/'.format(organizer.slug, event.slug, clist.pk, p.secret), {}, format='json'
    )
    assert resp.status_code == 201
    assert resp.data['status'] == 'ok'
...
def test_by_secret_special_chars(token_client, organizer, clist, event, order):
    ...
    p.save()
    resp = token_client.post(
        '/api/v1/organizers/{}/events/{}/checkinlists/{}/positions/{}/redeem/'.format(organizer.slug, event.slug, clist.pk, urlquote(p.secret, safe='')),
        {},
        format='json',
    )
    assert resp.status_code == 201
...
def test_only_once(token_client, organizer, clist, event, order):
    with scopes_disabled():
        p = order.positions.first()
    resp = token_client.post(
        '/api/v1/organizers/{}/events/{}/checkinlists/{}/positions/{}/redeem/'.format(organizer.slug, event.slug, clist.pk, p.pk), {}, format='json'
    )
    assert resp.status_code == 201
    assert resp.data['status'] == 'ok'
    resp = token_client.post(
...
def test_reupload_same_nonce(token_client, organizer, clist, event, order):
    with scopes_disabled():
        p = order.positions.first()
    resp = token_client.post(
        '/api/v1/organizers/{}/events/{}/checkinlists/{}/positions/{}/redeem/'.format(organizer.slug, event.slug, clist.pk, p.pk),
        {'nonce': 'foobar'},
        format='json',
    )
    assert resp.status_code == 201
...
def test_multiple_different_list(token_client, organizer, clist, clist_all, event, order):
    ...
    assert resp.data['status'] == 'ok'
    resp = token_client.post(
        '/api/v1/organizers/{}/events/{}/checkinlists/{}/positions/{}/redeem/'.format(organizer.slug, event.slug, clist_all.pk, p.pk),
        {'nonce': 'baz'},
        format='json',
    )
    assert resp.status_code == 201
