...
TEST_TAXRULE_RES = {'name': {'en': 'VAT'}, 'rate': '19.00', 'price_includes_tax': True, 'eu_reverse_charge': False, 'home_country': ''}
...
def test_rule_detail(token_client, organizer, event, taxrule):
    res = dict(TEST_TAXRULE_RES)
    res['id'] = taxrule.pk
    resp = token_client.get('/api/v1/organizers/{}/events/{}/taxrules/{}/'.format(organizer.slug, event.slug, taxrule.pk))
    assert resp.status_code == 200
...
def test_rule_create(token_client, organizer, event):
    resp = token_client.post(
        ...
        {'name': {'en': 'VAT', 'de': 'MwSt'}, 'rate': '19.00', 'price_includes_tax': True, 'eu_reverse_charge': False, 'home_country': 'DE'},
        format='json',
    )
