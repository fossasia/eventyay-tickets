...
def test_initialize_unknown_token(client, new_device: Device):
    resp = client.post(
        ...
        {'token': 'aaa', 'hardware_brand': 'Samsung', 'hardware_model': 'Galaxy S', 'software_brand': 'pretixdroid', 'software_version': '4.0.0'},
    )
    assert resp.status_code == 400
...
def test_update_required_auth(client, token_client, device: Device):
    resp = client.post(
        '/api/v1/device/update', {'hardware_brand': 'Samsung', 'hardware_model': 'Galaxy S', 'software_brand': 'pretixdroid', 'software_version': '5.0.0'}
    )
