...
def test_team_create(token_client, organizer, event):
    resp = token_client.post('/api/v1/organizers/{}/teams/'.format(organizer.slug), TEST_TEAM_CREATE_PAYLOAD, format='json')
...
def test_team_delete(token_client, organizer, event, second_team):
    resp = token_client.delete('/api/v1/organizers/{}/teams/{}/'.format(organizer.slug, second_team.pk), format='json')
    assert resp.status_code == 204
    assert organizer.teams.count() == 1


TEST_TEAM_MEMBER_RES = {'email': 'dummy@dummy.dummy', 'fullname': None, 'require_2fa': False}
...
def test_team_invites_create(token_client, organizer, event, team, user):
    resp = token_client.post('/api/v1/organizers/{}/teams/{}/invites/'.format(organizer.slug, team.pk), {'email': 'newmail@dummy.dummy'})
    assert resp.status_code == 201
    ...
    resp = token_client.post('/api/v1/organizers/{}/teams/{}/invites/'.format(organizer.slug, team.pk), {'email': user.email})
    assert resp.status_code == 201
...
def test_team_tokens_detail(token_client, organizer, event, second_team, token):
    res = dict(TEST_TEAM_TOKEN_RES)
    res['id'] = token.pk
    resp = token_client.get('/api/v1/organizers/{}/teams/{}/tokens/{}/'.format(organizer.slug, second_team.pk, token.pk))
    assert resp.status_code == 200
    assert res == resp.data
...
def test_team_tokens_delete(token_client, organizer, event, second_team, token):
    resp = token_client.delete('/api/v1/organizers/{}/teams/{}/tokens/{}/'.format(organizer.slug, second_team.pk, token.pk))
    assert resp.status_code == 200
...
def test_team_token_create(token_client, organizer, event, second_team):
    resp = token_client.post('/api/v1/organizers/{}/teams/{}/tokens/'.format(organizer.slug, second_team.pk), {'name': 'New token'})
    assert resp.status_code == 201
