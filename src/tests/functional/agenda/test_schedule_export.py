...
def test_schedule_frab_xml_export(slot, client, schedule_schema):
    response = client.get(reverse(f'agenda:XML (frab)', kwargs={'event': slot.submission.event.slug}), follow=True)
...
def test_schedule_frab_json_export(slot, answered_choice_question, personal_answer, client, orga_user, schedule_schema):
    regular_response = client.get(reverse(f'agenda:JSON (frab)', kwargs={'event': slot.submission.event.slug}), follow=True)
    client.force_login(orga_user)
    orga_response = client.get(reverse(f'agenda:JSON (frab)', kwargs={'event': slot.submission.event.slug}), follow=True)
...
def test_schedule_frab_xcal_export(slot, client, schedule_schema):
    response = client.get(reverse(f'agenda:XCal (frab)', kwargs={'event': slot.submission.event.slug}), follow=True)
    assert response.status_code == 200
