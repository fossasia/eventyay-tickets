...
class WidgetCartTest(CartTestMixin, TestCase):
    ...
    def test_allow_frame_on_order(self):
        ...
        assert 'X-Frame-Options' not in response
        response = self.client.get('/%s/%s/order/%s/%s/pay/change' % (self.orga.slug, self.event.slug, self.order.code, self.order.secret))
        assert 'X-Frame-Options' not in response
    ...
    def test_allow_cors_if_namespaced(self):
        response = self.client.post('/%s/%s/cart/add' % (self.orga.slug, self.event.slug), {'item_%d' % self.ticket.id: '1', 'ajax': 1})
        assert 'Access-Control-Allow-Origin' not in response
        response = self.client.post('/%s/%s/w/aaaaaaaaaaaaaaaa/cart/add' % (self.orga.slug, self.event.slug), {'item_%d' % self.ticket.id: '1', 'ajax': 1})
    ...
    def test_cart_isolation(self):
        ...
        response = self.client.post('/%s/%s/w/aaaaaaaaaaaaaaaa/cart/add' % (self.orga.slug, self.event.slug), {'item_%d' % self.ticket.id: '1'}, follow=True)
        self.assertRedirects(response, '/%s/%s/w/aaaaaaaaaaaaaaaa/?require_cookie=true' % (self.orga.slug, self.event.slug), target_status_code=200)
        doc = BeautifulSoup(response.rendered_content, 'lxml')
        assert len(doc.select('.cart .cart-row')) == 2
        self.assertIn('Early-bird', doc.select('.cart .cart-row')[0].select('strong')[0].text)
        self.assertIn('1', doc.select('.cart .cart-row')[0].select('.count')[0].text)
        ...
        doc = BeautifulSoup(response.rendered_content, 'lxml')
        assert len(doc.select('.cart .cart-row')) == 0
        response = self.client.post('/%s/%s/w/aaaaaaaaaaaaaaab/cart/add' % (self.orga.slug, self.event.slug), {'item_%d' % self.ticket.id: '1'}, follow=True)
        self.assertRedirects(response, '/%s/%s/w/aaaaaaaaaaaaaaab/?require_cookie=true' % (self.orga.slug, self.event.slug), target_status_code=200)
        doc = BeautifulSoup(response.rendered_content, 'lxml')
        assert len(doc.select('.cart .cart-row')) == 2
        self.assertIn('Early-bird', doc.select('.cart .cart-row')[0].select('strong')[0].text)
        self.assertIn('1', doc.select('.cart .cart-row')[0].select('.count')[0].text)
                            ...
                            'variations': [
                                {
                                    ...
                                    'original_price': None,
                                    'price': {'gross': '14.00', 'net': '11.76', 'tax': '2.24', 'name': '', 'rate': '19.00', 'includes_mixed_tax_rate': False},
                                    'description': None,
                                    'avail': [100, None],
                                    'order_max': 2,
                                },
                                {
                                    ...
                                    'original_price': None,
                                    'price': {'gross': '12.00', 'net': '10.08', 'tax': '1.92', 'name': '', 'rate': '19.00', 'includes_mixed_tax_rate': False},
                                    'description': None,
                                    'avail': [100, None],
                                    'order_max': 2,
                                },
    ...
    def test_product_list_view_filter(self):
        response = self.client.get('/%s/%s/widget/product_list?items=%s' % (self.orga.slug, self.event.slug, self.ticket.pk))
        assert response['Access-Control-Allow-Origin'] == '*'
        data = json.loads(response.content.decode())
        ...
        response = self.client.get('/%s/%s/widget/product_list?categories=0,%s' % (self.orga.slug, self.event.slug, self.category.pk))
        assert response['Access-Control-Allow-Origin'] == '*'
        data = json.loads(response.content.decode())
                            ...
                            'variations': [
                                {
                                    ...
                                    'description': None,
                                    'original_price': None,
                                    'price': {'gross': '14.00', 'net': '11.76', 'tax': '2.24', 'rate': '19.00', 'name': '', 'includes_mixed_tax_rate': False},
                                    'avail': [100, None],
                                },
                            ],
                        }
    ...
    def test_product_list_view_with_bundle_sold_out(self):
        ...
        self.ticket.bundles.create(bundled_item=self.shirt, bundled_variation=self.shirt_blue, designated_price=2, count=1)
        response = self.client.get('/%s/%s/widget/product_list' % (self.orga.slug, self.event.slug))
        assert response['Access-Control-Allow-Origin'] == '*'
        data = json.loads(response.content.decode())
