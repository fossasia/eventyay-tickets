...
class ItemDisplayTest(EventTestMixin, SoupTest):
    def test_link_rewrite(self):
        with scopes_disabled():
            ...
            item = Item.objects.create(
                event=self.event, name='Early-bird ticket', default_price=0, active=True, description='http://example.org [Sample](http://example.net)'
            )
            q.items.add(item)
    ...
    def test_sales_channel(self):
        with scopes_disabled():
            q = Quota.objects.create(event=self.event, name='Quota', size=2)
            item = Item.objects.create(event=self.event, name='Early-bird ticket', default_price=0, active=True, sales_channels=['bar'])
            q.items.add(item)
        html = self.client.get('/%s/%s/' % (self.orga.slug, self.event.slug)).rendered_content
        self.assertNotIn('Early-bird', html)
    ...
    def test_no_longer_available(self):
        with scopes_disabled():
            q = Quota.objects.create(event=self.event, name='Quota', size=2)
            item = Item.objects.create(
                event=self.event, name='Early-bird ticket', default_price=0, active=True, available_until=now() - datetime.timedelta(days=2)
            )
            q.items.add(item)
        html = self.client.get('/%s/%s/' % (self.orga.slug, self.event.slug)).rendered_content
        self.assertNotIn('Early-bird', html)
    ...
    def test_not_yet_available(self):
        with scopes_disabled():
            q = Quota.objects.create(event=self.event, name='Quota', size=2)
            item = Item.objects.create(
                event=self.event, name='Early-bird ticket', default_price=0, active=True, available_from=now() + datetime.timedelta(days=2)
            )
        q.items.add(item)
        html = self.client.get('/%s/%s/' % (self.orga.slug, self.event.slug)).rendered_content
        self.assertNotIn('Early-bird', html)
    ...
    def test_hidden_without_voucher(self):
        with scopes_disabled():
            q = Quota.objects.create(event=self.event, name='Quota', size=2)
            item = Item.objects.create(event=self.event, name='Early-bird ticket', default_price=0, active=True, hide_without_voucher=True)
        q.items.add(item)
        html = self.client.get('/%s/%s/' % (self.orga.slug, self.event.slug)).rendered_content
        self.assertNotIn('Early-bird', html)
    ...
    def test_subevent_calendar(self):
        ...
        resp = self.client.get('/%s/%s/?year=%d&month=%d' % (self.orga.slug, self.event.slug, se1.date_from.year, se1.date_from.month))
        self.assertIn('Foo SE1', resp.rendered_content)
        self.assertNotIn('Foo SE2', resp.rendered_content)
    ...
    def test_subevent_week_calendar(self):
        ...
        self.event.save()
        with scopes_disabled():
            se1 = self.event.subevents.create(name='Foo SE1', date_from=now() + datetime.timedelta(days=24), active=True)
            self.event.subevents.create(name='Foo SE2', date_from=now() + datetime.timedelta(days=12), active=True)
        resp = self.client.get('/%s/%s/' % (self.orga.slug, self.event.slug))
        print(resp.rendered_content)
        self.assertIn('Foo SE2', resp.rendered_content)
        self.assertNotIn('Foo SE1', resp.rendered_content)
        resp = self.client.get('/%s/%s/?year=%d&week=%d' % (self.orga.slug, self.event.slug, se1.date_from.isocalendar()[0], se1.date_from.isocalendar()[1]))
        self.assertIn('Foo SE1', resp.rendered_content)
        self.assertNotIn('Foo SE2', resp.rendered_content)
    ...
    def test_subevent_net_prices(self):
        ...
        with scopes_disabled():
            ...
            item = Item.objects.create(event=self.event, name='Early-bird ticket', default_price=15, tax_rule=tr)
            q = Quota.objects.create(event=self.event, name='Quota', size=2, subevent=se1)
            q.items.add(item)
            q = Quota.objects.create(event=self.event, name='Quota', size=2, subevent=se2)
            q.items.add(item)
    ...
    def test_hidden_if_available(self):
        with scopes_disabled():
            ...
            item = Item.objects.create(event=self.event, name='Early-bird ticket', default_price=12)
            item2 = Item.objects.create(event=self.event, name='Late-bird ticket', default_price=12, hidden_if_available=q)
            q.items.add(item)
            q2.items.add(item2)
        self.event.settings.hide_sold_out = True
...
class VoucherRedeemItemDisplayTest(EventTestMixin, SoupTest):
    ...
    def setUp(self):
        ...
        self.item = Item.objects.create(event=self.event, name='Early-bird ticket', default_price=Decimal('12.00'), active=True)
    ...
    def test_subevent_net_prices(self):
        with scopes_disabled():
            ...
            self.v.save()
        html = self.client.get('/%s/%s/redeem?voucher=%s&subevent=%s' % (self.orga.slug, self.event.slug, self.v.code, se1.pk))
    ...
    def test_voucher_ignore_other_subevent(self):
        ...
        self.v.subevent = se1
        self.v.save()
        html = self.client.get('/%s/%s/redeem?voucher=%s&subevent=%s' % (self.orga.slug, self.event.slug, self.v.code, se2.pk))
        assert 'SE1' in html.rendered_content
...
class WaitingListTest(EventTestMixin, SoupTest):
    ...
    def test_submit_form(self):
        ...
        response = self.client.post('/%s/%s/waitinglist/?item=%d' % (self.orga.slug, self.event.slug, self.item.pk), {'email': 'foo@bar.com'})
        self.assertEqual(response.status_code, 302)
        with scopes_disabled():
    ...
    def test_subevent_valid(self):
        ...
        response = self.client.get('/%s/%s/waitinglist/?item=%d&subevent=%d' % (self.orga.slug, self.event.slug, self.item.pk, se1.pk))
        self.assertEqual(response.status_code, 200)
        self.assertIn('waiting list', response.rendered_content)
        response = self.client.post(
            '/%s/%s/waitinglist/?item=%d&subevent=%d' % (self.orga.slug, self.event.slug, self.item.pk, se1.pk), {'email': 'foo@bar.com'}
        )
        self.assertEqual(response.status_code, 302)
        with scopes_disabled():
            wle = WaitingListEntry.objects.get(email='foo@bar.com')
    ...
    def test_invalid_subevent(self):
        ...
        response = self.client.get('/%s/%s/waitinglist/?item=%d' % (self.orga.slug, self.event.slug, self.item.pk))
        self.assertEqual(response.status_code, 302)
        response = self.client.get('/%s/%s/waitinglist/?item=%d&subevent=%d' % (self.orga.slug, self.event.slug, self.item.pk, se1.pk + 100))
...
class DeadlineTest(EventTestMixin, TestCase):
    ...
    def test_event_over(self):
        ...
        self.assertIn('alert-info', response.rendered_content)
        self.assertNotIn('btn-add-to-cart', response.rendered_content)
        response = self.client.post('/%s/%s/cart/add' % (self.orga.slug, self.event.slug), {'item_%d' % self.item.id: '1'}, follow=True)
        self.assertIn('alert-danger', response.rendered_content)
    ...
    def test_not_set(self):
        ...
        response = self.client.post('/%s/%s/cart/add' % (self.orga.slug, self.event.slug), {'item_%d' % self.item.id: '1'})
...
class EventIcalDownloadTest(EventTestMixin, SoupTest):
    ...
    def test_response_type(self):
        ...
        self.assertEqual(ical['Content-Disposition'], 'attachment; filename="{}-{}-0.ics"'.format(self.orga.slug, self.event.slug))
    ...
    def test_include_timezone(self):
        ...
        fmt = '%Y%m%dT%H%M%S'
        self.assertIn(
            'DTSTART;TZID=%s:%s' % (self.event.settings.timezone, self.event.date_from.astimezone(timezone(self.event.settings.timezone)).strftime(fmt)),
            ical,
            'incorrect start time',
        )
        self.assertIn(
            'DTEND;TZID=%s:%s' % (self.event.settings.timezone, self.event.date_to.astimezone(timezone(self.event.settings.timezone)).strftime(fmt)),
            ical,
            'incorrect end time',
        )
...
class EventMicrodataTest(EventTestMixin, SoupTest):
    ...
    def test_date_to(self):
        ...
        self.event.settings.show_date_to = False
        md = self._get_json()
        self.assertNotIn(self.event.date_to.isoformat(), md, msg='Date To present when show date to setting is false')
...
class EventSlugBanlistValidatorTest(EventTestMixin, SoupTest):
    ...
    def test_slug_validation(self):
        event = Event(organizer=self.orga, name='download', slug='download', date_from=datetime.datetime(2013, 12, 26, tzinfo=datetime.timezone.utc), live=True)
