...
@pytest.mark.parametrize("url", ("widgets/schedule.js", "schedule/widget.json"))
...
def test_widget_data(
    ...
    with django_assert_num_queries(11):
        response = client.get(event.urls.schedule + "widget.json", follow=True)
...
def test_versioned_widget_data(client, event, schedule, slot):
    ...
    response = client.get(event.urls.schedule + f"widget.json?v={schedule.version}")
    assert response.status_code == 200


@pytest.mark.django_db
def test_bogus_versioned_widget_data(client, event, schedule, slot):
    response = client.get(event.urls.schedule + "widget.json?v=nopedinope")
    assert response.status_code == 200


@pytest.mark.django_db
def test_anon_cannot_access_wip_schedule(client, event, schedule, slot):
    response = client.get(event.urls.schedule + "widget.json?v=wip")
    assert response.status_code == 404


@pytest.mark.django_db
def test_orga_can_access_wip_schedule(orga_client, event, schedule, slot):
    response = orga_client.get(event.urls.schedule + "widget.json?v=wip")
    assert response.status_code == 200
