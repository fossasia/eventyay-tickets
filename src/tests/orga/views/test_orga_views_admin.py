...
def test_update_notice_displayed(client, user):
    ...
    assert 'pretalx automatically checks for updates in the background' not in r.content.decode()

    user.is_administrator = True
    user.save()
    r = client.get('/orga/', follow=True)
    assert 'pretalx automatically checks for updates in the background' in r.content.decode()
...
def test_settings(client, user):
    ...
    client.post('/orga/admin/update/', {'update_check_email': 'test@example.org', 'update_check_enabled': 'on'})
    gs = GlobalSettings()
    gs.settings.flush()
    assert gs.settings.update_check_enabled
    assert gs.settings.update_check_email

    client.post('/orga/admin/update/', {'update_check_email': '', 'update_check_enabled': ''})
    gs.settings.flush()
