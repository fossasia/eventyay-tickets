...
def retrieve_preview_information(event=None, event_id=None):
    ...
    with suppress(Exception):  # If this fails, just move on
        preview_card = fetch_preview_data(url, event)

    if preview_card:
        event.content["preview_card"] = preview_card
        event.save()

        event_data = asgiref.sync.async_to_sync(ChatService(event).create_event)(
