...
class ListMultipleChoiceField(serializers.MultipleChoiceField):
    def to_internal_value(self, data):
        ...
        internal_value_data = [super(serializers.MultipleChoiceField, self).to_internal_value(product) for product in data]

        return remove_duplicates_from_list(internal_value_data)

    def to_representation(self, value):
        representation_data = [self.choice_strings_to_values.get(str(product), product) for product in value]
