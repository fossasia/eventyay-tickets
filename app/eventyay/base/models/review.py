...
class ReviewScoreCategory(PretalxModel):
    event = models.ForeignKey(to='Event', related_name='score_categories', on_delete=models.CASCADE)
    name = I18nCharField()
    ...
    def _validate_independence(self):
        if not self.event.score_categories.exclude(pk=self.pk).filter(is_independent=False).exists():
            raise ValidationError(_('You need to keep at least one non-independent score category!'))
...
class ReviewScore(PretalxModel):
    category = models.ForeignKey(to=ReviewScoreCategory, related_name='scores', on_delete=models.CASCADE)
...
class ReviewManager(models.Manager):
    def get_queryset(self):
        ...
        return super().get_queryset().exclude(submission__state=SubmissionStates.DELETED)
...
class Review(PretalxModel):
    ...
    class Meta:
        ...
        rules_permissions = {
            ...
            'list_all': orga_can_change_submissions | (is_reviewer & can_view_all_reviews),
            'list_reviewers': orga_can_change_submissions | (is_reviewer & can_view_reviewer_names),
    ...
    def update_score(self):
        scores = self.scores.all().select_related('category').filter(category__in=self.submission.score_categories)
...
class ReviewPhase(OrderedModel, PretalxModel):
    ...
    event = models.ForeignKey(to='Event', related_name='review_phases', on_delete=models.CASCADE)
