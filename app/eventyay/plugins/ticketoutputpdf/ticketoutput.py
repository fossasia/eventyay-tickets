    ...
    def layout_map(self):
        if not hasattr(self.event, '_ticketoutputpdf_cache_layoutmap'):
            self.event._ticketoutputpdf_cache_layoutmap = {
                (bi.product_id, bi.sales_channel): bi.layout
                for bi in TicketLayoutProduct.objects.select_related('layout').filter(product__event=self.event)
            }
            ...
            for op in order.positions_with_tickets:
                layout = override_layout.send_chained(
                    ...
                    layout=self.layout_map.get(
                        (op.product_id, order.sales_channel),
                        self.layout_map.get((op.product_id, 'web'), self.default_layout),
                    ),
                )
