...
from eventyay.control.views import ChartContainingView
    ...
    def get_context_data(self, **kwargs):
        ...
        if not ctx['obp_data']:
            ...
            num_ordered = {
                p['product']: p['cnt']
                for p in (
                    opqs.filter(order__event=self.request.event).values('product').annotate(cnt=Count('id')).order_by()
                )
            }
            num_paid = {
                p['product']: p['cnt']
            ...
            product_names = {i.id: str(i) for i in Product.objects.filter(event=self.request.event)}
        ...
        if not self.request.event.has_subevents or (ckey != 'all' and subevent):
            ...
            if ev.seating_plan_id is not None:
                ...
                product_cache = {
                    i.pk: i
                    for i in self.request.event.products.annotate(has_variations=Count('variations')).filter(
