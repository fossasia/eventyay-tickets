...
class SubEventProductOrVariationFormMixin:
    def __init__(self, *args, **kwargs):
        self.product = kwargs.pop('product')
        self.variation = kwargs.pop('variation', None)
        super().__init__(*args, **kwargs)
        change_decimal_field(self.fields['price'], self.product.event.currency)


class SubEventProductForm(SubEventProductOrVariationFormMixin, forms.ModelForm):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['price'].widget.attrs['placeholder'] = money_filter(
            self.product.default_price, self.product.event.currency, hide_currency=True
        )
        self.fields['price'].label = str(self.product)
...
class SubEventProductVariationForm(SubEventProductOrVariationFormMixin, forms.ModelForm):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['price'].widget.attrs['placeholder'] = money_filter(
...
class QuotaFormSet(I18nInlineFormSet):
    ...
    @cached_property
    def products(self):
        return self.event.products.prefetch_related('variations').all()
