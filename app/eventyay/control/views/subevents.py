...
from eventyay.base.models import CartPosition, LogEntry
...
from eventyay.control.forms.checkin import SimpleCheckinListForm
from eventyay.control.forms.filter import SubEventFilterForm
from eventyay.control.forms.product import QuotaForm
from eventyay.control.forms.subevents import (
    CheckinListFormSet,
    QuotaFormSet,
    RRuleFormSet,
...
from eventyay.control.signals import subevent_forms
from eventyay.control.views import PaginationMixin
from eventyay.control.views.event import MetaDataEditorMixin
from eventyay.helpers import GroupConcat
...
class SubEventDelete(EventPermissionRequiredMixin, DeleteView):
    ...
    def get(self, request, *args, **kwargs):
        if not self.get_object().allow_delete():
            messages.error(
                ...
                pgettext_lazy(
                    'subevent',
                    'A date can not be deleted if orders already have been placed.',
                ),
            )
    ...
    def form_valid(self, form):
        ...
        if not self.object.allow_delete():
            ...
            messages.success(
                self.request,
                pgettext_lazy('subevent', 'The selected date has been deleted.'),
            )
...
class SubEventEditorMixin(MetaDataEditorMixin):
    ...
    def plugin_forms(self):
        forms = []
        for rec, resp in subevent_forms.send(
    ...
    def formset(self):
        ...
        if self.object and self.object.pk:
            kwargs['queryset'] = self.object.quotas.prefetch_related('products', 'variations')
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx['productvar_forms'] = self.productvar_forms
    ...
    @cached_property
    def productvar_forms(self):
        if self.object and self.object.pk:
            se_product_instances = {sei.product_id: sei for sei in SubEventProduct.objects.filter(subevent=self.object)}
            se_var_instances = {
                sei.variation_id: sei for sei in SubEventProductVariation.objects.filter(subevent=self.object)
            }
        else:
            se_product_instances = {}
            se_var_instances = {}

        if self.copy_from:
            se_product_instances = {
                sei.product_id: SubEventProduct(product=sei.product, price=sei.price, disabled=sei.disabled)
        ...
        formlist = []
        for i in self.request.event.products.filter(active=True).prefetch_related('variations'):
            if i.has_variations:
    ...
    def is_valid(self, form):
        return (
            form.is_valid()
            and all([f.is_valid() for f in self.productvar_forms])
...
class SubEventUpdate(EventPermissionRequiredMixin, SubEventEditorMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        for f in self.productvar_forms:
            f.save()
            # TODO: LogEntry?
    ...
    def form_valid(self, form):
        ...
        for f in self.plugin_forms:
            data.update(
                {
                    k: (
                        f.cleaned_data.get(k).name if isinstance(f.cleaned_data.get(k), File) else f.cleaned_data.get(k)
                    )
...
class SubEventBulkAction(SubEventQueryMixin, EventPermissionRequiredMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        if request.POST.get('action') == 'disable':
            ...
            messages.success(
                request,
                pgettext_lazy('subevent', 'The selected dates have been disabled.'),
            )
            ...
            messages.success(
                request,
                pgettext_lazy('subevent', 'The selected dates have been enabled.'),
            )
            ...
            messages.success(
                request,
                pgettext_lazy('subevent', 'The selected dates have been deleted or disabled.'),
            )
    ...
    def form_valid(self, form):
                ...
                if t.get('time_to'):
                    se.date_to = (
                        ...
                        if t.get('time_to') > t.get('time_from')
                            ...
                            datetime.combine(rdate + timedelta(days=1), t['time_to']),
                            tz,
                        )
                    )
                else:
                    se.date_to = None
                se.date_admission = (
                    make_aware(datetime.combine(rdate, t['time_admission']), tz) if t.get('time_admission') else None
                )
                se.presale_start = (
        ...
        for f in self.productvar_forms:
            for se in subevents:
                ...
                i.pk = None
                i.subevent = se
                if isinstance(i, SubEventProduct):
                    to_save_products.append(i)
                else:
                    to_save_variations.append(i)
        SubEventProduct.objects.bulk_create(to_save_products)
        SubEventProductVariation.objects.bulk_create(to_save_variations)
        ...
        for f in self.formset.forms:
            ...
            for se in subevents:
                ...
                selected_products = set(
                    list(
                        self.request.event.products.filter(
                            id__in=[i.split('-')[0] for i in f.cleaned_data.get('productvars', [])]
                        )
                    )
                )
                selected_variations = list(
                    ProductVariation.objects.filter(
                        product__event=self.request.event,
                        id__in=[i.split('-')[1] for i in f.cleaned_data.get('productvars', []) if '-' in i],
                    )
                )
                for _i in selected_products:
                    to_save_products.append(Quota.products.through(quota_id=i.pk, product_id=_i.pk))
                for _i in selected_variations:
                    to_save_variations.append(Quota.variations.through(quota_id=i.pk, productvariation_id=_i.pk))
        ...
        Quota.products.through.objects.bulk_create(to_save_products)
        ...
        for f in self.cl_formset.forms:
            ...
            for se in subevents:
                ...
                for _i in f.cleaned_data.get('limit_products', []):
                    to_save_products.append(CheckinList.limit_products.through(checkinlist_id=i.pk, product_id=_i.pk))
        ...
        messages.success(
            self.request,
            pgettext_lazy('subevent', '{} new dates have been created.').format(len(subevents)),
        )
        return redirect(
            reverse(
                'control:event.subevents',
...
class SubEventBulkEdit(SubEventQueryMixin, EventPermissionRequiredMixin, FormView):
    ...
    def productvar_forms(self):
        ...
        for sei in (
            SubEventProduct.objects.filter(subevent__in=self.get_queryset())
            .order_by()
            .values('product', 'price', 'disabled')
            .annotate(c=Count('*'))
        ):
            matches['product', sei['product']].append(sei)
        for sei in (
            SubEventProductVariation.objects.filter(subevent__in=self.get_queryset())
            .order_by()
            ...
            if i.has_variations:
                for v in i.variations.all():
                    ...
                    if m and len(m) == 1 and m[0]['c'] == total:
                        inst = SubEventProductVariation(variation=v, disabled=m[0]['disabled'], price=m[0]['price'])
                    else:
                ...
                m = matches['product', i.pk]
                if m and len(m) == 1 and m[0]['c'] == total:
                    inst = SubEventProduct(product=i, disabled=m[0]['disabled'], price=m[0]['price'])
                else:
    ...
    def meta_forms(self):
        matches = defaultdict(list)
        for smv in (
            SubEventMetaValue.objects.filter(subevent__in=self.get_queryset())
            .order_by()
        ...
        for f in self.quota_formset.forms:
            ...
            if self.quota_formset._should_delete_form(f):
                ...
                for se in subevents:
                    ...
                    for _i in selected_products:
                        to_save_products.append(Quota.products.through(quota_id=q.pk, product_id=_i.pk))
                    for _i in selected_variations:
                        to_save_variations.append(Quota.variations.through(quota_id=q.pk, productvariation_id=_i.pk))
                ...
                if f.changed_data:
                    ...
                    for se in subevents:
                        ...
                        for fname in ('size', 'name', 'release_after_exit'):
                            setattr(q, fname, f.cleaned_data.get(fname))
                        q.save(clear_cache=False)
                        if 'productvars' in f.changed_data:
                            q.products.set(selected_products)
    ...
    def sampled_quotas(self):
        all_quotas = (
            ...
            .annotate(
                product_list=GroupConcat('products__id'),
    ...
    def sampled_lists(self):
        all_lists = (
            ...
            .annotate(
                product_list=GroupConcat('limit_products__id'),
            ...
            if f.cleaned_data.get('value'):
                for obj in self.get_queryset():
                    SubEventMetaValue.objects.update_or_create(
                        ...
                        subevent=obj,
                        defaults={'value': f.cleaned_data['value']},
                    )
            else:
                SubEventMetaValue.objects.filter(
                    property=f.instance.property, subevent__in=self.get_queryset()
                ).delete()

    def save_productvars(self):
        for f in self.productvar_forms:
            u = {}
            ...
            if isinstance(f, SubEventProductForm):
                if u.get('price') is None and not u.get('disabled'):
                    ...
                    for obj in self.get_queryset():
                        SubEventProduct.objects.update_or_create(subevent=obj, product=f.instance.product, defaults=u)
            elif isinstance(f, SubEventProductVariationForm):
                if u.get('price') is None and not u.get('disabled'):
                    ...
                    for obj in self.get_queryset():
                        SubEventProductVariation.objects.update_or_create(
                            subevent=obj, variation=f.instance.variation, defaults=u
                        )
