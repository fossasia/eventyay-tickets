...
from eventyay.base.services.shredder import export, shred
from eventyay.base.shredder import ShredError, shred_constraints
...
class ShredderMixin:
    ...
    def shredders(self):
        return OrderedDict(
            sorted(
                self.request.event.get_data_shredders().items(),
                key=lambda s: s[1].verbose_name,
            )
        )


class StartShredView(
