...
from eventyay.base.models.orders import CancellationRequest
from eventyay.control.forms.filter import OrderSearchFilterForm
from eventyay.control.views import LargeResultSetPaginator, PaginationMixin
...
class OrderSearch(PaginationMixin, ListView):
    ...
    def get_queryset(self):
        ...
        if not self.request.user.has_active_staff_session(self.request.session.session_key):
            qs = qs.filter(
                Q(
                    event_id__in=self.request.user.get_events_with_permission('can_view_orders').values_list(
                        'id', flat=True
                    )
                )
            )
