...
SITE_URL = os.getenv(
    "EVENTYAY_SITE_URL",
    config.get("eventyay", "url", fallback=config.get('eventyay', 'url', fallback='http://localhost')),
)
...
EVENTYAY_MULTIFACTOR_REQUIRE = (
    os.environ.get(
        "EVENTYAY_MULTIFACTOR_REQUIRE",
        str(_config.getboolean("eventyay", "multifactor_require", fallback=False)),
    )
    == "True"
)
...
MAIL_FROM = SERVER_EMAIL = DEFAULT_FROM_EMAIL = os.environ.get(
    "EVENTYAY_MAIL_FROM",
    config.get("mail", "from", fallback=config.get('mail', 'from', fallback='eventyay@localhost')),
)
