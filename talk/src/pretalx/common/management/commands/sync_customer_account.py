...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--ticket-organizer-slug", type=str, help="Ticket Organizer Slug"
        )
        parser.add_argument("--ticket-url", type=str, help="Ticket Url")
        parser.add_argument("--ticket-token", type=str, help="Ticket token")

    def handle(self, *args, **options):
        organizer_slug = options.get("ticket-organizer-slug") or input(
            "Enter Ticket Organizer Slug you want to retrieve customer: "
        )
        ...
        ticket_token = options.get("ticket-token") or input(
            "Enter Ticket token which be using for retrieve customer data from Ticket: "
        )
        try:
        ...
        try:
            ...
            self.stderr.write(
                self.style.ERROR(
                    "Error when getting customer data from Ticket, please check input data."
                )
            )
            return
        ...
        for customer in customer_dict:
            try:
                ...
                if not user:
                    ...
                    social_account.save()
                    self.stdout.write(
                        self.style.SUCCESS("Customer created: " + user.email)
                    )
            except Exception:
                self.stderr.write(
                    self.style.ERROR(
                        "Error when creating customer: " + customer["email"]
                    )
                )
                continue
        self.stdout.write(
